...
#if (                                                                      \
    configUSE_TICKLESS_IDLE != 0 && (TRC_HWTC_TYPE == TRC_OS_TIMER_INCR || \
                                     TRC_HWTC_TYPE == TRC_OS_TIMER_DECR))
/*
    The below error message is to alert you on the following issue:

    The hardware port selected in trcConfig.h uses the operating system timer
   for the timestamping, i.e., the periodic interrupt timer that drives the OS
   tick interrupt.
   ...
   independent periodic interrupt timer. See trcHardwarePort.h for details.
    ...
    For ARM Cortex-M3, M4 and M7 MCUs this is not an issue, since the recorder
   uses the DWT cycle counter for timestamping in these cases.
*/
...
extern void
                     vQueueSetQueueNumber(xQueueHandle pxQueue, unsigned char ucQueueNumber);
...
{
    vTaskSetTaskNumber(
        handle, TRACE_SET_LOW16(uxTaskGetTaskNumber(handle), value));
}

void prvTraceSetTaskNumberHigh16(void *handle, uint16_t value)
{
    vTaskSetTaskNumber(
        handle, TRACE_SET_HIGH16(uxTaskGetTaskNumber(handle), value));
}
...
void prvTraceSetQueueNumberLow16(void *handle, uint16_t value)
{
    vQueueSetQueueNumber(
        handle, TRACE_SET_LOW16(prvTraceGetQueueNumber(handle), value));
}

void prvTraceSetQueueNumberHigh16(void *handle, uint16_t value)
{
    vQueueSetQueueNumber(
        handle, TRACE_SET_HIGH16(prvTraceGetQueueNumber(handle), value));
}
...
void prvTraceSetTimerNumberLow16(void *handle, uint16_t value)
{
    vTimerSetTimerNumber(
        handle, TRACE_SET_LOW16(uxTimerGetTimerNumber(handle), value));
}

void prvTraceSetTimerNumberHigh16(void *handle, uint16_t value)
{
    vTimerSetTimerNumber(
        handle, TRACE_SET_HIGH16(uxTimerGetTimerNumber(handle), value));
}
#endif /* (TRC_CFG_INCLUDE_TIMER_EVENTS == 1 && TRC_CFG_FREERTOS_VERSION >= \
...
void prvTraceSetEventGroupNumberLow16(void *handle, uint16_t value)
{
    vEventGroupSetNumber(
        handle, TRACE_SET_LOW16(uxEventGroupGetNumber(handle), value));
}

void prvTraceSetEventGroupNumberHigh16(void *handle, uint16_t value)
{
    vEventGroupSetNumber(
        handle, TRACE_SET_HIGH16(uxEventGroupGetNumber(handle), value));
}
#endif /* (TRC_CFG_INCLUDE_EVENT_GROUP_EVENTS == 1 && TRC_CFG_FREERTOS_VERSION \
          >= TRC_FREERTOS_VERSION_10_0_0) */
...
int cortex_a9_enter_critical(void)
{
    ...
    {
        /* Executing in an ISR other than the context-switch (where interrupts
         * might have been enabled, motivating a critical section). */
    ...
    {
        // In the context-switch code. All interrupts are already masked here,
        // so don't modify the mask.
        cs_type = CS_TYPE_NONE;
    }
    else if (uiTraceSystemState != TRC_STATE_IN_TASKSWITCH)
    {
        // Not within ISR or task-switch context, use a regular critical
        // section.
        vPortEnterCritical();
...
void prvReportStackUsage()
{
    ...
    do
    {
        ...
        if (tasksInStackMonitor[i].tcb != NULL)
        {
            ...
            uint32_t unusedStackSpace = uxTaskGetStackHighWaterMark(
                (TaskType)tasksInStackMonitor[i].tcb);
            ...
            prvTraceStoreEvent2(
                PSF_EVENT_UNUSED_STACK, (uint32_t)tasksInStackMonitor[i].tcb,
                tasksInStackMonitor[i].uiPreviousLowMark);
        ...
        i = (i + 1) %
            TRC_CFG_STACK_MONITOR_MAX_TASKS; // Move i beyond this task
...
/* Creates the TzCtrl task - receives trace commands (start, stop, ...) */
    ...
    if (startOption == TRC_START_AWAIT_HOST)
    {
        ...
        do
        {
            ...
            status = TRC_STREAM_PORT_READ_DATA(
                &msg, sizeof(TracealyzerCommandType), (int32_t *)&bytes);
    ...
    {
        /* We start streaming directly - this assumes that the interface is
         * ready! */
        TRC_PORT_SPECIFIC_INIT();
...
/* If the internal buffer is disabled, the COMMIT macro instead sends the data
   directly from the "event functions" (using TRC_STREAM_PORT_WRITE_DATA). */
            ...
            /* If there is a buffer page, this sends it to the streaming
             * interface using TRC_STREAM_PORT_WRITE_DATA. */
            bytes = prvPagedEventBufferTransfer();
#endif

            /* If there was data sent or received (bytes != 0), loop around and
            repeat, if there is more data to send or receive. Otherwise, step
            out of this loop and sleep for a while. */
...
void vTraceSetQueueName(void *object, const char *name)
{
    prvTraceSetObjectName(
        TRACE_CLASS_QUEUE, TRACE_GET_OBJECT_NUMBER(QUEUE, object), name);
}
...
{
    prvTraceSetObjectName(
        TRACE_CLASS_SEMAPHORE, TRACE_GET_OBJECT_NUMBER(QUEUE, object), name);
}
...
{
    prvTraceSetObjectName(
        TRACE_CLASS_MUTEX, TRACE_GET_OBJECT_NUMBER(QUEUE, object), name);
}
...
{
    prvTraceSetObjectName(
        TRACE_CLASS_EVENTGROUP, TRACE_GET_OBJECT_NUMBER(EVENTGROUP, object),
        name);
}
#endif /* (TRC_CFG_INCLUDE_EVENT_GROUP_EVENTS == 1 && TRC_CFG_FREERTOS_VERSION \
          >= TRC_FREERTOS_VERSION_8_X) */
...
const char *pszTraceGetErrorNotEnoughHandles(traceObjectClass objectclass)
{
    switch (objectclass)
    {
        ...
        case TRACE_CLASS_SEMAPHORE:
            return "Not enough SEMAPHORE handles - increase TRC_CFG_NSEMAPHORE "
        ...
        case TRACE_CLASS_EVENTGROUP:
            return "Not enough EVENTGROUP handles - increase "
                   "TRC_CFG_NEVENTGROUP in trcSnapshotConfig.h";
        case TRACE_CLASS_STREAMBUFFER:
            return "Not enough STREAMBUFFER handles - increase "
                   "TRC_CFG_NSTREAMBUFFER in trcSnapshotConfig.h";
