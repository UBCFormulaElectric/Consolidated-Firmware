...
{
    union {
        uint32_t pSymbolTableBufferUINT32
            [SYMBOL_TABLE_BUFFER_SIZE / sizeof(uint32_t)];
...
{
    union {
        uint32_t pObjectDataTableBufferUINT32
            [OBJECT_DATA_TABLE_BUFFER_SIZE / sizeof(uint32_t)];
...
static void prvTraceStoreStringEventHelper(
    int         nArgs,
    uint16_t    eventID,
...
void prvTraceStoreSimpleStringEventHelper(
    uint16_t    eventID,
    traceString userEvtChannel,
    const char *str);
...
void vTraceStoreISREnd(int isTaskSwitchRequired)
{
    ...
    if (ISR_stack_index > 0)
    {
        ...
        prvTraceStoreEvent1(
            PSF_EVENT_ISR_RESUME, (uint32_t)ISR_stack[ISR_stack_index]);
#endif
    }
    else
    {
...
#if (TRC_CFG_INCLUDE_ISR_TRACING == 1)
            prvTraceStoreEvent1(
                PSF_EVENT_TS_RESUME, (uint32_t)TRACE_GET_CURRENT_TASK());
#endif
        }
    }
...
static void prvSetRecorderEnabled(uint32_t isEnabled)
{
    ...
    if (RecorderEnabled)
    {
        ...
        prvTraceStoreEvent3(
            PSF_EVENT_TRACE_START, (uint32_t)TRACE_GET_OS_TICKS(),
            (uint32_t)currentTask, SessionCounter++);
...
static void prvTraceStoreSymbolTable(void)
{
    ...
    if (RecorderEnabled)
    {
        ...
        {
            TRC_STREAM_PORT_ALLOCATE_EVENT_BLOCKING(
                uint32_t, data, SYMBOL_TABLE_SLOT_SIZE);

            for (j = 0; j < (SYMBOL_TABLE_SLOT_SIZE / sizeof(uint32_t)); j++)
            {
                data[j] = symbolTable.SymbolTableBuffer
                              .pSymbolTableBufferUINT32[i + j];
            }
...
static void prvTraceStoreObjectDataTable(void)
{
    ...
    if (RecorderEnabled)
    {
        ...
        {
            TRC_STREAM_PORT_ALLOCATE_EVENT_BLOCKING(
                uint32_t, data, OBJECT_DATA_SLOT_SIZE);
...
static void prvTraceStoreHeader(void)
{
    ...
    {
        TRC_STREAM_PORT_ALLOCATE_EVENT_BLOCKING(
            PSFHeaderInfo, header, sizeof(PSFHeaderInfo));
...
static void prvTraceStoreExtensionInfo(void)
{
    ...
    {
        TRC_STREAM_PORT_ALLOCATE_EVENT_BLOCKING(
            PSFExtensionInfoType, extinfo, sizeof(PSFExtensionInfoType));
...
static const char *prvTraceGetError(int errCode)
{
    ...
    switch (errCode)
    {
        case PSF_WARNING_SYMBOL_TABLE_SLOTS:
            ...
            The number of missing slots is counted by NoRoomForSymbol. Inspect
            this variable and increase TRC_CFG_SYMBOL_TABLE_SLOTS by at least
            that value. */
        ...
        case PSF_WARNING_STREAM_PORT_READ:
            ...
            successfully.
            This means there is an error in the communication with
            host/Tracealyzer. */
        ...
        case PSF_WARNING_STREAM_PORT_INITIAL_BLOCKING:
            /* Blocking occurred during vTraceEnable. This happens if the trace
            buffer is smaller than the initial transmission (trace header,
            object table, and symbol table). */
        ...
        case PSF_ERROR_EVENT_CODE_TOO_LARGE:
            ...
            unexpected error. Please contact support@percepio.com for
            assistance.*/
        ...
        case PSF_ERROR_ISR_NESTING_OVERFLOW:
            ...
            (TRC_CFG_MAX_ISR_NESTING). If this is unlikely, make sure that you
            call vTraceStoreISRExit in the end
        ...
        case PSF_ERROR_DWT_NOT_SUPPORTED:
            /* On ARM Cortex-M only - failed to initialize DWT Cycle Counter
            since not supported by this chip. DWT timestamping is selected
            automatically for ART Cortex-M3, M4 and higher, based on the
            __CORTEX_M macro normally set by ARM's CMSIS library, since
            typically available. You can however select SysTick timestamping
            instead by defining adding "#define TRC_CFG_ARM_CM_USE_SYSTICK".*/
    ...
    if (RecorderEnabled)
    {
...

        {
            TRC_STREAM_PORT_ALLOCATE_EVENT(
                EventWithParam_1, event, sizeof(EventWithParam_1));
            if (event != NULL)
            {
...
void prvTraceStoreEvent2(uint16_t eventID, uint32_t param1, uint32_t param2)
{
    ...
    if (RecorderEnabled)
    {
        ...
        {
            TRC_STREAM_PORT_ALLOCATE_EVENT(
                EventWithParam_2, event, sizeof(EventWithParam_2));
            if (event != NULL)
            {
    ...
    uint32_t param3)
{
    ...
    if (RecorderEnabled)
    {
        ...
        {
            TRC_STREAM_PORT_ALLOCATE_EVENT(
                EventWithParam_3, event, sizeof(EventWithParam_3));
            if (event != NULL)
            {
    ...
    if (RecorderEnabled)
    {
...

        {
            TRC_STREAM_PORT_ALLOCATE_DYNAMIC_EVENT(
                largestEventType, event, eventSize);
            if (event != NULL)
            {
    ...
    if (RecorderEnabled)
    {
...

        {
            ...
            if (event != NULL)
            {
                ...
                if (len < (15 * 4 - offset))
                    data8[offset + len] = 0; /* Only truncate if we don't fill
                                                up the buffer completely */
                TRC_STREAM_PORT_COMMIT_EVENT(event, (uint32_t)eventSize);
            }
        }
    }
...
void prvTraceSaveObjectSymbol(void *address, const char *name)
{
    ...
    if (firstFreeSymbolTableIndex < SYMBOL_TABLE_BUFFER_SIZE)
    {
        ...
        symbolTable.SymbolTableBuffer.pSymbolTableBufferUINT32
            [firstFreeSymbolTableIndex / sizeof(uint32_t)] = (uint32_t)address;

        /* We access the symbol table via the union member
         * pSymbolTableBufferUINT8 to avoid strict-aliasing issues */
        ptrSymbol = &symbolTable.SymbolTableBuffer.pSymbolTableBufferUINT8
                         [firstFreeSymbolTableIndex + sizeof(uint32_t)];
        for (i = 0; i < (TRC_CFG_SYMBOL_MAX_LENGTH); i++)
        {
            ptrSymbol[i] =
                (uint8_t)name[i]; /* We do this first to ensure we also get the
                                     0 termination, if there is one */
...
void prvTraceDeleteSymbol(void *address)
{
    ...
    for (i = 0; i < firstFreeSymbolTableIndex; i += SYMBOL_TABLE_SLOT_SIZE)
    {
        ...
        {
            /* See if we have another entry in the table, and that this isn't
                ...
                i != (firstFreeSymbolTableIndex - SYMBOL_TABLE_SLOT_SIZE))
            {
                ...
                for (j = 0; j < (SYMBOL_TABLE_SLOT_SIZE) / sizeof(uint32_t);
                     j++)
                {
...
void prvTraceSaveObjectData(const void *address, uint32_t data)
{
    ...
    if (foundSlot < OBJECT_DATA_TABLE_BUFFER_SIZE)
    {
        ...
        objectDataTable.ObjectDataTableBuffer
            ...
            (uint32_t)address;
        objectDataTable.ObjectDataTableBuffer
            .pObjectDataTableBufferUINT32[foundSlot / sizeof(uint32_t) + 1] =
            data;
...
int prvIsValidCommand(TracealyzerCommandType *cmd)
{
    uint16_t checksum = (uint16_t)(
        0xFFFF - (cmd->cmdCode + cmd->param1 + cmd->param2 + cmd->param3 +
...
void prvTraceError(int errCode)
{
    ...
    {
        /* Will never reach this point more than once, since we verify if
         * errorCode has already been set */
        errorCode = errCode;
        SET_ERROR_WARNING_FLAG(errorCode);

        prvTraceStoreSimpleStringEventHelper(
            PSF_EVENT_USER_EVENT, trcWarningChannel,
            prvTraceGetError(errorCode));
        prvTraceStoreSimpleStringEventHelper(
            PSF_EVENT_USER_EVENT, trcWarningChannel,
...
static void prvTraceStoreTSConfig(void)
{
    ...
    prvTraceStoreEvent(
        5, PSF_EVENT_TS_CONFIG, (uint32_t)timestampFrequency,
        (uint32_t)(TRACE_TICK_RATE_HZ), (uint32_t)(TRC_HWTC_TYPE),
    ...
    prvTraceStoreEvent(
        4, PSF_EVENT_TS_CONFIG, (uint32_t)timestampFrequency,
        (uint32_t)(TRACE_TICK_RATE_HZ), (uint32_t)(TRC_HWTC_TYPE),
        (uint32_t)(TRC_CFG_ISR_TAILCHAINING_THRESHOLD));
#endif
}
...
static int prvGetBufferPage(int32_t *bytesUsed)
{
    ...
    {
        *bytesUsed = (TRC_CFG_PAGED_EVENT_BUFFER_PAGE_SIZE)-PageInfo[index]
                         .BytesRemaining;
        lastPage = index;
        return index;
    }
...
uint32_t prvPagedEventBufferTransfer(void)
{
    ...
    page, that should be transmitted. There might be some unused junk bytes in
    the end, that must be ignored. */

    if (pageToTransfer > -1)
    {
        while (1) /* Keep going until we have transferred all that we intended
                     to */
        {
                    ...
                    &bytesTransferredNow) == 0)
            {
                ...
                {
                    /* All bytes have been transferred. Mark the buffer page as
                     * "Read Complete" (so it can be written to) and return OK.
                     */
