...
void App_SharedClock_Destroy(struct Clock *clock) { free(clock); }

void App_SharedClock_SetCurrentTimeInMilliseconds(struct Clock *const clock,
                                                  uint32_t current_time_ms) {
  clock->current_time_ms = current_time_ms;
}

void App_SharedClock_SetPreviousTimeInMilliseconds(struct Clock *const clock,
                                                   uint32_t previous_time_ms) {
...
uint32_t App_SharedClock_GetCurrentTimeInSeconds(
    const struct Clock *const clock) {
...
uint32_t App_SharedClock_GetPreviousTimeInSeconds(
    const struct Clock *const clock) {
