...
void App_RingQueue_Push(RingQueue *queue, uint8_t value) {
  ...
  // The queue has overflowed if the tail idx has "circled around" and caught up
  // to the head idx, so increment the head idx. Also check there are elements
  // in queue so this is not set for an empty queue (which has tail and head idx
  // equal).
  const bool queue_overflowed =
      (queue->tail == queue->head) && queue->count > 0;
  if (queue_overflowed) {
    // Count doesn't change if overflowed, since a new element replaced an old
    // element
    queue->head += 1;
...
bool App_RingQueue_Pop(RingQueue *queue, uint8_t *value) {
  ...
  *value = queue->elements[queue->head];
  ...
  // Move head to next-oldest value for future pop operations, and shrink size
  // by 1
  queue->head += 1;
  queue->head %= queue->size;
  queue->count -= 1;
