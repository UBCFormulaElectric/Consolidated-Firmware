...
struct WaitSignal *App_SharedWaitSignal_Create(
    ...
    struct World *const world, struct WaitSignalCallback callback) {
...
bool App_SharedWaitSignal_IsWaiting(
    const struct WaitSignal *const wait_signal) {
  return wait_signal->is_waiting;
}

void App_SharedWaitSignal_Update(struct WaitSignal *const wait_signal,
                                 uint32_t current_ms) {
