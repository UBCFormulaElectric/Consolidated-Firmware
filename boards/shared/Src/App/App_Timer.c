...
TimerState App_Timer_UpdateAndGetState(TimerChannel *const timer) {
  ...
  if (timer->state == TIMER_STATE_RUNNING &&
      App_Timer_GetElapsedTime(timer) >= timer->duration_ms) {
...
uint32_t App_Timer_GetElapsedTime(const TimerChannel *const timer) {
  ...
  switch (timer->state) {
    case TIMER_STATE_RUNNING: {
      ...
      elapsed_time = (total_elapsed_time > timer->duration_ms)
                         ? timer->duration_ms
                         : total_elapsed_time;
      break;
    }
    case TIMER_STATE_EXPIRED: {
      ...
      // It'd be confusing if elapsed time continued to run after expiry, since
      // these timers are supposed to be akin to stopwatches
...
void App_Timer_SetCurrentTimeMS(uint32_t time) { current_time_ms = time; }

uint32_t App_Timer_GetCurrentTimeMS() { return current_time_ms; }