...
#if (                                           \
    (TRC_HWTC_TYPE == TRC_CUSTOM_TIMER_INCR) || \
...
static void
                prvTraceGetChecksum(const char *pname, uint8_t *pcrc, uint8_t *plength);
static void *   prvTraceNextFreeEventBufferSlot(void);
static uint16_t prvTraceGetDTS(uint16_t param_maxDTS);
static traceString
            prvTraceOpenSymbol(const char *name, traceString userEventChannel);
static void prvTraceUpdateCounters(void);

void vTraceStoreMemMangEvent(
    uint32_t ecode,
    uint32_t address,
    int32_t  signed_size);
...
static traceString prvTraceCreateSymbolTableEntry(
    const char *name,
    uint8_t     crc6,
    uint8_t     len,
    traceString channel);

static traceString prvTraceLookupSymbolTableEntry(
    const char *name,
    uint8_t     crc6,
    uint8_t     len,
    traceString channel);
...
void vTraceSetRecorderDataBuffer(void *pRecorderData)
{
    TRACE_ASSERT(
        pRecorderData != NULL,
        "vTraceSetRecorderDataBuffer, pRecorderData == NULL", TRC_UNUSED);
...
uint32_t uiTraceStart(void)
{
    ...
    if (traceErrorMessage == NULL)
    {
        ...
        prvTraceStoreTaskswitch(
            handle); /* Register the currently running task */
        trcCRITICAL_SECTION_END();
    }
...
traceHandle xTraceSetISRProperties(const char *name, uint8_t priority)
{
    ...
    TRACE_ASSERT(
        RecorderDataPtr != NULL,
        "Recorder not initialized, call vTraceEnable() first!", (traceHandle)0);
    TRACE_ASSERT(
...
void vTraceStoreISRBegin(traceHandle handle)
{
    ...
    if (recorder_busy)
    {
         ...
         * recorder's internal critical sections (TRACE_ENTER_CRITICAL_SECTION
         *and TRACE_EXIT_CRITICAL_SECTION). They must disable the RTOS tick
         *interrupt and any other ISRs that calls the trace recorder directly or
         *via traced kernel functions. The ARM port disables all interrupts
         *using the PRIMASK register to avoid this issue.
         *************************************************************************/
    ...
    if (RecorderDataPtr->recorderActive && handle_of_last_logged_task)
    {
        ...
        TRACE_ASSERT(
            handle != 0, "vTraceStoreISRBegin: Invalid ISR handle (NULL)",
            TRC_UNUSED);
        TRACE_ASSERT(
            handle <= RecorderDataPtr->ObjectPropertyTable
                          .NumberOfObjectsPerClass[TRACE_CLASS_ISR],
...
void vTraceStoreISREnd(int pendingISR)
{
    ...
    if (nISRactive == 0)
    {
        prvTraceError("Unmatched call to vTraceStoreISREnd (nISRactive == 0, "
                      "expected > 0)");
        return;
    }
    ...
    if (nISRactive > 0)
    {
        ...
        hnd8 = prvTraceGet8BitHandle(
            isrstack[nISRactive - 1]); /* isrstack[nISRactive] is the handle of
                                          the ISR we're currently exiting.
                                          isrstack[nISRactive - 1] is the handle
                                          of the ISR that was executing
                                          previously. */
    }
    else if ((isPendingContextSwitch == 0) || (prvTraceIsSchedulerSuspended()))
    {
        /* Return to interrupted task, if no context switch will occur in
         * between. */
        type = TS_TASK_RESUME;
...
#if (                                                                       \
    (TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_USER_EVENTS == 1) && \
    ...
    uint8_t     byteOffset)
{
    ...
    while (formatStr[formatStrIndex] != '\0')
    {
        if (formatStr[formatStrIndex] == '%')
        {
            ...
            if (formatStr[formatStrIndex] != '\0')
            {
                switch (formatStr[formatStrIndex])
                {
                    case 'd':
                        i = writeInt32(
                            buffer, i, (uint32_t)va_arg(vl, uint32_t));
                        break;
                    case 'x':
                    case 'X':
                    case 'u':
                    ...
                    case 'f':
                        ...
                        break;
#else
                        /* No support for floats, but attempt to store a float
                        user event avoid a possible crash due to float
                        reference. Instead store the data on uint_32 format
                        (will not be displayed anyway). This is just to keep
                        va_arg and i consistent. */
                    ...
                    case 'l':
                        ...
                        switch (formatStr[formatStrIndex])
                        {
                            ...
                            case 'f':
                                i = writeDouble(
                                    buffer, i, (double)va_arg(vl, double));
                                break;
#else
                            /* No support for floats, but attempt to store a
                            float user event avoid a possible crash due to float
                            reference. Instead store the data on uint_32 format
                            (will not be displayed anyway). This is just
                            to keep va_arg and i consistent. */
                            case 'f':
                                i = writeInt32(
                                    buffer, /* In this case, the value will not
                                               be shown anyway */
                                    i, (uint32_t)va_arg(vl, double));

                                i = writeInt32(
                                    buffer, /* Do it twice, to write in total 8
                                               bytes */
                                    i, (uint32_t)va_arg(vl, double));
                                break;
#endif
                    ...
                    case 'h':
                        ...
                        switch (formatStr[formatStrIndex])
                        {
                            case 'd':
                                i = writeInt16(
                                    buffer, i, (uint16_t)va_arg(vl, uint32_t));
                                break;
                            case 'u':
                                i = writeInt16(
                                    buffer, i, (uint16_t)va_arg(vl, uint32_t));
                    ...
                    case 'b':
                        ...
                        switch (formatStr[formatStrIndex])
                        {
                            case 'd':
                                i = writeInt8(
                                    buffer, i, (uint8_t)va_arg(vl, uint32_t));
                                break;
                            case 'u':
                                i = writeInt8(
                                    buffer, i, (uint8_t)va_arg(vl, uint32_t));
                ...
                {
                    prvTraceError(
                        "vTracePrintF - Too many arguments, max 15 allowed!");
                    return 0;
                }
            }
            else
        ...
        {
            prvTraceError(
                "vTracePrintF - Too large arguments, max 32 byte allowed!");
            return 0;
        }
    }
...
static void prvTraceClearChannelBuffer(uint32_t count)
{
    ...
    TRACE_ASSERT(
        ...
        "prvTraceClearChannelBuffer: TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE "
        "is too small to handle this event.",
        TRC_UNUSED);
        ...
        (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE))
    {
        ...
        (void)memset(
            &RecorderDataPtr->userEventBuffer.channelBuffer
                 [RecorderDataPtr->userEventBuffer.nextSlotToWrite],
            0, slots);
        (void)memset(
            &RecorderDataPtr->userEventBuffer.channelBuffer[0], 0,
            (count - slots));
    }
    else
        (void)memset(
...
static void prvTraceCopyToDataBuffer(uint32_t *data, uint32_t count)
{
    ...
    TRACE_ASSERT(
        data != NULL, "prvTraceCopyToDataBuffer: data == NULL.", TRC_UNUSED);
    TRACE_ASSERT(
        ...
        (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE))
    {
        slots = (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE)-RecorderDataPtr
                    ...
                    .nextSlotToWrite; /* Number of slots before end of buffer */
        (void)memcpy(
            &RecorderDataPtr->userEventBuffer.dataBuffer
                 [RecorderDataPtr->userEventBuffer.nextSlotToWrite * 4],
            data, slots * 4);
        (void)memcpy(
            &RecorderDataPtr->userEventBuffer.dataBuffer[0], data + slots,
            (count - slots) * 4);
    }
    else
    {
    ...
    uint32_t       noOfSlots)
{
    ...
    TRACE_ASSERT(
        (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE) >= noOfSlots,
        "prvTraceUBHelper2: TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE is too "
        "small to handle this event.",
        TRC_UNUSED);
    ...
    old_nextSlotToWrite =
        RecorderDataPtr->userEventBuffer
            .nextSlotToWrite; /* Save the index that we want to write the
                                 channel data at when we're done */
    RecorderDataPtr->userEventBuffer.nextSlotToWrite =
    ...
    /* Write to the channel buffer to indicate that this user event is ready to
     * be used */
    if (channel != 0)
    {
    ...
    xTraceRegisterUBChannel(traceString channel, traceString formatStr)
{
    ...
    TRACE_ASSERT(
        formatStr != 0, "xTraceRegisterChannelFormat: formatStr == 0",
        (traceUBChannel)0);
...
void vTraceUBData(traceUBChannel channelPair, ...)
{
    ...
    TRACE_ASSERT(
        channelPair != 0, "vTraceUBData: Not a valid traceUBChannel!",
        TRC_UNUSED);
...
void vTraceUBData_Helper(traceUBChannel channelPair, va_list vl)
{
    ...
    TRACE_ASSERT(
        channelPair != 0, "vTraceUBData_Helper: channelPair == 0", TRC_UNUSED);
    TRACE_ASSERT(
        channelPair <= (TRC_CFG_UB_CHANNELS),
...
void vTraceUBEvent(traceUBChannel channelPair)
{
    ...
    TRACE_ASSERT(
        channelPair != 0, "vTraceUBEvent: channelPair == 0", TRC_UNUSED);
    TRACE_ASSERT(
        channelPair <= (TRC_CFG_UB_CHANNELS), "vTraceUBEvent: ", TRC_UNUSED);

    prvTraceUBHelper2(
        channelPair, data, 1); /* Only need one slot for timestamp */
}
#endif
...
void vTraceVPrintF(traceString eventLabel, const char *formatStr, va_list vl)
{
    ...
    TRACE_ASSERT(
        formatStr != NULL, "vTraceVPrintF: formatStr == NULL", TRC_UNUSED);
    ...
    if (RecorderDataPtr->recorderActive && handle_of_last_logged_task)
    {
        ...
        noOfSlots = prvTraceUserEventFormat(
            formatStr, vl, (uint8_t *)tempDataBuffer, 4);
        ...
        {
            /* If the data does not fit in the remaining main buffer, wrap
            around to 0 if allowed, otherwise stop the recorder and quit). */
...
using a memset to zero).*/
                ...
                /* Update the event type, i.e., number of data entries following
                the main USER_EVENT entry (Note: important that this is after
                the memcpy, but within the critical section!)*/
                RecorderDataPtr->eventData[RecorderDataPtr->nextFreeIndex * 4] =
                    (uint8_t)(USER_EVENT + noOfSlots - 1);

                /* Update the main buffer event index (already checked that it
                fits in the buffer, so no need to check for wrapping)*/

                RecorderDataPtr->nextFreeIndex += noOfSlots;
                RecorderDataPtr->numEvents += noOfSlots;

                if (RecorderDataPtr->nextFreeIndex >=
                    (TRC_CFG_EVENT_BUFFER_SIZE))
                {
#if (TRC_CFG_SNAPSHOT_MODE == TRC_SNAPSHOT_MODE_RING_BUFFER)
                    /* We have reached the end, but this is a ring buffer. Start
...
{
    TRACE_ASSERT(
        label != NULL, "xTraceRegisterString: label == NULL", (traceString)0);
    TRACE_ASSERT(
        RecorderDataPtr != NULL,
        "Recorder not initialized, call vTraceEnable() first!", (traceHandle)0);
...
#if (                                          \
    (!defined TRC_CFG_INCLUDE_READY_EVENTS) || \
...
void prvTraceStoreLowPower(uint32_t flag)
{
    ...
    TRACE_ASSERT(
        flag <= 1, "prvTraceStoreLowPower: Invalid flag value", TRC_UNUSED);
    ...
    uint32_t         objectNumber)
{
    ...
    TRACE_ASSERT(
        ecode < 0xFF, "prvTraceStoreKernelCall: ecode >= 0xFF", TRC_UNUSED);
    TRACE_ASSERT(
        objectClass < TRACE_NCLASSES,
        "prvTraceStoreKernelCall: objectClass >= TRACE_NCLASSES", TRC_UNUSED);
    TRACE_ASSERT(
    ...
    uint32_t         param)
{
    ...
    TRACE_ASSERT(
        evtcode < 0xFF, "prvTraceStoreKernelCallWithParam: evtcode >= 0xFF",
        TRC_UNUSED);
    TRACE_ASSERT(
        objectClass < TRACE_NCLASSES,
...
static uint32_t prvTraceGetParam(uint32_t param_max, uint32_t param)
{
    ...
    TRACE_ASSERT(
        param_max == 0xFF || param_max == 0xFFFF,
    ...
    uint32_t param)
{
    ...
    TRACE_ASSERT(
        evtcode < 0xFF,
        "prvTraceStoreKernelCallWithNumericParamOnly: Invalid value for "
        "evtcode",
        TRC_UNUSED);
    ...
    uint8_t     value)
{
    TRACE_ASSERT(
        ...
        TRC_UNUSED);
    TRACE_ASSERT(
        id <= RecorderDataPtr->ObjectPropertyTable
                  .NumberOfObjectsPerClass[objectclass],
...
objectHandleStackType objectHandleStacks = { { 0 },
                                             { 0 },
                                             { 0 },
                                             { 0 },
                                             { 0 } };
...
void prvTraceInitTraceData()
{
    ...
    TRC_RECORDER_BUFFER_ALLOCATION_DYNAMIC)
        RecorderDataPtr =
            (RecorderDataType *)TRACE_MALLOC(sizeof(RecorderDataType));
        if (!RecorderDataPtr)
        {
    ...
    TRC_RECORDER_BUFFER_ALLOCATION_CUSTOM)
        if (!RecorderDataPtr)
        {
            prvTraceError("Recorder data pointer not set! Use "
                          "vTraceSetRecorderDataBuffer().");
            return;
        }
#endif
    ...
    RecorderDataPtr->userEventBuffer.numberOfSlots =
        (TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE);
    RecorderDataPtr->userEventBuffer.numberOfChannels =
        (TRC_CFG_UB_CHANNELS) + 1;
#endif
    ...
    The start and end markers must be unique, but without proper precautions
    there might be a risk of accidental duplicates of the start/end markers,
    e.g., due to compiler optimizations.
    ...
    if (traceErrorMessage != NULL)
    {
        // An error was detected before vTraceEnable was called, make sure this
        // is stored in the trace data.
    ...
    const char *     name)
{
    ...
    TRACE_ASSERT(
        name != NULL, "prvTraceSetObjectName: name == NULL", TRC_UNUSED);
    ...
    else
    {
        ...
        {
            prvStrncpy(
                (char *)&(RecorderDataPtr->ObjectPropertyTable.objbytes[idx]),
                name,
                RecorderDataPtr->ObjectPropertyTable
...
traceString prvTraceOpenSymbol(const char *name, traceString userEventChannel)
{
    ...
    TRACE_ASSERT(
        name != NULL, "prvTraceOpenSymbol: name == NULL", (traceString)0);
    ...
    {
        result =
            prvTraceCreateSymbolTableEntry(name, crc, len, userEventChannel);
    }
    trcCRITICAL_SECTION_END();
...
void prvCheckDataToBeOverwrittenForMultiEntryEvents(uint8_t nofEntriesToCheck)
{
    ...
    TRACE_ASSERT(
        nofEntriesToCheck != 0,
        "prvCheckDataToBeOverwrittenForMultiEntryEvents: nofEntriesToCheck == "
        "0",
        TRC_UNUSED);
    ...
    while (i < nofEntriesToCheck)
    {
            ...
            (RecorderDataPtr->eventData[e * 4] < USER_EVENT + 16))
        {
            ...
            {
                (void)memset(
                    &RecorderDataPtr->eventData[e * 4], 0,
...
uint16_t prvTraceGetDTS(uint16_t param_maxDTS)
{
    ...
    TRACE_ASSERT(
        param_maxDTS == 0xFF || param_maxDTS == 0xFFFF,
    ...
    if (RecorderDataPtr->frequency == 0)
    {
        if (timestampFrequency != 0)
        {
            ...
            RecorderDataPtr->frequency =
                timestampFrequency / (TRC_HWTC_DIVISOR);
        }
        else if (init_hwtc_count != (TRC_HWTC_COUNT))
        {
            /* If using default value and timer has been started.
            Note: If the default frequency value set here would be incorrect,
            e.g., if the timer has actually not been configured yet, override
            this with vTraceSetFrequency.
            */
            RecorderDataPtr->frequency =
    ...
    if (RecorderDataPtr->frequency > 0)
    {
        ...
        if (dts > RecorderDataPtr->frequency)
        {
            ...
            /* The part that is not an entire second is added to
             * absTimeLastEvent */
        ...
        {
            /* RecorderDataPtr->absTimeLastEvent is more than or equal to 1
             * second, but always less than 2 seconds */
            RecorderDataPtr->absTimeLastEventSecond++;
    ...
    /* If the dts (time since last event) does not fit in event->dts (only 8 or
     * 16 bits) */
    if (dts > param_maxDTS)
    {
        /* Create an XTS event (eXtended TimeStamp) containing the higher dts
         * bits*/
 ...
 * zero-termination
 ******************************************************************************/
traceString prvTraceLookupSymbolTableEntry(
    const char *name,
    uint8_t     crc6,
    uint8_t     len,
    traceString chn)
{
    ...
    TRACE_ASSERT(
        name != NULL, "prvTraceLookupSymbolTableEntry: name == NULL",
        (traceString)0);
    TRACE_ASSERT(
        len != 0, "prvTraceLookupSymbolTableEntry: len == 0", (traceString)0);
    ...
    while (i != 0)
    {
        if (RecorderDataPtr->SymbolTable.symbytes[i + 2] == (chn & 0x00FF))
        {
            if (RecorderDataPtr->SymbolTable.symbytes[i + 3] == (chn / 0x100))
            {
                ...
                {
                    if (strncmp(
                            (char *)(&RecorderDataPtr->SymbolTable
                                          .symbytes[i + 4]),
                            name, len) == 0)
                    {
                        break; /* found */
        ...
        i = (uint16_t)(
            RecorderDataPtr->SymbolTable.symbytes[i] +
 ...
 * zero-termination
 ******************************************************************************/
uint16_t prvTraceCreateSymbolTableEntry(
    const char *name,
    uint8_t     crc6,
    uint8_t     len,
    traceString channel)
{
    ...
    TRACE_ASSERT(
        name != NULL, "prvTraceCreateSymbolTableEntry: name == NULL", 0);
    ...
    {
        prvTraceError("Symbol table full. Increase TRC_CFG_SYMBOL_TABLE_SIZE "
                      "in trcConfig.h");
        ret = 0;
    }
    else
    {
        RecorderDataPtr->SymbolTable
            .symbytes[RecorderDataPtr->SymbolTable.nextFreeSymbolIndex] =
            (uint8_t)(
...
void prvTraceGetChecksum(const char *pname, uint8_t *pcrc, uint8_t *plength)
{
    ...
    TRACE_ASSERT(
        pname != NULL, "prvTraceGetChecksum: pname == NULL", TRC_UNUSED);
    TRACE_ASSERT(pcrc != NULL, "prvTraceGetChecksum: pcrc == NULL", TRC_UNUSED);
    TRACE_ASSERT(
        plength != NULL, "prvTraceGetChecksum: plength == NULL", TRC_UNUSED);
...
void prvTraceInitCortexM()
{
    ...
    do
    {
        ...
        if (TRC_REG_DEMCR == 0)
        {
            /* This function is called on Cortex-M3, M4 and M7 devices to
            initialize the DWT unit, assumed present. The DWT cycle counter is
            used for timestamping.

            If the below error is produced, the DWT unit does not seem to be
            available.

            In that case, define the macro TRC_CFG_ARM_CM_USE_SYSTICK in your
            build to use SysTick timestamping instead, or define your own
            timestamping by setting TRC_CFG_HARDWARE_PORT to
            TRC_HARDWARE_PORT_APPLICATION_DEFINED and make the necessary
            definitions, as explained in trcHardwarePort.h.*/
        ...
        if (TRC_REG_DWT_CTRL & TRC_DWT_CTRL_NOCYCCNT)
        {
            ...
            If the below error is produced, the cycle counter does not seem to
            be available.
...
#if (                                     \
    TRC_HWTC_TYPE == TRC_OS_TIMER_INCR || \
...
#error "TRC_HWTC_TYPE has unexpected value"
    ...
    if (pTimestamp)
    {
        /* Get timestamp from trace ticks. Scale down the period to avoid
         * unwanted overflows. */
        last_timestamp =
            traceTickCount * ((TRC_HWTC_PERIOD) / (TRC_HWTC_DIVISOR));
    ...
    /* This part handles free running clocks that can be scaled down to avoid
    too large DTS values. Without this, the scaled timestamp will incorrectly
    wrap at (2^32 / TRC_HWTC_DIVISOR) ticks.
    The scaled timestamp returned from this function is supposed to go from 0 ->
    2^32, which in real time would represent (0 -> 2^32 * TRC_HWTC_DIVISOR)
    ticks. */

    /* First we see how long time has passed since the last timestamp call, and
     * we also add the ticks that was lost when we scaled down the last time. */
    diff = (hwtc_count - last_hwtc_count) + last_hwtc_rest;

    /* Scale down the diff */
    diff_scaled = diff / (TRC_HWTC_DIVISOR);

    /* Find out how many ticks were lost when scaling down, so we can add them
     * the next time */
