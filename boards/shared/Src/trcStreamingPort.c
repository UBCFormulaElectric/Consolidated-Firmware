...
int32_t readFromRTT(void *ptrData, uint32_t size, int32_t *ptrBytesRead)
{
    ...
    {
        bytesRead = SEGGER_RTT_Read(
            (TRC_CFG_RTT_DOWN_BUFFER_INDEX), (char *)ptrData, size);
...
{
    uint32_t bytesWritten = SEGGER_RTT_Write(
        (TRC_CFG_RTT_UP_BUFFER_INDEX), (const char *)ptrData, size);
