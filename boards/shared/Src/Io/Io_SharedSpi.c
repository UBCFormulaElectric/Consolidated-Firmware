...
struct SharedSpi *Io_SharedSpi_Create(SPI_HandleTypeDef *spi_handle,
...
void Io_SharedSpi_SetNssLow(const struct SharedSpi *const spi_interface) {
  HAL_GPIO_WritePin(spi_interface->nss_port, spi_interface->nss_pin,
                    GPIO_PIN_RESET);
}
...
bool Io_SharedSpi_TransmitAndReceive(
    const struct SharedSpi *const spi_interface, uint8_t *tx_buffer,
    uint16_t tx_buffer_size, uint8_t *rx_buffer, uint16_t rx_buffer_size)

{
  ...
  if (HAL_SPI_Transmit(spi_interface->spi_handle, tx_buffer, tx_buffer_size,
                       spi_interface->timeout_ms) == HAL_OK) {
    status =
        HAL_SPI_Receive(spi_interface->spi_handle, rx_buffer, rx_buffer_size,
                        spi_interface->timeout_ms) == HAL_OK;
  }
...
bool Io_SharedSpi_Transmit(const struct SharedSpi *const spi_interface,
                           uint8_t *tx_buffer, uint16_t tx_buffer_size) {
  Io_SharedSpi_SetNssLow(spi_interface);
  const bool status =
      HAL_SPI_Transmit(spi_interface->spi_handle, tx_buffer, tx_buffer_size,
                       spi_interface->timeout_ms) == HAL_OK;
  Io_SharedSpi_SetNssHigh(spi_interface);
...
bool Io_SharedSpi_Receive(const struct SharedSpi *const spi_interface,
...
bool Io_SharedSpi_MultipleTransmitWithoutNssToggle(
    const struct SharedSpi *const spi_interface, uint8_t *tx_buffer,
    uint16_t tx_buffer_size, size_t num_tx_data_copies) {
...
bool Io_SharedSpi_TransmitWithoutNssToggle(
    const struct SharedSpi *const spi_interface, uint8_t *tx_data,
    uint16_t tx_buffer_size) {
  return HAL_SPI_Transmit(spi_interface->spi_handle, tx_data, tx_buffer_size,
                          spi_interface->timeout_ms) == HAL_OK;
}
