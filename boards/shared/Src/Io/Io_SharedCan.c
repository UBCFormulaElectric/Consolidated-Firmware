...
static HAL_StatusTypeDef Io_TransmitCanMessage(struct CanMsg *message) {
  ...
  return HAL_CAN_AddTxMessage(sharedcan_hcan, &tx_header, message->data,
                              &mailbox);
}
...
static inline void Io_CanRxCallback(CAN_HandleTypeDef *hcan, uint32_t rx_fifo) {
  ...
  if (HAL_CAN_GetRxMessage(hcan, rx_fifo, &header, &message.data[0]) ==
    ...
    if (Io_CanRx_FilterMessageId(header.StdId) == true) {
      ...
      if (xQueueSendToBackFromISR(can_rx_msg_fifo.handle, &message, NULL) !=
          pdPASS) {
...
void Io_SharedCan_Init(CAN_HandleTypeDef *hcan,
  ...
  can_tx_msg_fifo.handle =
      xQueueCreateStatic(CAN_TX_MSG_FIFO_LENGTH, CAN_TX_MSG_FIFO_ITEM_SIZE,
  ...
  can_rx_msg_fifo.handle =
      xQueueCreateStatic(CAN_RX_MSG_FIFO_LENGTH, CAN_RX_MSG_FIFO_ITEM_SIZE,
  ...
  assert(HAL_CAN_ActivateNotification(
...
void Io_SharedCan_TxMessageQueueSendtoBack(const struct CanMsg *message) {
  ...
  if (xPortIsInsideInterrupt()) {
    ...
    } else if (uxQueueMessagesWaitingFromISR(CanTxBinarySemaphore.handle) ==
               0U) {
