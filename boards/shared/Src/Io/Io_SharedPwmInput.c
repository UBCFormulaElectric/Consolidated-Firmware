 ...
 */
static void Io_SetDutyCycle(struct PwmInput *const pwm_input,
 ...
 */
static void Io_SetFrequency(struct PwmInput *const pwm_input,
...
void Io_SharedPwmInput_Tick(struct PwmInput *const pwm_input) {
  ...
  if (ic_rising_edge != 0U) {
    ...
    Io_SetDutyCycle(pwm_input,
                    (float)ic_falling_edge * 100.0f / (float)ic_rising_edge);

    Io_SetFrequency(pwm_input,
                    pwm_input->timer_frequency_hz / (float)ic_rising_edge);
  } else {
