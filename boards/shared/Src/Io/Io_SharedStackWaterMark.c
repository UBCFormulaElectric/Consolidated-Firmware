 ...
 */
static bool Io_IsStackAboveWatermarkThreshold(TaskHandle_t xTask,
                                              uint32_t stack_size,
...
void Io_SharedStackWaterMark_Check(struct stack_watermark *stacks,
                                   size_t num_of_stacks) {
  for (size_t i = 0; i < num_of_stacks; i++) {
    if (Io_IsStackAboveWatermarkThreshold(*(stacks[i].handle),
                                          stacks[i].stack_size,
