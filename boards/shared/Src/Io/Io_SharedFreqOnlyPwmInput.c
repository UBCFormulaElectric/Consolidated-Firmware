 ...
 */
static void Io_SetFrequency(struct FreqOnlyPwmInput *pwm_input,
                            float frequency_hz);

static void Io_SetFrequency(struct FreqOnlyPwmInput *const pwm_input,
...
struct FreqOnlyPwmInput *Io_SharedFreqOnlyPwmInput_Create(
    TIM_HandleTypeDef *htim, float tim_frequency_hz, uint32_t tim_channel,
    uint32_t tim_auto_reload_reg, HAL_TIM_ActiveChannel tim_active_channel) {
  assert(htim != NULL);
...
void Io_SharedFreqOnlyPwmInput_Tick(struct FreqOnlyPwmInput *const pwm_input) {
  ...
  if (pwm_input->first_tick) {
    ...
    if (curr_rising_edge > prev_rising_edge) {
      ...
      Io_SetFrequency(pwm_input,
                      pwm_input->tim_frequency_hz / (float)rising_edge_delta);
    } else if (curr_rising_edge < prev_rising_edge) {
      // Occurs when the counter rolls over
      rising_edge_delta = pwm_input->tim_auto_reload_reg - prev_rising_edge +
                          curr_rising_edge + 1;
      Io_SetFrequency(pwm_input,
                      pwm_input->tim_frequency_hz / (float)rising_edge_delta);
    } else {
