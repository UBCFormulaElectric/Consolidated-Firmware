...
#define TRC_FREERTOS_VERSION_10_0_0 \
    8 /* If using FreeRTOS v10.0.0 or later version */

#define TRC_FREERTOS_VERSION_9_X 42 /* Not allowed anymore */

#if (TRC_CFG_FREERTOS_VERSION == TRC_FREERTOS_VERSION_9_X)
/* This setting for TRC_CFG_FREERTOS_VERSION is no longer allowed as v9.0.1
 * needs special handling. */
#error \
    "Please specify your exact FreeRTOS version in trcConfig.h, from the options listed above."
#endif
...
#define prvRemoveTaskFromStackMonitor(task)      /* Do nothing */
    ...
    /*******************************************************************************
     * Note: Setting names for event groups is difficult to support, this has
     *been excluded intentionally. This since we don't know if event_groups.c is
     * included in the build, so referencing it from the recorder may cause
     *errors.
     ******************************************************************************/
...
#if (                                                                     \
    (TRC_CFG_HARDWARE_PORT == TRC_HARDWARE_PORT_Renesas_RX600) ||         \
...
#error \
     ...
     *
     * xQueueGenericReceive had a parameter "xJustPeeking", used by the trace
     *hooks to tell between xQueuePeek events and others. This is no longer
     *present, so we need another way to correctly identify peek events. Since
     *all three functions call the same trace macros, the context of these macro
     *is unknown.
     *
     * We therefore check the __LINE__ macro inside of the trace macros. This
     *gives the line number of queue.c, where the macros are used. This can be
     *used to tell if the context is xQueuePeek or another function.
     * __LINE__ is a standard compiler feature since ancient times, so it should
     * work on all common compilers.
     *
     * This might seem as a quite brittle and unusual solution, but works in
     *this particular case and is only for FreeRTOS v9.0.1. Future versions of
     *FreeRTOS should not need this fix, as we have submitted a correction of
     *queue.c with individual trace macros for each function.
     ******************************************************************************/
...
#if (                                    \
    TRC_CFG_INCLUDE_TIMER_EVENTS == 1 && \
...
#if (                                          \
    TRC_CFG_INCLUDE_EVENT_GROUP_EVENTS == 1 && \
    TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0)
...
#if (                                            \
    TRC_CFG_INCLUDE_STREAM_BUFFER_EVENTS == 1 && \
    TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_10_0_0)
...
#define TRACE_GET_TASK_FILTER(pxTask) \
    prvTraceGetTaskNumberHigh16((void *)pxTask)
...
#define TRACE_OBJECT_TABLE_SIZE \
    ...
    /* Returns the "Not enough handles" error message for the specified object
     * class */
     ...
     * Translates a FreeRTOS QueueType into trace objects classes
     *(TRACE_CLASS_). Has one entry for each QueueType, gives TRACE_CLASS ID.
     ******************************************************************************/
...
#define USER_EVENT_LAST (EVENTGROUP_USEREVENT + 15UL) /*0xA7*/
     ...
     * The timestamps used in the recorder are "differential timestamps" (DTS),
     *i.e. the time since the last stored event. The DTS fields are either 1 or
     *2 bytes in the other events, depending on the bytes available in the event
     *struct. If the time since the last event (the DTS) is larger than allowed
     *for by the DTS field of the current event, an XTS event is inserted
     *immediately before the original event. The XTS event contains up to 3
     *additional bytes of the DTS value - the higher bytes of the true DTS
     *value. The lower 1-2 bytes are stored in the normal DTS field. There are
     *two types of XTS events, XTS8 and XTS16. An XTS8 event is stored when
     *there is only room for 1 byte (8 bit) DTS data in the original event,
...
#define TRACE_GET_TRACE_CLASS_FROM_STREAMBUFFER_CLASS(xIsMessageBuffer) \
    (xIsMessageBuffer == 1 ? TRACE_CLASS_MESSAGEBUFFER                  \
...
#define TRACE_SET_TIMER_NUMBER(tmr)             \
    prvTraceSetTimerNumberLow16(                \
        tmr, (uint16_t)prvTraceGetObjectHandle( \
...
#define TRACE_GET_EVENTGROUP_CLASS_EVENT_CODE(SERVICE, RESULT, kernelClass) \
    --THIS IS NOT USED--
#define TRACE_GET_STREAMBUFFER_CLASS_EVENT_CODE( \
    SERVICE, RESULT, isMessageBuffer)            \
    (uint8_t)(                                   \
        TRACE_STREAMBUFFER_##SERVICE##_##RESULT + (uint8_t)isMessageBuffer)
...
#define TRACE_GET_STREAMBUFFER_OBJECT_EVENT_CODE(SERVICE, RESULT, pxObject) \
    (uint8_t)(                                                              \
        TRACE_STREAMBUFFER_##SERVICE##_##RESULT +                           \
    ...
    {                                                    \
        trcKERNEL_HOOKS_TASK_CREATE(                     \
            TRACE_GET_OBJECT_EVENT_CODE(                 \
                CREATE_OBJ, TRCSUCCESS, TASK, pxNewTCB), \
            TASK, pxNewTCB);                             \
...
#define traceTASK_DELETE(pxTaskToDelete)                          \
    {                                                             \
        ...
        trcKERNEL_HOOKS_TASK_DELETE(                              \
            TRACE_GET_OBJECT_EVENT_CODE(                          \
                DELETE_OBJ, TRCSUCCESS, TASK, pxTaskToDelete),    \
            TRACE_GET_OBJECT_EVENT_CODE(                          \
                OBJCLOSE_NAME, TRCSUCCESS, TASK, pxTaskToDelete), \
            TRACE_GET_OBJECT_EVENT_CODE(                          \
                OBJCLOSE_PROP, TRCSUCCESS, TASK, pxTaskToDelete), \
            pxTaskToDelete);                                      \
...
#define traceQUEUE_CREATE(pxNewQueue)                   \
    trcKERNEL_HOOKS_OBJECT_CREATE(                      \
        TRACE_GET_OBJECT_EVENT_CODE(                    \
            CREATE_OBJ, TRCSUCCESS, QUEUE, pxNewQueue), \
        QUEUE, pxNewQueue);
...
#define traceQUEUE_CREATE_FAILED(queueType)                                  \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY(                  \
        TRACE_GET_CLASS_EVENT_CODE(CREATE_OBJ, TRCFAILED, QUEUE, queueType), \
        0);
...
#define traceQUEUE_DELETE(pxQueue)                          \
    {                                                       \
        ...
        trcKERNEL_HOOKS_OBJECT_DELETE(                      \
            TRACE_GET_OBJECT_EVENT_CODE(                    \
                DELETE_OBJ, TRCSUCCESS, QUEUE, pxQueue),    \
            TRACE_GET_OBJECT_EVENT_CODE(                    \
                OBJCLOSE_NAME, TRCSUCCESS, QUEUE, pxQueue), \
            TRACE_GET_OBJECT_EVENT_CODE(                    \
                OBJCLOSE_PROP, TRCSUCCESS, QUEUE, pxQueue), \
            QUEUE, pxQueue);                                \
        TRACE_EXIT_CRITICAL_SECTION();                      \
    }
...
#define traceCREATE_MUTEX_FAILED()                                \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY(       \
        TRACE_GET_CLASS_EVENT_CODE(                               \
            CREATE_OBJ, TRCFAILED, QUEUE, queueQUEUE_TYPE_MUTEX), \
        0);
#endif /* (TRC_CFG_FREERTOS_VERSION < TRC_FREERTOS_VERSION_9_0_0) */
...
#define traceQUEUE_RECEIVE(pxQueue)                                           \
    ...
    {                                                                         \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                       \
            TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCSUCCESS, QUEUE, pxQueue),    \
            QUEUE, pxQueue);                                                  \
    }                                                                         \
    else                                                                      \
    {                                                                         \
...
#define traceQUEUE_RECEIVE_FAILED(pxQueue)                                   \
    ...
    {                                                                        \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                      \
            TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCFAILED, QUEUE, pxQueue),    \
            QUEUE, pxQueue);                                                 \
    }                                                                        \
    else                                                                     \
    {                                                                        \
...
#define traceBLOCKING_ON_QUEUE_RECEIVE(pxQueue)                             \
    ...
    {                                                                       \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                     \
            TRACE_GET_OBJECT_EVENT_CODE(PEEK, TRCBLOCK, QUEUE, pxQueue),    \
            QUEUE, pxQueue);                                                \
    }                                                                       \
    else                                                                    \
    {                                                                       \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                     \
            TRACE_GET_OBJECT_EVENT_CODE(RECEIVE, TRCBLOCK, QUEUE, pxQueue), \
            QUEUE, pxQueue);                                                \
    }                                                                       \
...
#define xQueueGiveFromISR__pcHead(__a, __b)                        \
    ...
    {                                                              \
        return MyWrapper(                                          \
            xQueue, pxHigherPriorityTaskWoken, queueSEND_TO_BACK); \
    }                                                              \
...
#define traceQUEUE_SEND_FROM_ISR(pxQueue)                     \
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                  \
        xCopyPosition == queueSEND_TO_BACK                    \
            ? (TRACE_GET_OBJECT_EVENT_CODE(                   \
                  SEND_FROM_ISR, TRCSUCCESS, QUEUE, pxQueue)) \
            : TRACE_QUEUE_SEND_TO_FRONT_FROM_ISR_TRCSUCCESS,  \
        QUEUE, pxQueue);                                      \
    trcKERNEL_HOOKS_SET_OBJECT_STATE(                         \
        QUEUE, pxQueue, (uint8_t)(pxQueue->uxMessagesWaiting + 1));
...
#define traceQUEUE_SEND_FROM_ISR_FAILED(pxQueue)             \
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                 \
        xCopyPosition == queueSEND_TO_BACK                   \
            ? (TRACE_GET_OBJECT_EVENT_CODE(                  \
                  SEND_FROM_ISR, TRCFAILED, QUEUE, pxQueue)) \
...
#define traceQUEUE_RECEIVE_FROM_ISR(pxQueue)               \
    trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(               \
        TRACE_GET_OBJECT_EVENT_CODE(                       \
            RECEIVE_FROM_ISR, TRCSUCCESS, QUEUE, pxQueue), \
        QUEUE, pxQueue);                                   \
    trcKERNEL_HOOKS_SET_OBJECT_STATE(                      \
        QUEUE, pxQueue, (uint8_t)(pxQueue->uxMessagesWaiting - 1));
...
#define traceTASK_PRIORITY_SET(pxTask, uxNewPriority) \
    trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE(             \
        TASK_PRIORITY_SET, pxTask, uxNewPriority);
...
#define traceTASK_PRIORITY_INHERIT(pxTask, uxNewPriority) \
    trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE(                 \
        TASK_PRIORITY_INHERIT, pxTask, uxNewPriority);
...
#define traceTASK_PRIORITY_DISINHERIT(pxTask, uxNewPriority) \
    trcKERNEL_HOOKS_TASK_PRIORITY_CHANGE(                    \
        TASK_PRIORITY_DISINHERIT, pxTask, uxNewPriority);
...
#define traceTASK_RESUME_FROM_ISR(pxTaskToResume) \
    ...
    extern void
        vTraceStoreMemMangEvent(uint32_t ecode, uint32_t address, int32_t size);
...
#define traceMALLOC(pvAddress, uiSize)                                        \
    ...
    {                                                                         \
        vTraceStoreMemMangEvent(                                              \
            MEM_MALLOC_SIZE, (uint32_t)pvAddress, (int32_t)uiSize);           \
    }                                                                         \
    else                                                                      \
    {                                                                         \
        vTraceStoreMemMangEvent(                                              \
            MEM_MALLOC_SIZE_TRCFAILED, (uint32_t)pvAddress, (int32_t)uiSize); \
    }

#undef traceFREE
#define traceFREE(pvAddress, uiSize) \
    vTraceStoreMemMangEvent(         \
        MEM_FREE_SIZE, (uint32_t)pvAddress, -((int32_t)uiSize));
        ...
        {                                                                  \
            ...
            {                                                              \
                trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                 \
                    TIMER_CHANGE_PERIOD, TIMER, tmr, xOptionalValue);      \
            }                                                              \
            else                                                           \
            {                                                              \
                trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                 \
                    TIMER_CHANGE_PERIOD_TRCFAILED, TIMER, tmr,             \
                    xOptionalValue);                                       \
            }                                                              \
        }                                                                  \
...
#define tracePEND_FUNC_CALL(func, arg1, arg2, ret)                             \
    ...
    {                                                                          \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                        \
            PEND_FUNC_CALL, TASK, xTimerGetTimerDaemonTaskHandle());           \
    }                                                                          \
    else                                                                       \
    {                                                                          \
        trcKERNEL_HOOKS_KERNEL_SERVICE(                                        \
            PEND_FUNC_CALL_TRCFAILED, TASK, xTimerGetTimerDaemonTaskHandle()); \
    }
...
#define traceEVENT_GROUP_SYNC_BLOCK(eg, bitsToSet, bitsToWaitFor) \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                    \
        EVENT_GROUP_SYNC_TRCBLOCK, EVENTGROUP, eg, bitsToWaitFor);
...
#define traceEVENT_GROUP_SYNC_END(eg, bitsToSet, bitsToWaitFor, wasTimeout) \
    ...
    {                                                                       \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                          \
            EVENT_GROUP_SYNC_END_TRCFAILED, EVENTGROUP, eg, bitsToWaitFor); \
    }                                                                       \
    else                                                                    \
    {                                                                       \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                          \
            EVENT_GROUP_SYNC_END, EVENTGROUP, eg, bitsToWaitFor);           \
    }

#undef traceEVENT_GROUP_WAIT_BITS_BLOCK
#define traceEVENT_GROUP_WAIT_BITS_BLOCK(eg, bitsToWaitFor)             \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                          \
        EVENT_GROUP_WAIT_BITS_TRCBLOCK, EVENTGROUP, eg, bitsToWaitFor); \
    trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED();
...
#define traceEVENT_GROUP_WAIT_BITS_END(eg, bitsToWaitFor, wasTimeout)  \
    ...
    {                                                                  \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                     \
            EVENT_GROUP_WAIT_BITS_END_TRCFAILED, EVENTGROUP, eg,       \
            bitsToWaitFor);                                            \
    }                                                                  \
    else                                                               \
    {                                                                  \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                     \
            EVENT_GROUP_WAIT_BITS_END, EVENTGROUP, eg, bitsToWaitFor); \
    }
...
#define traceEVENT_GROUP_SET_BITS(eg, bitsToSet) \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(   \
        EVENT_GROUP_SET_BITS, EVENTGROUP, eg, bitsToSet);
...
#define traceTASK_NOTIFY_TAKE()                                        \
    ...
    {                                                                  \
        trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                     \
            TRACE_TASK_NOTIFY_TAKE_TRCFAILED, TASK, pxCurrentTCB,      \
            xTicksToWait);                                             \
    }
#else /* TRC_CFG_FREERTOS_VERSION < TRC_FREERTOS_VERSION_9_0_0 */
#define traceTASK_NOTIFY_TAKE()                                        \
...
#define traceTASK_NOTIFY()                                                    \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &             \
        ...
        if (TRACE_GET_OBJECT_FILTER(TASK, xTaskToNotify) & CurrentFilterMask) \
            prvTraceStoreKernelCall(                                          \
                TRACE_TASK_NOTIFY, TRACE_CLASS_TASK,                          \
...
#define traceTASK_NOTIFY_FROM_ISR()                                       \
    if (TRACE_GET_OBJECT_FILTER(TASK, xTaskToNotify) & CurrentFilterMask) \
        prvTraceStoreKernelCall(                                          \
            TRACE_TASK_NOTIFY_FROM_ISR, TRACE_CLASS_TASK,                 \
            TRACE_GET_TASK_NUMBER(xTaskToNotify));
...
#define traceTASK_NOTIFY_GIVE_FROM_ISR()                                  \
    if (TRACE_GET_OBJECT_FILTER(TASK, xTaskToNotify) & CurrentFilterMask) \
        prvTraceStoreKernelCall(                                          \
            TRACE_TASK_NOTIFY_GIVE_FROM_ISR, TRACE_CLASS_TASK,            \
            TRACE_GET_TASK_NUMBER(xTaskToNotify));
...
#define traceSTREAM_BUFFER_CREATE(pxStreamBuffer, xIsMessageBuffer) \
    trcKERNEL_HOOKS_OBJECT_CREATE(                                  \
        TRACE_GET_OBJECT_EVENT_CODE(                                \
            CREATE_OBJ, TRCSUCCESS, STREAMBUFFER, pxStreamBuffer),  \
...
#define traceSTREAM_BUFFER_CREATE_FAILED(xIsMessageBuffer)          \
    trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_NUMERIC_PARAM_ONLY(         \
        TRACE_GET_CLASS_EVENT_CODE(                                 \
            CREATE_OBJ, TRCFAILED, STREAMBUFFER, xIsMessageBuffer), \
        0);
...
#define traceSTREAM_BUFFER_DELETE(xStreamBuffer)                     \
    trcKERNEL_HOOKS_OBJECT_DELETE(                                   \
        TRACE_GET_OBJECT_EVENT_CODE(                                 \
            DELETE_OBJ, TRCSUCCESS, STREAMBUFFER, xStreamBuffer),    \
        TRACE_GET_OBJECT_EVENT_CODE(                                 \
            OBJCLOSE_NAME, TRCSUCCESS, STREAMBUFFER, xStreamBuffer), \
        TRACE_GET_OBJECT_EVENT_CODE(                                 \
            OBJCLOSE_PROP, TRCSUCCESS, STREAMBUFFER, xStreamBuffer), \
        STREAMBUFFER, xStreamBuffer);

#undef traceSTREAM_BUFFER_RESET
#define traceSTREAM_BUFFER_RESET(xStreamBuffer)                               \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                                           \
        prvGetStreamBufferType(xStreamBuffer) > 0 ? TRACE_MESSAGEBUFFER_RESET \
...
#define traceSTREAM_BUFFER_SEND(xStreamBuffer, xReturn)     \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                         \
        TRACE_GET_OBJECT_EVENT_CODE(                        \
            SEND, TRCSUCCESS, STREAMBUFFER, xStreamBuffer), \
        STREAMBUFFER, xStreamBuffer);                       \
    trcKERNEL_HOOKS_SET_OBJECT_STATE(                       \
        STREAMBUFFER, xStreamBuffer, prvBytesInBuffer(xStreamBuffer));

#undef traceBLOCKING_ON_STREAM_BUFFER_SEND
#define traceBLOCKING_ON_STREAM_BUFFER_SEND(xStreamBuffer) \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                        \
        TRACE_GET_OBJECT_EVENT_CODE(                       \
            SEND, TRCBLOCK, STREAMBUFFER, xStreamBuffer),  \
        STREAMBUFFER, xStreamBuffer);
...
#define traceSTREAM_BUFFER_SEND_FAILED(xStreamBuffer)      \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                        \
        TRACE_GET_OBJECT_EVENT_CODE(                       \
            SEND, TRCFAILED, STREAMBUFFER, xStreamBuffer), \
        STREAMBUFFER, xStreamBuffer);
...
#define traceSTREAM_BUFFER_RECEIVE(xStreamBuffer, xReceivedLength) \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                                \
        TRACE_GET_OBJECT_EVENT_CODE(                               \
            RECEIVE, TRCSUCCESS, STREAMBUFFER, xStreamBuffer),     \
        STREAMBUFFER, xStreamBuffer);                              \
    trcKERNEL_HOOKS_SET_OBJECT_STATE(                              \
        STREAMBUFFER, xStreamBuffer, prvBytesInBuffer(xStreamBuffer));
...
#define traceBLOCKING_ON_STREAM_BUFFER_RECEIVE(xStreamBuffer) \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                           \
        TRACE_GET_OBJECT_EVENT_CODE(                          \
            RECEIVE, TRCBLOCK, STREAMBUFFER, xStreamBuffer),  \
        STREAMBUFFER, xStreamBuffer);
...
#define traceSTREAM_BUFFER_RECEIVE_FAILED(xStreamBuffer)      \
    trcKERNEL_HOOKS_KERNEL_SERVICE(                           \
        TRACE_GET_OBJECT_EVENT_CODE(                          \
            RECEIVE, TRCFAILED, STREAMBUFFER, xStreamBuffer), \
        STREAMBUFFER, xStreamBuffer);
...
#define traceSTREAM_BUFFER_SEND_FROM_ISR(xStreamBuffer, xReturn)           \
    ...
    {                                                                      \
        trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                           \
            TRACE_GET_OBJECT_EVENT_CODE(                                   \
                SEND_FROM_ISR, TRCSUCCESS, STREAMBUFFER, xStreamBuffer),   \
            STREAMBUFFER, xStreamBuffer);                                  \
        trcKERNEL_HOOKS_SET_OBJECT_STATE(                                  \
            STREAMBUFFER, xStreamBuffer, prvBytesInBuffer(xStreamBuffer)); \
    }                                                                      \
    ...
    {                                                                      \
        trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                           \
            TRACE_GET_OBJECT_EVENT_CODE(                                   \
                SEND_FROM_ISR, TRCFAILED, STREAMBUFFER, xStreamBuffer),    \
            STREAMBUFFER, xStreamBuffer);                                  \
    }
...
#define traceSTREAM_BUFFER_RECEIVE_FROM_ISR(xStreamBuffer, xReceivedLength) \
    ...
    {                                                                       \
        trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                            \
            TRACE_GET_OBJECT_EVENT_CODE(                                    \
                RECEIVE_FROM_ISR, TRCSUCCESS, STREAMBUFFER, xStreamBuffer), \
            STREAMBUFFER, xStreamBuffer);                                   \
        trcKERNEL_HOOKS_SET_OBJECT_STATE(                                   \
            STREAMBUFFER, xStreamBuffer, prvBytesInBuffer(xStreamBuffer));  \
    }                                                                       \
    ...
    {                                                                       \
        trcKERNEL_HOOKS_KERNEL_SERVICE_FROM_ISR(                            \
            TRACE_GET_OBJECT_EVENT_CODE(                                    \
                RECEIVE_FROM_ISR, TRCFAILED, STREAMBUFFER, xStreamBuffer),  \
            STREAMBUFFER, xStreamBuffer);                                   \
    }
...
/******************************************************************************/
     ...
     * prvIsNewTCB
     *
     * Tells if this task is already executing, or if there has been a
     *task-switch. Assumed to be called within a trace hook in kernel context.
     *******************************************************************************/
...
#define traceTASK_SWITCHED_IN()                                   \
    ...
    {                                                             \
        ...
        {                                                         \
            prvTraceStoreEvent2(                                  \
                PSF_EVENT_TASK_ACTIVATE, (uint32_t)pxCurrentTCB,  \
...
#define traceTASK_CREATE(pxNewTCB)                                           \
    ...
    {                                                                        \
        ...
        prvTraceStoreStringEvent(                                            \
            1, PSF_EVENT_OBJ_NAME, (const char *)pcName, pxNewTCB);          \
        TRACE_SET_OBJECT_FILTER(TASK, pxNewTCB, CurrentFilterGroup);         \
        if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
            CurrentFilterMask)                                               \
            if (TRACE_GET_OBJECT_FILTER(TASK, pxNewTCB) & CurrentFilterMask) \
                prvTraceStoreEvent2(                                         \
                    PSF_EVENT_TASK_CREATE, (uint32_t)pxNewTCB, uxPriority);  \
    }
#endif /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */
...
#define traceTASK_SUSPEND(pxTaskToSuspend)                        \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
        CurrentFilterMask)                                        \
        if (TRACE_GET_OBJECT_FILTER(TASK, pxTaskToSuspend) &      \
            CurrentFilterMask)                                    \
            prvTraceStoreEvent1(                                  \
                PSF_EVENT_TASK_SUSPEND, (uint32_t)pxTaskToSuspend);
...
#define traceQUEUE_CREATE_HELPER()                                         \
    ...
    case queueQUEUE_TYPE_RECURSIVE_MUTEX:                                  \
        prvTraceStoreEvent1(                                               \
            PSF_EVENT_MUTEX_RECURSIVE_CREATE, (uint32_t)pxNewQueue);       \
        break;
#else
#define traceQUEUE_CREATE_HELPER()
#endif /* (TRC_CFG_FREERTOS_VERSION < TRC_FREERTOS_VERSION_9_0_0) */
        ...
        {                                                                   \
            ...
            {                                                               \
                case queueQUEUE_TYPE_BASE:                                  \
                    prvTraceStoreEvent2(                                    \
                        PSF_EVENT_QUEUE_CREATE, (uint32_t)pxNewQueue,       \
                        uxQueueLength);                                     \
                    break;                                                  \
                case queueQUEUE_TYPE_BINARY_SEMAPHORE:                      \
                    prvTraceStoreEvent1(                                    \
                        PSF_EVENT_SEMAPHORE_BINARY_CREATE,                  \
                        (uint32_t)pxNewQueue);                              \
                    break;                                                  \
...
#define traceQUEUE_CREATE_FAILED(queueType)                           \
    ...
    {                                                                 \
        ...
        {                                                             \
            case queueQUEUE_TYPE_BASE:                                \
                prvTraceStoreEvent2(                                  \
                    PSF_EVENT_QUEUE_CREATE_FAILED, 0, uxQueueLength); \
                break;                                                \
            case queueQUEUE_TYPE_BINARY_SEMAPHORE:                    \
                prvTraceStoreEvent1(                                  \
                    PSF_EVENT_SEMAPHORE_BINARY_CREATE_FAILED, 0);     \
                break;                                                \
...
#define traceCREATE_COUNTING_SEMAPHORE()                                 \
    ...
    prvTraceStoreEvent2(                                                 \
        PSF_EVENT_SEMAPHORE_COUNTING_CREATE, (uint32_t)xHandle, uxMaxCount)
...
#define traceCREATE_COUNTING_SEMAPHORE()                                 \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, xHandle) & CurrentFilterMask) \
            prvTraceStoreEvent2(                                         \
                PSF_EVENT_SEMAPHORE_COUNTING_CREATE, (uint32_t)xHandle,  \
                uxInitialCount);
...
#define traceCREATE_COUNTING_SEMAPHORE()                                  \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &         \
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxHandle) & CurrentFilterMask) \
            prvTraceStoreEvent2(                                          \
                PSF_EVENT_SEMAPHORE_COUNTING_CREATE, (uint32_t)pxHandle,  \
                uxCountValue);
#endif /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_8_X */
...
#define traceCREATE_COUNTING_SEMAPHORE_FAILED()                   \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
        CurrentFilterMask)                                        \
        prvTraceStoreEvent2(                                      \
            PSF_EVENT_SEMAPHORE_COUNTING_CREATE_FAILED, 0, uxMaxCount);
#elif (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_7_5_OR_7_6)
#define traceCREATE_COUNTING_SEMAPHORE_FAILED()                   \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
        CurrentFilterMask)                                        \
        prvTraceStoreEvent2(                                      \
            PSF_EVENT_SEMAPHORE_COUNTING_CREATE_FAILED, 0, uxInitialCount);
#elif (TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_7_4)
#define traceCREATE_COUNTING_SEMAPHORE_FAILED()                   \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
        CurrentFilterMask)                                        \
        prvTraceStoreEvent2(                                      \
            PSF_EVENT_SEMAPHORE_COUNTING_CREATE_FAILED, 0, uxCountValue);
#else
#define traceCREATE_COUNTING_SEMAPHORE_FAILED()                   \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) & \
        CurrentFilterMask)                                        \
    ...
    {                                                                       \
        ...
        {                                                                   \
            ...
            {                                                               \
                case queueQUEUE_TYPE_MUTEX:                                 \
                    prvTraceStoreEvent1(                                    \
                        PSF_EVENT_MUTEX_CREATE, (uint32_t)pxNewQueue);      \
                    break;                                                  \
                case queueQUEUE_TYPE_RECURSIVE_MUTEX:                       \
                    prvTraceStoreEvent1(                                    \
                        PSF_EVENT_MUTEX_RECURSIVE_CREATE,                   \
                        (uint32_t)pxNewQueue);                              \
                    break;                                                  \
            }                                                               \
        }                                                                   \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &           \
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)    \
            ...
            {                                                               \
                ...
                case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                    \
                    prvTraceStoreEvent2(                                    \
                        PSF_EVENT_SEMAPHORE_GIVE, (uint32_t)pxQueue,        \
                        pxQueue->uxMessagesWaiting + 1);                    \
                    break;                                                  \
                case queueQUEUE_TYPE_MUTEX:                                 \
                case queueQUEUE_TYPE_RECURSIVE_MUTEX:                       \
                    prvTraceStoreEvent1(                                    \
                        PSF_EVENT_MUTEX_GIVE, (uint32_t)pxQueue);           \
                    break;                                                  \
            }
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &           \
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)    \
            ...
            {                                                               \
                ...
                case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                    \
                    prvTraceStoreEvent2(                                    \
                        PSF_EVENT_SEMAPHORE_GIVE_FAILED, (uint32_t)pxQueue, \
                        pxQueue->uxMessagesWaiting);                        \
                    break;                                                  \
                case queueQUEUE_TYPE_MUTEX:                                 \
                case queueQUEUE_TYPE_RECURSIVE_MUTEX:                       \
                    prvTraceStoreEvent1(                                    \
                        PSF_EVENT_MUTEX_GIVE_FAILED, (uint32_t)pxQueue);    \
                    break;                                                  \
            }
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &          \
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)   \
            ...
            {                                                              \
                ...
                case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                   \
                    prvTraceStoreEvent2(                                   \
                        PSF_EVENT_SEMAPHORE_GIVE_BLOCK, (uint32_t)pxQueue, \
                        pxQueue->uxMessagesWaiting);                       \
                    break;                                                 \
                case queueQUEUE_TYPE_MUTEX:                                \
                case queueQUEUE_TYPE_RECURSIVE_MUTEX:                      \
                    prvTraceStoreEvent1(                                   \
                        PSF_EVENT_MUTEX_GIVE_BLOCK, (uint32_t)pxQueue);    \
                    break;                                                 \
            }
...
#define traceQUEUE_SEND_FROM_ISR(pxQueue)                                \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)     \
        ...
        {                                                                \
            ...
            case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                     \
                prvTraceStoreEvent2(                                     \
                    PSF_EVENT_SEMAPHORE_GIVE_FROMISR, (uint32_t)pxQueue, \
                    pxQueue->uxMessagesWaiting + 1);                     \
                break;                                                   \
        }
...
#define traceQUEUE_SEND_FROM_ISR_FAILED(pxQueue)                     \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) \
        ...
        {                                                            \
            ...
            case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                 \
                prvTraceStoreEvent2(                                 \
                    PSF_EVENT_SEMAPHORE_GIVE_FROMISR_FAILED,         \
                    (uint32_t)pxQueue, pxQueue->uxMessagesWaiting);  \
                break;                                               \
        }
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)   \
            ...
            {                                                              \
                case queueQUEUE_TYPE_BASE:                                 \
                    if (isQueueReceiveHookActuallyPeek)                    \
                        prvTraceStoreEvent3(                               \
                            PSF_EVENT_QUEUE_PEEK, (uint32_t)pxQueue,       \
                            xTicksToWait, pxQueue->uxMessagesWaiting - 1); \
                    else                                                   \
                        prvTraceStoreEvent3(                               \
                            PSF_EVENT_QUEUE_RECEIVE, (uint32_t)pxQueue,    \
                            xTicksToWait, pxQueue->uxMessagesWaiting - 1); \
                    break;                                                 \
                case queueQUEUE_TYPE_BINARY_SEMAPHORE:                     \
                case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                   \
                    if (isQueueReceiveHookActuallyPeek)                    \
                        prvTraceStoreEvent3(                               \
                            PSF_EVENT_SEMAPHORE_PEEK, (uint32_t)pxQueue,   \
                            xTicksToWait, pxQueue->uxMessagesWaiting - 1); \
                    else                                                   \
                        prvTraceStoreEvent3(                               \
                            PSF_EVENT_SEMAPHORE_TAKE, (uint32_t)pxQueue,   \
                            xTicksToWait, pxQueue->uxMessagesWaiting - 1); \
                    break;                                                 \
                case queueQUEUE_TYPE_MUTEX:                                \
                case queueQUEUE_TYPE_RECURSIVE_MUTEX:                      \
                    if (isQueueReceiveHookActuallyPeek)                    \
                        prvTraceStoreEvent2(                               \
                            PSF_EVENT_MUTEX_PEEK, (uint32_t)pxQueue,       \
                            xTicksToWait);                                 \
                    else                                                   \
                        prvTraceStoreEvent2(                               \
                            PSF_EVENT_MUTEX_TAKE, (uint32_t)pxQueue,       \
                            xTicksToWait);                                 \
                    break;                                                 \
            }
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask) \
            ...
            {                                                            \
                case queueQUEUE_TYPE_BASE:                               \
                    prvTraceStoreEvent3(                                 \
                        isQueueReceiveHookActuallyPeek                   \
                            ? PSF_EVENT_QUEUE_PEEK_FAILED                \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &           \
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)    \
            ...
            {                                                               \
                case queueQUEUE_TYPE_BASE:                                  \
                    prvTraceStoreEvent3(                                    \
                        PSF_EVENT_QUEUE_PEEK_FAILED, (uint32_t)pxQueue,     \
                        xTicksToWait, pxQueue->uxMessagesWaiting);          \
                    break;                                                  \
                case queueQUEUE_TYPE_BINARY_SEMAPHORE:                      \
                case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                    \
                    prvTraceStoreEvent3(                                    \
                        PSF_EVENT_SEMAPHORE_PEEK_FAILED, (uint32_t)pxQueue, \
                        xTicksToWait, pxQueue->uxMessagesWaiting);          \
                    break;                                                  \
                case queueQUEUE_TYPE_MUTEX:                                 \
                case queueQUEUE_TYPE_RECURSIVE_MUTEX:                       \
                    prvTraceStoreEvent2(                                    \
                        PSF_EVENT_MUTEX_PEEK_FAILED, (uint32_t)pxQueue,     \
                        xTicksToWait);                                      \
                    break;                                                  \
            }
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &          \
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)   \
            ...
            {                                                              \
                case queueQUEUE_TYPE_BASE:                                 \
                    prvTraceStoreEvent3(                                   \
                        PSF_EVENT_QUEUE_PEEK_BLOCK, (uint32_t)pxQueue,     \
                        xTicksToWait, pxQueue->uxMessagesWaiting);         \
                    break;                                                 \
                case queueQUEUE_TYPE_BINARY_SEMAPHORE:                     \
                case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                   \
                    prvTraceStoreEvent3(                                   \
                        PSF_EVENT_SEMAPHORE_PEEK_BLOCK, (uint32_t)pxQueue, \
                        xTicksToWait, pxQueue->uxMessagesWaiting);         \
                    break;                                                 \
                case queueQUEUE_TYPE_MUTEX:                                \
                case queueQUEUE_TYPE_RECURSIVE_MUTEX:                      \
                    prvTraceStoreEvent2(                                   \
                        PSF_EVENT_MUTEX_PEEK_BLOCK, (uint32_t)pxQueue,     \
                        xTicksToWait);                                     \
                    break;                                                 \
            }
...
#define traceQUEUE_RECEIVE_FROM_ISR(pxQueue)                             \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)     \
        ...
        {                                                                \
            case queueQUEUE_TYPE_BASE:                                   \
                prvTraceStoreEvent2(                                     \
                    PSF_EVENT_QUEUE_RECEIVE_FROMISR, (uint32_t)pxQueue,  \
                    pxQueue->uxMessagesWaiting - 1);                     \
                break;                                                   \
            case queueQUEUE_TYPE_BINARY_SEMAPHORE:                       \
            case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                     \
                prvTraceStoreEvent2(                                     \
                    PSF_EVENT_SEMAPHORE_TAKE_FROMISR, (uint32_t)pxQueue, \
                    pxQueue->uxMessagesWaiting - 1);                     \
                break;                                                   \
        }
...
#define traceQUEUE_RECEIVE_FROM_ISR_FAILED(pxQueue)                            \
    if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)           \
        ...
        {                                                                      \
            case queueQUEUE_TYPE_BASE:                                         \
                prvTraceStoreEvent2(                                           \
                    PSF_EVENT_QUEUE_RECEIVE_FROMISR_FAILED, (uint32_t)pxQueue, \
                    pxQueue->uxMessagesWaiting);                               \
                break;                                                         \
            case queueQUEUE_TYPE_BINARY_SEMAPHORE:                             \
            case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                           \
                prvTraceStoreEvent2(                                           \
                    PSF_EVENT_SEMAPHORE_TAKE_FROMISR_FAILED,                   \
                    (uint32_t)pxQueue, pxQueue->uxMessagesWaiting);            \
                break;                                                         \
        }
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &              \
        ...
        if (TRACE_GET_OBJECT_FILTER(QUEUE, pxQueue) & CurrentFilterMask)       \
            ...
            {                                                                  \
                case queueQUEUE_TYPE_BASE:                                     \
                    prvTraceStoreEvent3(                                       \
                        PSF_EVENT_QUEUE_PEEK, (uint32_t)pxQueue, xTicksToWait, \
                        pxQueue->uxMessagesWaiting);                           \
                    break;                                                     \
                case queueQUEUE_TYPE_BINARY_SEMAPHORE:                         \
                case queueQUEUE_TYPE_COUNTING_SEMAPHORE:                       \
...
#define traceTASK_PRIORITY_SET(pxTask, uxNewPriority)                  \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &      \
        ...
        if (TRACE_GET_OBJECT_FILTER(TASK, pxTask) & CurrentFilterMask) \
            prvTraceStoreEvent2(                                       \
                PSF_EVENT_TASK_PRIORITY, (uint32_t)pxTask, uxNewPriority);
...
#define traceTASK_PRIORITY_INHERIT(pxTask, uxNewPriority)              \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &      \
        ...
        if (TRACE_GET_OBJECT_FILTER(TASK, pxTask) & CurrentFilterMask) \
            prvTraceStoreEvent2(                                       \
                PSF_EVENT_TASK_PRIO_INHERIT, (uint32_t)pxTask, uxNewPriority);
...
#define traceTASK_PRIORITY_DISINHERIT(pxTask, uxNewPriority)           \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &      \
        ...
        if (TRACE_GET_OBJECT_FILTER(TASK, pxTask) & CurrentFilterMask) \
            prvTraceStoreEvent2(                                       \
                PSF_EVENT_TASK_PRIO_DISINHERIT, (uint32_t)pxTask,      \
                uxNewPriority);
...
#define traceTASK_RESUME(pxTaskToResume)                                       \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &              \
        ...
        if (TRACE_GET_OBJECT_FILTER(TASK, pxTaskToResume) & CurrentFilterMask) \
            prvTraceStoreEvent1(                                               \
                PSF_EVENT_TASK_RESUME, (uint32_t)pxTaskToResume);
...
#define traceTASK_RESUME_FROM_ISR(pxTaskToResume)                          \
    if (TRACE_GET_OBJECT_FILTER(TASK, pxTaskToResume) & CurrentFilterMask) \
        prvTraceStoreEvent1(                                               \
            PSF_EVENT_TASK_RESUME_FROMISR, (uint32_t)pxTaskToResume);
...
#define traceMALLOC(pvAddress, uiSize)                                 \
    ...
    {                                                                  \
        ...
        {                                                              \
            prvTraceStoreEvent2(                                       \
                PSF_EVENT_MALLOC, (uint32_t)pvAddress, uiSize);        \
        }                                                              \
        else                                                           \
        {                                                              \
            prvTraceStoreEvent2(                                       \
                PSF_EVENT_MALLOC_FAILED, (uint32_t)pvAddress, uiSize); \
        }                                                              \
    }
...
#define traceFREE(pvAddress, uiSize)                                         \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &            \
        ...
        prvTraceStoreEvent2(                                                 \
            ...
            (uint32_t)(0 - uiSize)); /* "0 -" instead of just "-" to get rid \
                                        of a warning... */
...
#define traceTIMER_CREATE(tmr)                                       \
    ...
    prvTraceStoreStringEvent(                                        \
        1, PSF_EVENT_OBJ_NAME, (const char *)tmr->pcTimerName, tmr); \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &    \
        CurrentFilterMask)                                           \
        if (TRACE_GET_OBJECT_FILTER(TIMER, tmr) & CurrentFilterMask) \
            prvTraceStoreEvent2(                                     \
                PSF_EVENT_TIMER_CREATE, (uint32_t)tmr,               \
...
#define traceTIMER_COMMAND_SEND_8_0_CASES(tmr)                                 \
    case tmrCOMMAND_RESET:                                                     \
        prvTraceStoreEvent2(                                                   \
            (xReturn == pdPASS) ? PSF_EVENT_TIMER_RESET                        \
                                : PSF_EVENT_TIMER_RESET_FAILED,                \
            (uint32_t)tmr, xOptionalValue);                                    \
        break;                                                                 \
    case tmrCOMMAND_START_FROM_ISR:                                            \
        prvTraceStoreEvent2(                                                   \
            (xReturn == pdPASS) ? PSF_EVENT_TIMER_START_FROMISR                \
                                : PSF_EVENT_TIMER_START_FROMISR_FAILED,        \
            (uint32_t)tmr, xOptionalValue);                                    \
        break;                                                                 \
    case tmrCOMMAND_RESET_FROM_ISR:                                            \
        prvTraceStoreEvent2(                                                   \
            (xReturn == pdPASS) ? PSF_EVENT_TIMER_RESET_FROMISR                \
    ...
    case tmrCOMMAND_CHANGE_PERIOD_FROM_ISR:                                    \
        prvTraceStoreEvent2(                                                   \
            (xReturn == pdPASS) ? PSF_EVENT_TIMER_CHANGEPERIOD_FROMISR         \
        ...
        if (TRACE_GET_OBJECT_FILTER(TIMER, tmr) & CurrentFilterMask)         \
            ...
            {                                                                \
                case tmrCOMMAND_START:                                       \
                    prvTraceStoreEvent1(                                     \
                        (xReturn == pdPASS) ? PSF_EVENT_TIMER_START          \
                                            : PSF_EVENT_TIMER_START_FAILED,  \
                        (uint32_t)tmr);                                      \
                    break;                                                   \
                case tmrCOMMAND_STOP:                                        \
                    prvTraceStoreEvent1(                                     \
                        (xReturn == pdPASS) ? PSF_EVENT_TIMER_STOP           \
                ...
                case tmrCOMMAND_DELETE:                                      \
                    prvTraceStoreEvent1(                                     \
                        (xReturn == pdPASS) ? PSF_EVENT_TIMER_DELETE         \
...
#define traceTIMER_EXPIRED(tmr)                                             \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &           \
        ...
        if (TRACE_GET_OBJECT_FILTER(TIMER, tmr) & CurrentFilterMask)        \
            prvTraceStoreEvent2(                                            \
                PSF_EVENT_TIMER_EXPIRED, (uint32_t)tmr->pxCallbackFunction, \
...
#define tracePEND_FUNC_CALL(func, arg1, arg2, ret)             \
    prvTraceStoreEvent1(                                       \
        (ret == pdPASS) ? PSF_EVENT_TIMER_PENDFUNCCALL         \
...
#define tracePEND_FUNC_CALL_FROM_ISR(func, arg1, arg2, ret)            \
    prvTraceStoreEvent1(                                               \
        (ret == pdPASS) ? PSF_EVENT_TIMER_PENDFUNCCALL_FROMISR         \
...
#define traceEVENT_GROUP_SYNC_BLOCK(eg, bitsToSet, bitsToWaitFor)        \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
        ...
        if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask) \
            prvTraceStoreEvent2(                                         \
                PSF_EVENT_EVENTGROUP_SYNC_BLOCK, (uint32_t)eg, bitsToWaitFor);
...
#define traceEVENT_GROUP_WAIT_BITS_END(eg, bitsToWaitFor, wasTimeout)          \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &              \
        ...
        if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask)       \
            prvTraceStoreEvent2(                                               \
                (wasTimeout != pdTRUE) ? PSF_EVENT_EVENTGROUP_WAITBITS         \
...
#define traceEVENT_GROUP_CLEAR_BITS(eg, bitsToClear)                     \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
        ...
        if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask) \
            prvTraceStoreEvent2(                                         \
                PSF_EVENT_EVENTGROUP_CLEARBITS, (uint32_t)eg, bitsToClear);

#undef traceEVENT_GROUP_CLEAR_BITS_FROM_ISR
#define traceEVENT_GROUP_CLEAR_BITS_FROM_ISR(eg, bitsToClear)        \
    if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask) \
        prvTraceStoreEvent2(                                         \
            PSF_EVENT_EVENTGROUP_CLEARBITS_FROMISR, (uint32_t)eg,    \
            bitsToClear);
...
#define traceEVENT_GROUP_SET_BITS(eg, bitsToSet)                         \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &        \
        ...
        if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask) \
            prvTraceStoreEvent2(                                         \
                PSF_EVENT_EVENTGROUP_SETBITS, (uint32_t)eg, bitsToSet);

#undef traceEVENT_GROUP_SET_BITS_FROM_ISR
#define traceEVENT_GROUP_SET_BITS_FROM_ISR(eg, bitsToSet)            \
    if (TRACE_GET_OBJECT_FILTER(EVENTGROUP, eg) & CurrentFilterMask) \
        prvTraceStoreEvent2(                                         \
            PSF_EVENT_EVENTGROUP_SETBITS_FROMISR, (uint32_t)eg, bitsToSet);
...
#define traceTASK_NOTIFY_TAKE()                                            \
    ...
    {                                                                      \
        if (pxCurrentTCB->ucNotifyState == taskNOTIFICATION_RECEIVED)      \
            prvTraceStoreEvent2(                                           \
                PSF_EVENT_TASK_NOTIFY_TAKE, (uint32_t)pxCurrentTCB,        \
                xTicksToWait);                                             \
        else                                                               \
            prvTraceStoreEvent2(                                           \
                PSF_EVENT_TASK_NOTIFY_TAKE_FAILED, (uint32_t)pxCurrentTCB, \
                xTicksToWait);                                             \
    }
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */
#define traceTASK_NOTIFY_TAKE()                                            \
...
#define traceTASK_NOTIFY_TAKE_BLOCK()                                 \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &     \
        CurrentFilterMask)                                            \
        prvTraceStoreEvent2(                                          \
            PSF_EVENT_TASK_NOTIFY_TAKE_BLOCK, (uint32_t)pxCurrentTCB, \
            xTicksToWait);
...
#define traceTASK_NOTIFY_WAIT()                                            \
    ...
    {                                                                      \
        if (pxCurrentTCB->ucNotifyState == taskNOTIFICATION_RECEIVED)      \
            prvTraceStoreEvent2(                                           \
                PSF_EVENT_TASK_NOTIFY_WAIT, (uint32_t)pxCurrentTCB,        \
                xTicksToWait);                                             \
        else                                                               \
            prvTraceStoreEvent2(                                           \
                PSF_EVENT_TASK_NOTIFY_WAIT_FAILED, (uint32_t)pxCurrentTCB, \
                xTicksToWait);                                             \
    }
#else /* TRC_CFG_FREERTOS_VERSION >= TRC_FREERTOS_VERSION_9_0_0 */
#define traceTASK_NOTIFY_WAIT()                                            \
...
#define traceTASK_NOTIFY_WAIT_BLOCK()                                 \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &     \
        CurrentFilterMask)                                            \
        prvTraceStoreEvent2(                                          \
            PSF_EVENT_TASK_NOTIFY_WAIT_BLOCK, (uint32_t)pxCurrentTCB, \
            xTicksToWait);
...
#define traceTASK_NOTIFY_FROM_ISR()                                       \
    if (TRACE_GET_OBJECT_FILTER(TASK, xTaskToNotify) & CurrentFilterMask) \
        prvTraceStoreEvent1(                                              \
            PSF_EVENT_TASK_NOTIFY_FROM_ISR, (uint32_t)xTaskToNotify);

#undef traceTASK_NOTIFY_GIVE_FROM_ISR
#define traceTASK_NOTIFY_GIVE_FROM_ISR()                                  \
    if (TRACE_GET_OBJECT_FILTER(TASK, xTaskToNotify) & CurrentFilterMask) \
        prvTraceStoreEvent1(                                              \
            PSF_EVENT_TASK_NOTIFY_GIVE_FROM_ISR, (uint32_t)xTaskToNotify);
...
#define traceSTREAM_BUFFER_CREATE(pxStreamBuffer, xIsMessageBuffer)            \
    ...
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &              \
        ...
        if (TRACE_GET_OBJECT_FILTER(STREAMBUFFER, pxStreamBuffer) &            \
            CurrentFilterMask)                                                 \
            prvTraceStoreEvent2(                                               \
                xIsMessageBuffer == 1 ? PSF_EVENT_MESSAGEBUFFER_CREATE         \
