...
#define TRACE_SET_HIGH16(current, value) \
     ...
     *
     * Initializes and optionally starts the trace, depending on the start
     *option. To use the trace recorder, the startup must call vTraceEnable
     *before any RTOS calls are made (including "create" calls). Three start
     *options are provided:
     *
     * TRC_START: Starts the tracing directly. In snapshot mode this allows for
     * starting the trace at any point in your code, assuming
     *vTraceEnable(TRC_INIT) has been called in the startup. Can also be used
     *for streaming without Tracealyzer control, e.g. to a local flash file
     *system (assuming such a "stream port", see trcStreamingPort.h).
     *
     * TRC_START_AWAIT_HOST: For streaming mode only. Initializes the trace
     *recorder if necessary and waits for a Start command from Tracealyzer
     *("Start Recording" button). This call is intentionally blocking! By
     *calling vTraceEnable with this option from the startup code, you start
     *tracing at this point and capture the early events.
     *
     * TRC_INIT: Initializes the trace recorder, but does not start the tracing.
     * In snapshot mode, this must be followed by a vTraceEnable(TRC_START)
     *sometime later.
     *
     * Usage examples:
     *
...
#if (TRC_CFG_SCHEDULING_ONLY == 0) && (TRC_CFG_INCLUDE_USER_EVENTS == 1)
    void vTraceVPrintF(
        traceString eventLabel,
        const char *formatStr,
        va_list     vl);
#else
#define vTraceVPrintF(chn, formatStr, vl)                               \
    (void)(chn), (void)(formatStr),                                     \
        (void)(vl) /* Comma operator is used to avoid "unused variable" \
                      compiler warnings in a single statement */
#endif
...
#define xTraceRegisterString(x)                                              \
     ...
     *
     * Kernel-specific functions for setting names of kernel objects, for
     *display in Tracealyzer.
     ******************************************************************************/
     ...
     *
     * Stores a name and priority level for an Interrupt Service Routine, to
     *allow for better visualization. Returns a traceHandle used by
     *vTraceStoreISRBegin.
     *
     * Example:
     ...
     *traceHandle provided by xTraceSetISRProperties.
     *
     * Example:
     *	 #define PRIO_ISR_TIMER1 3 // the hardware priority of the interrupt
     *	 ...
     ...
     *
     * Creates an event that ends the current task instance at this very
     *instant. This makes the viewer to splits the current fragment at this
     *point and begin a new actor instance, even if no task-switch has occurred.
     *****************************************************************************/
     ...
     *
     * If that kernel call is blocking, the instance ends after the blocking
     *event and the corresponding return event is then the start of the next
     *instance. If the kernel call is not blocking, the viewer instead splits
     *the current fragment right before the kernel call, which makes this call
     *the first event of the next instance.
     *****************************************************************************/
     ...
     * This is normally not required, but if the default value
     *(TRC_HWTC_FREQ_HZ) should be incorrect for your setup, you can override it
     *using this function.
     *
     * Must be called prior to vTraceEnable, and the time source is assumed to
     * have a fixed clock frequency after the startup.
     *
     * Note that, in snapshot mode, the value is divided by the
     *TRC_HWTC_DIVISOR. This is a software "prescaler" that is also applied on
     *the timestamps.
     *****************************************************************************/
 ...
 ******************************************************************************/
#if (                                     \
    TRC_CFG_RECORDER_BUFFER_ALLOCATION == \
...
#define TRC_ALLOC_CUSTOM_BUFFER(bufname)                                       \
    char                                                                       \
        bufname[TRC_CFG_RTT_BUFFER_SIZE_UP]; /* Not static in this case, since \
...
#define TRC_ALLOC_CUSTOM_BUFFER(                                               \
     ...
     *
     * Returns true (1) if the recorder is enabled (i.e. is recording),
     *otherwise 0.
     ******************************************************************************/
     ...
     *
     * Sets the "filter group" to assign when creating RTOS objects, such as
     *tasks, queues, semaphores and mutexes. This together with
     *vTraceSetFilterMask allows you to control what events that are recorded,
     *based on the objects they refer to.
     *
     * There are 16 filter groups named FilterGroup0 .. FilterGroup15.
     *
     * Note: We don't recommend filtering out the Idle task, so make sure to
     *call vTraceSetFilterGroup just before initializing the RTOS, in order to
     *assign such "default" objects to the right Filter Group (typically group
     *0).
     *
     * Example:
     *
     ...
     *		<Create Queue Q3>
     *
     *		// Only include FilterGroup0 and FilterGroup2, exclude FilterGroup1
     *(Q1 and Q2) from the trace vTraceSetFilterMask( FilterGroup0 |
     *FilterGroup2 );
     *
     ...
     *
     * Note that filtering per event type (regardless of object) is also
     *available in trcConfig.h.
     ******************************************************************************/
     ...
     * vTraceSetFilterMask
     *
     * Sets the "filter mask" that is used to filter the events by object. This
     *can be used to reduce the trace data rate, i.e., if your streaming
     *interface is a bottleneck or if you want longer snapshot traces without
     *increasing the buffer size.
     *
     * Note: There are two kinds of filters in the recorder. The other filter
     *type excludes all events of certain kinds (e.g., OS ticks). See
     *trcConfig.h.
     *
     ...
     *
     * Declares a data type for a call back function that will be invoked
     *whenever the recorder is stopped.
     *
     * Snapshot mode only!
     ******************************************************************************/
     ...
     *
     * Starts the recorder. The recorder will not be started if an error has
     *been indicated using prvTraceError, e.g. if any of the Nx constants in
...
#define trcKERNEL_HOOKS_TASK_CREATE(SERVICE, CLASS, pxTCB)            \
    ...
    prvTraceSetObjectName(                                            \
        TRACE_CLASS_TASK, TRACE_GET_TASK_NUMBER(pxTCB),               \
...
#define trcKERNEL_HOOKS_TASK_DELETE(                                      \
    SERVICE, SERVICE_NAME, SERVICE_PROP, pxTCB)                           \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &         \
        CurrentFilterMask)                                                \
        if (TRACE_GET_OBJECT_FILTER(TASK, pxTCB) & CurrentFilterMask)     \
            prvTraceStoreKernelCall(                                      \
...
#define trcKERNEL_HOOKS_OBJECT_DELETE(                                    \
    SERVICE, SERVICE_NAME, SERVICE_PROP, CLASS, pxObject)                 \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &         \
        CurrentFilterMask)                                                \
        if (TRACE_GET_OBJECT_FILTER(CLASS, pxObject) & CurrentFilterMask) \
            prvTraceStoreKernelCall(                                      \
...
#define trcKERNEL_HOOKS_KERNEL_SERVICE_WITH_PARAM(                        \
    SERVICE, CLASS, pxObject, param)                                      \
    if (TRACE_GET_OBJECT_FILTER(TASK, TRACE_GET_CURRENT_TASK()) &         \
        CurrentFilterMask)                                                \
        if (TRACE_GET_OBJECT_FILTER(CLASS, pxObject) & CurrentFilterMask) \
...
#define trcKERNEL_HOOKS_SET_OBJECT_STATE(CLASS, pxObject, STATE) \
    prvTraceSetObjectState(                                      \
        TRACE_GET_OBJECT_TRACE_CLASS(CLASS, pxObject),           \
        TRACE_GET_OBJECT_NUMBER(CLASS, pxObject), (uint8_t)STATE);
...
#define trcKERNEL_HOOKS_TASK_DELAY(SERVICE, pxTCB, xValue)            \
    ...
    {                                                                 \
        prvTraceStoreKernelCallWithNumericParamOnly(SERVICE, xValue); \
        prvTraceSetTaskInstanceFinished(                              \
            (uint8_t)TRACE_GET_TASK_NUMBER(pxTCB));                   \
    }
...
#define prvTraceSetReadyEventsEnabled(status)
    ...
    void prvTraceStoreKernelCall(
        uint32_t         eventcode,
        traceObjectClass objectClass,
        uint32_t         byteParam);

    void prvTraceStoreKernelCallWithNumericParamOnly(
        uint32_t evtcode,
        uint32_t param);

    void prvTraceStoreKernelCallWithParam(
        uint32_t         evtcode,
        traceObjectClass objectClass,
...
#define prvTraceStoreKernelCallWithParam(      \
    evtcode, objectClass, objectNumber, param) \
    {                                          \
    }
     ...
     *
     * Allocates and initializes the recorder data structure, based on the
     *constants in trcConfig.h. This allows for allocating the data on the heap,
     *instead of using a static declaration.
     ******************************************************************************/
    void prvTraceInitTraceData(void);

    void prvTraceSetTaskInstanceFinished(traceHandle handle);

    void prvTraceSetPriorityProperty(
        uint8_t     objectclass,
        traceHandle id,
        uint8_t     value);
    ...
    void prvTraceStoreObjectNameOnCloseEvent(
        uint8_t          evtcode,
        traceHandle      handle,
        traceObjectClass objectclass);

    void prvTraceStoreObjectPropertiesOnCloseEvent(
        uint8_t          evtcode,
        traceHandle      handle,
        traceObjectClass objectclass);
...
#define xTraceSetISRProperties(name, priority)                                 \
     ...
     * solution, e.g., in case a debugger connection is not available for
     *uploading the data.
     ******************************************************************************/
     ...
     * vTraceGetTraceBuffer if you wish to implement an own store/upload
     *solution, e.g., in case a debugger connection is not available for
     *uploading the data.
     ******************************************************************************/
...
#define TRC_CFG_INCLUDE_USER_EVENTS 0
    ...
    traceUBChannel
         xTraceRegisterUBChannel(traceString channel, traceString formatStr);
...
#define trcCRITICAL_SECTION_END_ON_CORTEX_M_ONLY() recorder_busy--;
     ...
     *object handles. This way, only 1 byte is necessary instead of 4 bytes (a
     *pointer) when storing a reference to an object. This allows for up to 255
     *objects of each object class active at any given moment. There can be more
     *"historic" objects, that have been deleted - that number is only limited
     *by the size of the symbol table.
     *
     * Note that handle zero (0) is not used, it is a code for an invalid
     *handle.
     *
     * This data structure keeps track of the FREE handles, not the handles in
     *use. This data structure contains one stack per object class. When a
     *handle is allocated to an object, the next free handle is popped from the
     *stack. When a handle is released (on object delete), it is pushed back on
     *the stack. Note that there is no initialization code that pushed the free
     *handles initially, that is not necessary due to the following
     *optimization:
     *
     ...
     * The Object Table contains name and other properties of the objects
     *(tasks, queues, mutexes, etc). The below data structures defines the
     *properties of each object class and are used to cast the byte buffer into
     *a cleaner format.
     *
     * The values in the object table are continuously overwritten and always
     * represent the current state. If a property is changed during runtime, the
     *OLD value should be stored in the trace buffer, not the new value (since
     *the new value is found in the Object Property Table).
     *
    ...
    typedef struct
    {
        ...
        uint8_t           channelBuffer
            [((TRC_CFG_SEPARATE_USER_EVENT_BUFFER_SIZE) + 3) &
    ...
    {
        volatile uint8_t
                         startmarker0; /* Volatile is important, see init code. */
      ...
      */
        ...
        the have executed back-to-back and don't show any fragment of the
        previous context in between. */
...
#endif
        /* This is non-zero if an internal error occurred in the recorder, e.g.,
        if one of the Nxxx constants was too small. The systemInfo string will
        then contain an error message that is displayed when attempting to view
        the trace file. */
     ...
     *
     * Returns the current time based on the HWTC macros which provide a
     *hardware isolation layer towards the hardware timer/counter.
     *
    ...
    RecorderDataPtr->ObjectPropertyTable.objbytes          \
        [uiIndexOfObject(handle, objectclass) +            \
         RecorderDataPtr->ObjectPropertyTable              \
             .NameLengthPerClass[objectclass] +            \
         1]
...
#define TRC_STREAM_PORT_ON_TRACE_END() /* Do nothing */
     ...
     * space in the paged event buffer, or on the local stack. In the latter
     *case, the COMMIT event is used to write the data to the streaming
     *interface.
     *
...
#define TRC_STREAM_PORT_ALLOCATE_EVENT_BLOCKING(_type, _ptrData, _size) \
     ...
     * This macro is used to allocate memory for each event record, just before
     * assigning the record fields.
     * This has the same purpose as TRC_STREAM_PORT_ALLOCATE_EVENT and by
     *default it has the same definition as TRC_STREAM_PORT_ALLOCATE_EVENT. This
     *is used for events carrying variable-sized payload, such as strings. In
     *the SEGGER RTT port, we need this in order to make a worst-case allocation
     *on the stack.
     ******************************************************************************/
...
#define TRC_STREAM_PORT_ALLOCATE_DYNAMIC_EVENT(_type, _ptrData, _size) \
     ...
     * streaming inteface, without first storing the event in the internal
     *buffer. This is currently only used in the SEGGER J-Link RTT port.
     *
     * The BLOCKING version is used when sending the initial trace header, which
     *is important to receive in full. Otherwise, when using non-blocking RTT
     *transfer this may be corrupted if using an RTT buffer smaller than the
     *combined size of the header, object table and symbol table.
     *
     ...
     * prvTraceWarning(TRC_STREAM_PORT_WRITE_DATA) if a non-zero value is
     *returned from TRC_STREAM_PORT_WRITE_DATA. If zero (0) is returned, it is
     *assumed that all data was successfully written.
     *
     * In ports using the internal buffer, this macro has no purpose as the
     *events are written to the internal buffer instead. They are then flushed
     *to the streaming interface in the TzCtrl task using
...
#define PSF_WARNING_STACKMON_NO_SLOTS 13
    ...
    void
        prvTraceStoreEvent2(uint16_t eventID, uint32_t param1, uint32_t param2);

    /* Store an event with three 32-bit parameters */
    void prvTraceStoreEvent3(
        uint16_t eventID,
        uint32_t param1,
        uint32_t param2,
        uint32_t param3);
