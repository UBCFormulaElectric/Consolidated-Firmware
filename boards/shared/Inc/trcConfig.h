 ...
 * TRC_FREERTOS_VERSION_7_5_OR_7_6				If using FreeRTOS v7.5.0 -
 *v7.6.0 TRC_FREERTOS_VERSION_8_X						If using FreeRTOS v8.X.X
 * TRC_FREERTOS_VERSION_9_0_0					If using FreeRTOS v9.0.0
 * TRC_FREERTOS_VERSION_9_0_1					If using FreeRTOS v9.0.1
 * TRC_FREERTOS_VERSION_9_0_2					If using FreeRTOS v9.0.2
 * TRC_FREERTOS_VERSION_10_0_0					If using FreeRTOS v10.0.0 or
 *later
 *****************************************************************************/
...
#define TRC_CFG_SCHEDULING_ONLY 0
     ...
     *
     * This controls if malloc and free calls should be traced. Set this to zero
     *(0) to exclude malloc/free calls, or one (1) to include such events in the
     *trace.
     *
     * Default value is 1.
     *****************************************************************************/
#define TRC_CFG_INCLUDE_MEMMANG_EVENTS 1
     ...
     * to reduce code size. Any attempts of storing User Events are then
     *silently ignored.
     *
...
#define TRC_CFG_INCLUDE_ISR_TRACING 1
     ...
     * Macro which should be defined as either zero (0) or one (1).
     *
     * If one (1), events are recorded when tasks enter scheduling state
     *"ready". This allows Tracealyzer to show the initial pending time before
     *tasks enter the execution state, and present accurate response times. If
     *zero (0), "ready events" are not created, which allows for recording
...
#define TRC_CFG_INCLUDE_READY_EVENTS 1
     ...
     * increased. If zero (0), OS tick events are not generated, which allows
     *for recording longer traces in the same amount of RAM.
     *
     * Default value is 1.
     *****************************************************************************/
...
#define TRC_CFG_INCLUDE_STREAM_BUFFER_EVENTS 0
     ...
     *of all active tasks. The stack monitoring runs in the Tracealyzer Control
     *task, TzCtrl. This task is always created by the recorder when in
...
#define TRC_CFG_STACK_MONITOR_MAX_TASKS 10
     ...
     * Macro which should be defined as a non-zero integer value.
     *
     * This defines how many tasks that will be subject to stack usage analysis
     *for each execution of the Tracealyzer Control task (TzCtrl). Note that the
     *stack monitoring cycles between the tasks, so this does not affect WHICH
     *tasks that are monitored, but HOW OFTEN each task stack is analyzed.
     *
     * This setting can be combined with TRC_CFG_CTRL_TASK_DELAY to tune the
     * frequency of the stack monitoring. This is motivated since the stack
     *analysis can take some time to execute. However, note that the stack
     *analysis runs in a separate task (TzCtrl) that can be executed on low
     *priority. This way, you can avoid that the stack analysis disturbs any
...
#define TRC_CFG_STACK_MONITOR_MAX_REPORTS 1
     ...
     * execution and transfer of the data, but low enough to avoid disturbing
     *any time-sensitive functions.
     *
     * In Snapshot mode, TzCtrl is only used for the stack usage monitoring and
     *is not created if stack monitoring is disabled. TRC_CFG_CTRL_TASK_PRIORITY
...
#define TRC_CFG_CTRL_TASK_PRIORITY 1
     ...
     *
     * The delay between loops of the TzCtrl task (see
     *TRC_CFG_CTRL_TASK_PRIORITY), which affects the frequency of the stack
     *monitoring.
     *
     * In streaming mode, this also affects the trace data transfer if you are
     *using a stream port leveraging the internal buffer (like TCP/IP). A
     *shorter delay increases the CPU load of TzCtrl somewhat, but may improve
     *the performance of of the trace streaming, especially if the trace buffer
     *is small.
     ******************************************************************************/
