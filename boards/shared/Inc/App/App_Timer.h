...
typedef enum {
  ...
  TIMER_STATE_RUNNING,  // Running: Timer is running, but its duration hasn't
                        // elapsed yet.
 ...
 * Note: Do not interact with member vars directly! Instead, use the functions
...
/**
 * Restart a timer, i.e. set the elapsed time back to 0. Leaves the timer in
 * TIMER_STATE_RUNNING.
 * @param timer The timer in question
 */
 ...
 * TIMER_STATE_EXPIRED: Timer ran, and has completed its duration. This is
 * typically when your code will decide to do something.
 * @param timer The timer in question
...
/**
 * If condition is true and timer isn't running, restart the timer. If condition
 * is true and timer is already running, allow the timer to continue running. If
 * condition is false, stop the timer.
 * @param timer The timer in question
...
/**
 * Get the elapsed time since timer started, in milliseconds. Note that if timer
 * is TIMER_STATE_IDLE, the elapsed time is zero, and if the timer is
 * TIMER_STATE_EXPIRED, the elapsed time is the timer's duration.
 * @param timer The timer in question
 * @return Elapsed time
 */
...
/**
 * Set the present time used by all timers. Add this to the 1kHz task, or timers
 * won't work!
