...
TEST_F(TimerTest, test_get_elapsed_time) {
  ...
  for (int t = 1000; t < 3000; t += 100) {
    App_Timer_RunIfCondition(&timer, true);
    ASSERT_EQ(App_Timer_GetElapsedTime(&timer),
              (t >= 2000) ? 2000 : t);  // If t >= 2000, timer expired, so clamp
                                        // elapsed time to duration
    ASSERT_EQ(App_Timer_UpdateAndGetState(&timer),
              (t >= 2000) ? TIMER_STATE_EXPIRED : TIMER_STATE_RUNNING);
    PassTime(100);
  }
}
