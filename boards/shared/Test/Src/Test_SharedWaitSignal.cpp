...
class SharedWaitSignalTest : public testing::Test {
 protected:
  ...
  void SetInitialTimeAndDuration(uint32_t initial_time_ms,
                                 uint32_t wait_duration_ms) {
    TearDownObject(wait_signal, App_SharedWaitSignal_Destroy);

    callback.function = wait_callback_function;
    callback.wait_duration_ms = wait_duration_ms;
    wait_signal = App_SharedWaitSignal_Create(initial_time_ms, is_wait_high,
                                              world, callback);
  }

  void testCreateAndUpdateWaitSignalOnTheSameMillisecond(
      uint32_t start_ms, uint32_t wait_duration_ms) {
  ...
  void testWaitSignalStatusHighTriggersWaitForSomeMs(
      uint32_t start_ms, uint32_t first_wait_duration_ms,
    ...
    for (uint32_t ms = 0; ms < first_wait_duration_ms - 1; ms++) {
      ...
      ASSERT_EQ(start_ms + 1,
                App_SharedWaitSignal_GetLastTimeHighMs(wait_signal));
      ASSERT_TRUE(App_SharedWaitSignal_IsWaiting(wait_signal));
      ASSERT_EQ(expected_wait_callback_call_count,
                wait_callback_function_fake.call_count);
  ...
  void testWaitSignalStaysLowDoesNotTriggerWaitForSomeMs(
    ...
    for (uint32_t ms = 0; ms < first_wait_duration_ms + second_wait_duration_ms;
         ms++) {
      App_SharedWaitSignal_Update(wait_signal, ++current_ms);
      ASSERT_EQ(start_ms, App_SharedWaitSignal_GetLastTimeHighMs(wait_signal));
      ASSERT_FALSE(App_SharedWaitSignal_IsWaiting(wait_signal));
...
TEST_F(
    ...
    create_and_update_signal_on_the_same_millisecond_with_non_zero_start_time) {
  testCreateAndUpdateWaitSignalOnTheSameMillisecond(1, 0);
}

TEST_F(SharedWaitSignalTest,
       wait_signal_high_triggers_wait_with_zero_start_time) {
  testWaitSignalStatusHighTriggersWaitForSomeMs(0, WAIT_DURATION_MS, 0);
}

TEST_F(SharedWaitSignalTest,
       wait_signal_high_triggers_wait_with_non_zero_start_time) {
  testWaitSignalStatusHighTriggersWaitForSomeMs(1, WAIT_DURATION_MS, 0);
}

TEST_F(SharedWaitSignalTest,
       wait_signal_high_triggers_wait_with_non_zero_start_time_with_overflow) {
  testWaitSignalStatusHighTriggersWaitForSomeMs(UINT32_MAX - 1,
                                                WAIT_DURATION_MS, 0);
}

TEST_F(
    SharedWaitSignalTest,
    wait_signal_high_triggers_wait_with_zero_start_time_and_keep_waiting_after) {
  testWaitSignalStatusHighTriggersWaitForSomeMs(0, WAIT_DURATION_MS,
                                                WAIT_DURATION_MS);
}

TEST_F(
    SharedWaitSignalTest,
    wait_signal_high_triggers_wait_with_non_zero_start_time_and_keep_waiting_after) {
  testWaitSignalStatusHighTriggersWaitForSomeMs(1, WAIT_DURATION_MS,
                                                WAIT_DURATION_MS);
}
...
TEST_F(
  ...
  testWaitSignalStatusHighTriggersWaitForSomeMs(
      UINT32_MAX - 1, WAIT_DURATION_MS, WAIT_DURATION_MS);
}

TEST_F(SharedWaitSignalTest,
       wait_signal_low_does_not_trigger_wait_with_zero_start_time) {
  testWaitSignalStaysLowDoesNotTriggerWaitForSomeMs(0, WAIT_DURATION_MS, 0);
}

TEST_F(SharedWaitSignalTest,
       wait_signal_low_does_not_trigger_wait_with_non_zero_start_time) {
  testWaitSignalStaysLowDoesNotTriggerWaitForSomeMs(1, WAIT_DURATION_MS, 0);
}

TEST_F(SharedWaitSignalTest,
       wait_signal_low_does_not_trigger_wait_with_non_zero_time_with_overflow) {
  testWaitSignalStaysLowDoesNotTriggerWaitForSomeMs(UINT32_MAX - 1,
                                                    WAIT_DURATION_MS, 0);
}

TEST_F(
    SharedWaitSignalTest,
    wait_signal_low_does_not_trigger_wait_with_zero_start_time_and_keep_waiting_after) {
  testWaitSignalStaysLowDoesNotTriggerWaitForSomeMs(0, WAIT_DURATION_MS,
                                                    WAIT_DURATION_MS);
}

TEST_F(
    SharedWaitSignalTest,
    wait_signal_low_does_not_trigger_wait_with_non_zero_start_time_and_keep_waiting_after) {
  testWaitSignalStaysLowDoesNotTriggerWaitForSomeMs(1, WAIT_DURATION_MS,
                                                    WAIT_DURATION_MS);
}
...
TEST_F(
  ...
  testWaitSignalStaysLowDoesNotTriggerWaitForSomeMs(
      UINT32_MAX - 1, WAIT_DURATION_MS, WAIT_DURATION_MS);
}
