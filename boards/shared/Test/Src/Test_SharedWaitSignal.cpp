...
class SharedWaitSignalTest : public testing::Test
{
  protected:
    void SetUp() override
    {
        ...
        wait_signal =
            App_SharedWaitSignal_Create(0, is_wait_high, world, callback);
        ...
        uint32_t wait_duration_ms)
    {
        ...
        wait_signal               = App_SharedWaitSignal_Create(
            initial_time_ms, is_wait_high, world, callback);
    }
        ...
        uint32_t wait_duration_ms)
    {
        ...
        ASSERT_EQ(
            start_ms, App_SharedWaitSignal_GetLastTimeHighMs(wait_signal));
    }
        ...
        uint32_t second_wait_duration_ms)
    {
        ...
        expected_wait_callback_call_count++;
        ASSERT_EQ(
            start_ms + 1, App_SharedWaitSignal_GetLastTimeHighMs(wait_signal));
        ...
        uint32_t second_wait_duration_ms)
    {
        ...
        for (uint32_t ms = 0;
             ms < first_wait_duration_ms + second_wait_duration_ms; ms++)
        {
            App_SharedWaitSignal_Update(wait_signal, ++current_ms);
            ASSERT_EQ(
                start_ms, App_SharedWaitSignal_GetLastTimeHighMs(wait_signal));
    ...
    wait_signal_high_triggers_wait_with_non_zero_start_time_with_overflow)
{
    testWaitSignalStatusHighTriggersWaitForSomeMs(
        UINT32_MAX - 1, WAIT_DURATION_MS, 0);
}
...
{
    testWaitSignalStatusHighTriggersWaitForSomeMs(
        0, WAIT_DURATION_MS, WAIT_DURATION_MS);
}
...
{
    testWaitSignalStatusHighTriggersWaitForSomeMs(
        1, WAIT_DURATION_MS, WAIT_DURATION_MS);
}
