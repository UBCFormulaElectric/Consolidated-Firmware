...
class SharedSignalTest : public testing::Test {
 protected:
  ...
  // Should be enough duration to give us confidence in the test without making
  // the test too slow.
  const uint32_t ENTRY_DURATION_HIGH_MS = 10;
  const uint32_t EXIT_DURATION_HIGH_MS = 10;
  // The number of multiple cycles is more-or-less arbitrarily chosen. It should
  // be enough cycles and time to give us confidence in the test without making
  // the test too slow.
  ...
  void SetUp() override {
    ...
    signal =
        App_SharedSignal_Create(ENTRY_DURATION_HIGH_MS, EXIT_DURATION_HIGH_MS);
  }
...
TEST_F(SharedSignalTest, stays_high) {
  for (uint32_t i = 0; i < 1000; i++) {
    ...
    SignalState correctState =
        (i < ENTRY_DURATION_HIGH_MS) ? SIGNAL_STATE_CLEAR : SIGNAL_STATE_ACTIVE;
