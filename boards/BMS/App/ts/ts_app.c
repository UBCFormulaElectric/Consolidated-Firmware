...
static float ts_app_getVoltage(void);
static float ts_app_getLowResCurrent(void);
static float ts_app_getHighResCurrent(void);

static void ts_app_tick10Hz(void);

module_s tractive_system = {
    .init       = NULL,
    .tick_1Hz   = NULL,
    .tick_10Hz  = ts_app_tick10Hz,
    .tick_100Hz = NULL,
    .tick_1kHz  = NULL,
};
...
static void ts_app_tick10Hz(void)
{
    const float low_res_current = ts_app_getLowResCurrent();
    const float ts_current =
        (low_res_current < HIGH_RES_MAX_CURRENT_READING) ? ts_app_getHighResCurrent() : low_res_current;

    App_CanTx_SetPeriodicSignal_TS_VOLTAGE(&can_tx, ts_app_getVoltage());
