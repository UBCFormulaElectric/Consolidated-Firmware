...
struct BmsWorld *App_BmsWorld_Create(
    struct Imd *const imd, struct HeartbeatMonitor *const heartbeat_monitor,
    struct RgbLedSequence *const rgb_led_sequence,
    struct Charger *const charger, struct OkStatus *const bms_ok,
    struct OkStatus *const imd_ok, struct OkStatus *const bspd_ok,
    struct Accumulator *const accumulator, struct Airs *const airs,
    struct PrechargeRelay *const precharge_relay,
    struct TractiveSystem *const tractive_system, struct Clock *const clock,
...
void App_BmsWorld_Destroy(struct BmsWorld *world) { free(world); }
...
struct OkStatus *App_BmsWorld_GetBmsOkStatus(
    const struct BmsWorld *const world) {
  return world->bms_ok;
}

struct OkStatus *App_BmsWorld_GetImdOkStatus(
    const struct BmsWorld *const world) {
  return world->imd_ok;
}

struct OkStatus *App_BmsWorld_GetBspdOkStatus(
    const struct BmsWorld *const world) {
  return world->bspd_ok;
}

struct Accumulator *App_BmsWorld_GetAccumulator(
    const struct BmsWorld *const world) {
