 ...
 */
static void App_Accumulator_CalculateVoltageStats(
    struct Accumulator *accumulator) {
  VoltageStats temp_voltage_stats = {
      .min_voltage = {.segment = 0U, .cell = 0U, .voltage = FLT_MAX},
  ...
  for (uint8_t segment = 0U; segment < ACCUMULATOR_NUM_SEGMENTS; segment++) {
    for (uint8_t cell = 0U; cell < ACCUMULATOR_NUM_SERIES_CELLS_PER_SEGMENT;
         cell++) {
      // Collect each cell voltage to find the min/max
      const float cell_voltage =
          App_Accumulator_GetCellVoltage(accumulator, segment, cell);
    ...
    temp_voltage_stats.pack_voltage +=
        temp_voltage_stats.segment_voltages[segment];
  }
...
void App_Accumulator_InitRunOnEntry(
...
bool App_Accumulator_EnableDischarge(
    const struct Accumulator *const accumulator) {
  return accumulator->enable_discharge();
}

bool App_Accumulator_DisableDischarge(
    const struct Accumulator *const accumulator) {
...
void App_Accumulator_RunOnTick100Hz(struct Accumulator *const accumulator) {
  switch (accumulator->state) {
    case GET_CELL_VOLTAGE_STATE: {
      // Attempt to read voltages from the LTCs, write output to cell voltages
      // array
      UPDATE_PEC15_ERROR_COUNT(
          accumulator->read_cell_voltages(accumulator->cell_voltages),
    ...
    case GET_CELL_TEMP_STATE: {
      UPDATE_PEC15_ERROR_COUNT(accumulator->read_cell_temperatures(),
                               accumulator->num_comm_tries)
...
bool App_Accumulator_CheckFaults(struct Accumulator *const accumulator,
  ...
  bool overtemp_fault = App_Accumulator_GetMaxCellTempDegC(
                            accumulator, &throwaway_segment, &throwaway_loc) >
                        max_allowable_cell_temp;
  bool undertemp_fault = App_Accumulator_GetMinCellTempDegC(
                             accumulator, &throwaway_segment, &throwaway_loc) <
                         min_allowable_cell_temp;
  bool overvoltage_fault =
      App_Accumulator_GetMaxVoltage(accumulator, &throwaway_segment,
                                    &throwaway_loc) > MAX_CELL_VOLTAGE;
  bool undervoltage_fault =
      App_Accumulator_GetMinVoltage(accumulator, &throwaway_segment,
                                    &throwaway_loc) < MIN_CELL_VOLTAGE;
  ...
  return (overtemp_fault || undertemp_fault || overvoltage_fault ||
          undervoltage_fault || communication_fault);
}
