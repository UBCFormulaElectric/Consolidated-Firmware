...
static void convert_float_to_bytes(uint8_t *byte_array,
                                   float float_to_convert) {
...
static void convert_short_to_bytes(uint8_t *byte_array,
                                   uint16_t short_to_convert) {
...
void App_Eeprom_Destroy(struct Eeprom *eeprom) { free(eeprom); }

EEPROM_StatusTypeDef App_Eeprom_WriteFloats(struct Eeprom *eeprom,
  ...
  return eeprom->write_page(page, offset, data_bytes,
                            (uint16_t)(num_floats * sizeof(float)));
}

EEPROM_StatusTypeDef App_Eeprom_ReadFloats(struct Eeprom *eeprom, uint16_t page,
                                           uint8_t offset, float *output_data,
                                           uint8_t num_floats) {
  ...
  read_status = eeprom->read_page(page, offset, data,
                                  (uint16_t)(num_floats * sizeof(float)));
...
EEPROM_StatusTypeDef App_Eeprom_PageErase(struct Eeprom *eeprom,
                                          uint16_t page) {
  return eeprom->page_erase(page);
}

EEPROM_StatusTypeDef App_Eeprom_Write4CopiesOfAddress(struct Eeprom *eeprom,
                                                      uint16_t page,
                                                      uint16_t address) {
  uint16_t num_bytes =
      SAVED_COPIES *
      sizeof(uint16_t);  // saving 3 copies of address, each 2 bytes
...
EEPROM_StatusTypeDef App_Eeprom_Read4CopiesOfAddresses(struct Eeprom *eeprom,
                                                       uint16_t page,
                                                       uint16_t *addresses) {
  uint16_t num_bytes =
      SAVED_COPIES *
