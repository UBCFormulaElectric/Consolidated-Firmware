...
static void App_CheckCellVoltageRange(struct Accumulator *accumulator) {
  // Get the min and max cell voltages and check to see if the voltages are in
  // range
  uint8_t min_segment = 0U;
  uint8_t min_loc = 0U;
  uint8_t max_segment = 0U;
  uint8_t max_loc = 0U;
  const float curr_min_cell_voltage =
      App_Accumulator_GetMinVoltage(accumulator, &min_segment, &min_loc);
  const float curr_max_cell_voltage =
      App_Accumulator_GetMaxVoltage(accumulator, &max_segment, &max_loc);
...
static void App_CheckCellTemperatureRange(struct Accumulator *accumulator,
  ...
  uint8_t max_segment = 0U;
  uint8_t max_loc = 0U;
  const float curr_min_cell_temp =
      App_Accumulator_GetMinCellTempDegC(accumulator, &min_segment, &min_loc);
  const float curr_max_cell_temp =
      App_Accumulator_GetMaxCellTempDegC(accumulator, &max_segment, &max_loc);
...
static void App_AdvertisePackPower(struct Accumulator *accumulator,
                                   struct TractiveSystem *ts) {
  uint8_t segment = 0;
  UNUSED(segment);

  const float max_cell_temp =
      App_Accumulator_GetMaxCellTempDegC(accumulator, &segment, &segment);
  const float available_power =
      MIN(App_SharedProcessing_LinearDerating(max_cell_temp, MAX_POWER_LIMIT_W,
...
bool App_AllStatesRunOnTick100Hz(struct StateMachine *const state_machine) {
  ...
  App_CanTx_BMS_PackVoltage_PackVoltage_Set(
  ...
  App_CanTx_BMS_Contactors_AirNegative_Set(App_Airs_IsAirNegativeClosed(airs)
                                               ? CONTACTOR_STATE_CLOSED
                                               : CONTACTOR_STATE_OPEN);
  App_CanTx_BMS_Contactors_AirPositive_Set(App_Airs_IsAirPositiveClosed(airs)
                                               ? CONTACTOR_STATE_CLOSED
                                               : CONTACTOR_STATE_OPEN);
  ...
  // Wait for cell voltage and temperature measurements to settle. We expect to
  // read back valid values from the monitoring chips within 3 cycles
