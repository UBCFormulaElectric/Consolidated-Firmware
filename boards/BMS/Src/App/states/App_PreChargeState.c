    ...
    PreChargeStateRunOnTick100Hz(struct StateMachine *const state_machine)
{
    ...
    //percentage threshold, which when TS reaches this threshold of ACC, AIR+ engages and precharge is disabled
    //must be at least 90% EV.6.6.1
    ...
    float threshold_voltage = accumulator_voltage * precharge_threshold_percentage;
    ...
    if ((tractive_system_voltage < threshold_voltage && (float)elapsed_time >= upper_precharge_time)
         || (tractive_system_voltage > threshold_voltage && (float)elapsed_time <= lower_precharge_time))
    {
        App_SharedStateMachine_SetNextState(state_machine, App_GetFaultState());
    }

    // Successful precharge if tractive system reaches threshold between the lower
    // and upper time bounds (expected range precharge time)
