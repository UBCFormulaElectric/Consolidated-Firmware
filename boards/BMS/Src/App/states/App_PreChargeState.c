...
static const struct State *_prechargeInDurationRunOnTick100Hz(struct BmsWorld *world) {
    struct BmsCanTxInterface *can_tx      = App_BmsWorld_GetCanTx(world);
    struct PreCharge *        pre_charge  = App_BmsWorld_GetPreCharge(world);
    ...
    const float pre_charge_voltage_threshold = _getPrechargeVoltageThreshold(max_pack_voltage);
    ...
    return App_GetPreChargeState();
}

static const struct State *_prechargeOutOfDurationRunOnTick100Hz(struct BmsWorld *world) {
    struct BmsCanTxInterface *can_tx      = App_BmsWorld_GetCanTx(world);
    struct PreCharge *        pre_charge  = App_BmsWorld_GetPreCharge(world);
    struct Accumulator *      accumulator = App_BmsWorld_GetAccumulator(world);
    ...
    {
        return App_GetChargeState();
    }

    // todo: add ErrorTable_HasAnyMotorShutdownErrorSet checks to drive state
    // BMS-45: Precharge success without charger
    return App_GetDriveState();
    ...
    PreChargeStateRunOnTick100Hz(struct StateMachine *const state_machine)
{
    ...
    {
        next_state = _prechargeInDurationRunOnTick100Hz(world);
    }
    else
    {
        next_state = _prechargeOutOfDurationRunOnTick100Hz(world);
    }
