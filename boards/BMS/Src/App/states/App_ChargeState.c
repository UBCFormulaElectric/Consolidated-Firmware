...
static void ChargeStateRunOnTick100Hz(
  ...
  if (App_AllStatesRunOnTick100Hz(state_machine)) {
    ...
    const bool has_reached_max_v =
        App_Accumulator_GetMaxVoltage(accumulator, &segment, &cell) >
        MAX_CELL_VOLTAGE_THRESHOLD;

    App_CanTx_BMS_Charger_IsChargingComplete_Set(has_reached_max_v);
    App_CanAlerts_SetFault(BMS_FAULT_CHARGER_DISCONNECTED_DURING_CHARGE,
                           is_charger_disconnected);
    App_CanAlerts_SetFault(BMS_FAULT_CHARGER_FAULT, has_charger_faulted);
    App_CanAlerts_SetFault(BMS_FAULT_CHARGER_EXTERNAL_SHUTDOWN,
    ...
    if (is_charger_disconnected || has_charger_faulted || has_reached_max_v ||
        has_external_shutdown_occurred || is_missing_hb) {
