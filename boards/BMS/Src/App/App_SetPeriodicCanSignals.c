...
void App_SetPeriodicCanSignals_Imd(struct Imd *imd) {
  App_CanTx_BMS_ImdStatus_SecondsSincePowerOn_Set(
      App_Imd_GetSecondsSincePowerOn(imd));
  ...
  App_CanTx_BMS_ImdStatus_ValidDutyCycle_Set(
  ...
  switch (condition.name) {
    ...
    case IMD_SST: {
      App_CanTx_BMS_ImdData_SpeedStartStatus30Hz_Set(
