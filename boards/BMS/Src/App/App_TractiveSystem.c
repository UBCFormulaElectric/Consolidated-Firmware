...
void App_TractiveSystem_Destroy(struct TractiveSystem *ts) { free(ts); }
...
float App_TractiveSystem_GetPower(struct TractiveSystem *ts) {
  return App_TractiveSystem_GetVoltage(ts) *
         App_TractiveSystem_GetLowResCurrent(ts);
}
...
bool App_TractveSystem_CheckFaults(struct TractiveSystem *ts) {
  ...
  bool ts_current_out_of_bounds =
      !IS_IN_RANGE(MAX_TS_CHARGE_CURRENT_AMPS, MAX_TS_DISCHARGE_CURRENT_AMPS,
