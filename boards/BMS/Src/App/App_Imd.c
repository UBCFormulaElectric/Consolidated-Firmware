 ...
 */
static enum Imd_ConditionName App_EstimateConditionName(float frequency,
                                                        float tolerance);
...
static enum Imd_ConditionName App_EstimateConditionName(const float frequency,
  ...
  for (enum Imd_ConditionName i = 0U; i < NUM_OF_IMD_CONDITIONS; i++) {
    ...
    // underflow
    const float lower_bound = MIN(App_GetIdealPwmFrequency(i),
...
static float App_GetIdealPwmFrequency(
    const enum Imd_ConditionName condition_name) {
  ...
  static const float imd_frequency_lookup[NUM_OF_IMD_CONDITIONS] = {
      ...
      [IMD_DEVICE_ERROR] = 40.0f,          [IMD_GROUND_FAULT] = 50.0f,
  };

  return imd_frequency_lookup[condition_name];
}

struct Imd *App_Imd_Create(float (*const get_pwm_frequency)(void),
                           float pwm_frequency_tolerance,
...
void App_Imd_Destroy(struct Imd *const imd) { free(imd); }
    ...
    case IMD_UNDERVOLTAGE_DETECTED: {
      ...
      if (condition.pwm_encoding.valid_duty_cycle) {
        if (pwm_duty_cycle == 5.0f) {
          ...
          uint16_t resistance =
              (uint16_t)(1080.0f / (pwm_duty_cycle / 100.0f - 0.05f) - 1200.0f);
