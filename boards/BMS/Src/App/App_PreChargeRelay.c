...
void App_PrechargeRelay_IncFaultCounter(
...
bool App_PrechargeRelay_CheckFaults(struct PrechargeRelay *precharge_relay,
  ...
  const bool has_precharge_fault =
      (is_charger_connected) ? is_ts_rising_slowly
                             : (is_ts_rising_slowly | is_ts_rising_quickly);

  if (has_precharge_fault) {
    App_PrechargeRelay_IncFaultCounter(precharge_relay);
  }

  *precharge_limit_exceeded = App_PrechargeRelay_GetFaultCounterVal(
                                  precharge_relay) >= MAX_PRECHARGE_ATTEMPTS;
