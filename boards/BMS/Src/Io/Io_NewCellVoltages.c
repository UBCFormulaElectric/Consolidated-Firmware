...
//    uint8_t *rx_cell_v)
//{
//    for (uint8_t curr_segment = 0U; curr_segment <
...
//    Io_ResetVoltages();
//
//    // Loop through each segment and cell to get: min and max voltages,
//    segment
...
void Io_ChangeEndianness(uint16_t rx_buffer[4U])
{
...
bool Io_NewCellVoltages_ReadVoltages(void)
{
         ...
         curr_reg_group++)
    {
        ...
        uint16_t tx_cmd[2U] = { [0] = cv_read_cmds[curr_reg_group], [1] = 0U };
        Io_LTC6813_PrepareCmdNew(tx_cmd);
                ...
                (uint8_t *)rx_buffer, TOTAL_NUM_OF_REG_BYTES))
        {
            ...
            Io_ChangeEndianness(rx_buffer);

            for (uint8_t curr_segment = 0U;
                 curr_segment < NUM_OF_ACCUMULATOR_SEGMENTS; curr_segment++)
            {
                uint8_t        start_index = (uint8_t)(curr_segment * 4U);
                const uint16_t recv_pec15  = rx_buffer[3U];
                ...
                if (recv_pec15 == calc_pec15)
                {
                    memcpy(
                        ...
                        (uint8_t *)&rx_buffer[start_index], 6U);
                }
                else
                {
                    return false;
