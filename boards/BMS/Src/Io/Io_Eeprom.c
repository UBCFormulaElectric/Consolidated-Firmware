...
#define MAX_ADDRESS_WORDS (MAX_ADDRESS_BYTES / BYTES_PER_WORD)
...
{
    if (address_bytes >= MAX_ADDRESS_BYTES)
    {
        return false;
    }

    const uint8_t tx_buf_size = (uint8_t)(1 + size_bytes);
    uint8_t       tx_buf[tx_buf_size];

    uint8_t  address_lsb = (uint8_t)(address_bytes & 0xFFU);
    tx_buf[0] = address_lsb;
    memcpy(&tx_buf[1], data, size_bytes);

    const uint8_t i2c_address = Io_Eeprom_GetI2cAddress(address_bytes);
    return EXIT_OK(Io_SharedI2c_Transmit(&i2c, i2c_address, tx_buf, tx_buf_size));
}
...
bool Io_Eeprom_ReadRaw(uint16_t address_bytes, void *data, uint8_t size_bytes)
{
    ...
    uint8_t  address_lsb = (uint8_t)(address_bytes & 0xFFU);
    return EXIT_OK(
        Io_SharedI2c_TransmitAndReceive(&i2c, i2c_address, &address_lsb, 1, (uint8_t *)data, size_bytes));
}
