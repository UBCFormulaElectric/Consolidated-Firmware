...
#define PAGE_ADDR_START_BIT    \
  ((uint16_t)(log(PAGE_SIZE) / \
              log(2)))  // number of bit where addressing starts
#define MEM_ACCESS_TIMEOUT (1000U)

EEPROM_StatusTypeDef Io_Eeprom_WritePage(uint16_t page, uint8_t offset,
  ...
  i2c_status =
      HAL_I2C_Mem_Write(EEPROM_I2C, EEPROM_ADDR, mem_address,
                        I2C_MEMADD_SIZE_8BIT, data, size, MEM_ACCESS_TIMEOUT);
...
EEPROM_StatusTypeDef Io_Eeprom_ReadPage(uint16_t page, uint8_t offset,
                                        uint8_t *data, uint16_t size) {
  HAL_StatusTypeDef i2c_status;
  ...
  i2c_status =
      HAL_I2C_Mem_Read(EEPROM_I2C, EEPROM_ADDR, mem_address,
                       I2C_MEMADD_SIZE_8BIT, data, size, MEM_ACCESS_TIMEOUT);
...
EEPROM_StatusTypeDef Io_Eeprom_PageErase(uint16_t page) {
  ...
  i2c_status = HAL_I2C_Mem_Write(EEPROM_I2C, EEPROM_ADDR, mem_address, 2, data,
                                 PAGE_SIZE, 1000);
