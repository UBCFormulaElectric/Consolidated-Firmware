...
struct LTC6813Temperatures {
  ...
  uint16_t cell[ACCUMULATOR_NUM_SEGMENTS][NUM_OF_AUX_REGISTER_GROUPS]
 ...
 */
static uint16_t Io_CalculateThermistorTempDeciDegC(
 ...
 */
static bool Io_ParseCellTempFromAllSegments(
  ...
  for (uint8_t curr_segment = 0U; curr_segment < ACCUMULATOR_NUM_SEGMENTS;
    ...
    if (recv_pec15 == calc_pec15) {
      for (uint8_t curr_thermistor = 0U;
        ...
        if (IS_CELL_TEMP_READING(curr_reg_group, curr_thermistor)) {
          ltc6813_temp.cell[curr_segment][curr_reg_group][curr_thermistor] =
              Io_CalculateThermistorTempDeciDegC(
                  rx_buffer[start_index + curr_thermistor]);
        }
      }
    } else {
      status = false;
...
bool Io_LTC6813CellTemperatures_ReadTemperatures(void) {
  ...
  if (Io_LTC6813Shared_PollAdcConversions()) {
    ...
    Io_UpdateCellTemperatureStatistics();
  }

  return status;
}

float Io_LTC6813CellTemperatures_GetMinTempDegC(uint8_t *segment,
                                                uint8_t *therm) {
...
float Io_LTC6813CellTemperatures_GetMaxTempDegC(uint8_t *segment,
                                                uint8_t *therm) {
