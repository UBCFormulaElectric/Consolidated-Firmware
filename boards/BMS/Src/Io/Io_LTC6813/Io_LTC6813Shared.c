  ...
  */
  ...
  for (uint8_t curr_segment = 0U; curr_segment < ACCUMULATOR_NUM_SEGMENTS;
    ...
    const uint8_t tx_cfg_idx =
        (uint8_t)(ACCUMULATOR_NUM_SEGMENTS - curr_segment - 1);

    // Set default tx_cfg for each segment
    memcpy(&tx_cfg[tx_cfg_idx], ltc6813_configs[curr_cfg_reg].default_cfg_reg,
...
void Io_LTC6813Shared_InitSpiHandle(SPI_HandleTypeDef *spi_handle) {
  ...
  ltc6813_spi = Io_SharedSpi_Create(spi_handle, SPI1_NSS_GPIO_Port,
                                    SPI1_NSS_Pin, SPI_TIMEOUT_MS);
}

uint16_t Io_LTC6813Shared_CalculateRegGroupPec15(const uint8_t *data_buffer) {
  return CHANGE_WORD_ENDIANNESS(
      Io_CalculatePec15(data_buffer, NUM_REG_GROUP_PAYLOAD_BYTES));
}
...
void Io_LTC6813Shared_PackRegisterGroupPec15(uint8_t *tx_cfg) {
  *(uint16_t *)(tx_cfg + NUM_REG_GROUP_PAYLOAD_BYTES) =
      Io_LTC6813Shared_CalculateRegGroupPec15(tx_cfg);
}
...
bool Io_LTC6813Shared_SendCommand(uint16_t cmd) {
  ...
  return Io_SharedSpi_Transmit(ltc6813_spi, (uint8_t *)tx_cmd,
                               TOTAL_NUM_CMD_BYTES);
}
...
bool Io_LTC6813Shared_WriteConfigurationRegisters(void) {
  for (uint8_t curr_cfg_reg = 0U; curr_cfg_reg < NUM_OF_CFG_REGS;
    ...
    uint16_t tx_cmd[NUM_OF_CMD_WORDS] = {
        [CMD_WORD] = ltc6813_configs[curr_cfg_reg].cfg_reg_cmds,
        [CMD_PEC15] = 0U};
    ...
    if (Io_SharedSpi_TransmitWithoutNssToggle(ltc6813_spi, (uint8_t *)tx_cmd,
                                              TOTAL_NUM_CMD_BYTES)) {
      if (!Io_SharedSpi_TransmitWithoutNssToggle(ltc6813_spi, (uint8_t *)tx_cfg,
