...
float Io_VoltageSense_GetTractiveSystemVoltage()
{
    ...
    //                                      GND
    //
    // TS Voltage x Voltage Ratio x Amplifier Gain x Error Correction Constant = ADC voltage, where
    //
    //                              1.24k
    // Voltage Ratio = -------------------------------------
    ...
    // Amplifier Gain = 8
    //
    // Error Correction: value is found through testing. Error can be caused by resistor tolerances and other factors
    //
    // We can rewrite the earlier equation as:
    //
    //                         ADC voltage
    // TS_Voltage = ----------------------------------  * Error Correction Constant
    ...
    return (adc_voltage) / (voltage_ratio * amplifier_gain) * (1 + real_error_percentage/100);
}
