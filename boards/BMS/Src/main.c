...
int main(void) {
  ...
  HAL_ADC_Start_DMA(&hadc1, (uint32_t *)Io_Adc_GetRawAdcValues(),
                    hadc1.Init.NbrOfConversion);
  HAL_TIM_Base_Start(&htim13);

  Io_SharedHardFaultHandler_Init();
  Io_SharedSoftwareWatchdog_Init(Io_HardwareWatchdog_Refresh,
                                 Io_SoftwareWatchdog_TimeoutCallback);
  Io_SharedCan_Init(&hcan1, CanTxQueueOverflowCallBack,
                    CanRxQueueOverflowCallBack);
  ...
  Io_Imd_Init();
  imd = App_Imd_Create(Io_Imd_GetFrequency, IMD_FREQUENCY_TOLERANCE,
                       Io_Imd_GetDutyCycle, Io_Imd_GetTimeSincePowerOn);

  heartbeat_monitor = App_SharedHeartbeatMonitor_Create(
      Io_SharedHeartbeatMonitor_GetCurrentMs,
      HEARTBEAT_MONITOR_TIMEOUT_PERIOD_MS, HEARTBEAT_MONITOR_BOARDS_TO_CHECK);

  rgb_led_sequence = App_SharedRgbLedSequence_Create(
      Io_RgbLedSequence_TurnOnRedLed, Io_RgbLedSequence_TurnOnBlueLed,
      Io_RgbLedSequence_TurnOnGreenLed);

  charger = App_Charger_Create(Io_Charger_Enable, Io_Charger_Disable,
                               Io_Charger_IsConnected, Io_Charger_HasFaulted);

  bms_ok =
      App_OkStatus_Create(Io_OkStatuses_EnableBmsOk, Io_OkStatuses_DisableBmsOk,
                          Io_OkStatuses_IsBmsOkEnabled);

  imd_ok =
      App_OkStatus_Create(Io_OkStatuses_EnableImdOk, Io_OkStatuses_DisableImdOk,
                          Io_OkStatuses_IsImdOkEnabled);

  bspd_ok = App_OkStatus_Create(Io_OkStatuses_EnableBspdOk,
                                Io_OkStatuses_DisableBspdOk,
  ...
  accumulator = App_Accumulator_Create(
      ...
      Io_LTC6813CellTemperatures_GetMinTempDegC,
      Io_LTC6813CellTemperatures_GetMaxTempDegC,
      Io_LTC6813CellTemperatures_GetAverageTempDegC,
      Io_LTC6813Shared_EnableDischarge, Io_LTC6813Shared_DisableDischarge);

  ts = App_TractiveSystem_Create(Io_VoltageSense_GetTractiveSystemVoltage,
                                 Io_CurrentSense_GetHighResolutionMainCurrent,
                                 Io_CurrentSense_GetLowResolutionMainCurrent);

  airs =
      App_Airs_Create(Io_Airs_IsAirPositiveClosed, Io_Airs_IsAirNegativeClosed,
  ...
  eeprom = App_Eeprom_Create(Io_Eeprom_WritePage, Io_Eeprom_ReadPage,
                             Io_Eeprom_PageErase);

  world = App_BmsWorld_Create(imd, heartbeat_monitor, rgb_led_sequence, charger,
                              bms_ok, imd_ok, bspd_ok, accumulator, airs,
  ...
  osThreadStaticDef(Task100Hz, RunTask100Hz, osPriorityBelowNormal, 0,
                    TASK100HZ_STACK_SIZE, Task100HzBuffer,
                    &Task100HzControlBlock);
  Task100HzHandle = osThreadCreate(osThread(Task100Hz), NULL);

  /* definition and creation of TaskCanRx */
  osThreadStaticDef(TaskCanRx, RunTaskCanRx, osPriorityIdle, 0,
                    TASKCANRX_STACK_SIZE, TaskCanRxBuffer,
                    &TaskCanRxControlBlock);
  TaskCanRxHandle = osThreadCreate(osThread(TaskCanRx), NULL);

  /* definition and creation of TaskCanTx */
  osThreadStaticDef(TaskCanTx, RunTaskCanTx, osPriorityIdle, 0,
                    TASKCANTX_STACK_SIZE, TaskCanTxBuffer,
                    &TaskCanTxControlBlock);
  TaskCanTxHandle = osThreadCreate(osThread(TaskCanTx), NULL);

  /* definition and creation of Task1kHz */
  osThreadStaticDef(Task1kHz, RunTask1kHz, osPriorityNormal, 0,
                    TASK1KHZ_STACK_SIZE, Task1kHzBuffer, &Task1kHzControlBlock);
  Task1kHzHandle = osThreadCreate(osThread(Task1kHz), NULL);

  /* definition and creation of Task1Hz */
  osThreadStaticDef(Task1Hz, RunTask1Hz, osPriorityLow, 0, TASK1HZ_STACK_SIZE,
...
static void MX_I2C1_Init(void) {
  ...
  // Manually pulse the I2C clock signal 10 times, this can recover the I2C bus
  // from a locked state after a reflash or reset

  // To manually pulse must first set I2C pins to GPIO General Purpose Output
  // Open-Drain
...
static void MX_GPIO_Init(void) {
  ...
  HAL_GPIO_WritePin(GPIOC,
                    AIR_EN_Pin | PRE_CHARGE_EN_Pin | AUX_TSENSE_MUX1_Pin |
                        AUX_TSENSE_MUX2_Pin | AUX_TSENSE_MUX3_Pin,
                    GPIO_PIN_RESET);
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOA, CHRG_EN_3V3_Pin | AUX_TSENSE_MUX0_Pin,
                    GPIO_PIN_RESET);

  /*Configure GPIO pins : AIR_PWR_STATUS_Pin TS_ISENSE_OCSC_OK_5V_Pin
     ACCEL_BRAKE_OK_5V_Pin BSPD_OK_3V3_Pin CHRG_STATE_3V3_Pin CHRG_FLT_3V3_Pin
   */
  GPIO_InitStruct.Pin = AIR_PWR_STATUS_Pin | TS_ISENSE_OCSC_OK_5V_Pin |
                        ACCEL_BRAKE_OK_5V_Pin | BSPD_OK_3V3_Pin |
                        CHRG_STATE_3V3_Pin | CHRG_FLT_3V3_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);

  /*Configure GPIO pins : AIR_EN_Pin PRE_CHARGE_EN_Pin AUX_TSENSE_MUX1_Pin
     AUX_TSENSE_MUX2_Pin AUX_TSENSE_MUX3_Pin */
  GPIO_InitStruct.Pin = AIR_EN_Pin | PRE_CHARGE_EN_Pin | AUX_TSENSE_MUX1_Pin |
                        AUX_TSENSE_MUX2_Pin | AUX_TSENSE_MUX3_Pin;
  ...
  /*Configure GPIO pins : BMS_LATCH_Pin IMD_LATCH_Pin BSPD_LATCH_Pin
   * BSPD_I_TRIP_5V_Pin */
  GPIO_InitStruct.Pin =
      BMS_LATCH_Pin | IMD_LATCH_Pin | BSPD_LATCH_Pin | BSPD_I_TRIP_5V_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(GPIOB, &GPIO_InitStruct);
...
void RunTask1kHz(void const *argument) {
  ...
  /* Infinite loop */
  for (;;) {
    // ADC wasn't reading any voltages when triggered by TIM3 like on other
    // boards But worked fine when starting the conversion via software as below
    // TODO: Figure out why
    HAL_ADC_Start_DMA(&hadc1, (uint32_t *)Io_Adc_GetRawAdcValues(),
                      hadc1.Init.NbrOfConversion);
...
void RunTask1Hz(void const *argument) {
  ...
  for (;;) {
    ...
    const bool debug_mode_enabled =
        App_CanRx_Debug_CanModes_EnableDebugMode_Get();
