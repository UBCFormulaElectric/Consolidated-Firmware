...
TEST_F(BmsFaultTest, check_state_transition_to_fault_state_from_all_states_overtemp_charge_state)
{
    ...
    // Set TS current above threshold to remain charging and to trigger charging condition in temperature check
    get_high_res_current_fake.return_val = 10.0f;
    get_low_res_current_fake.return_val  = 10.0f;
...
TEST_F(BmsFaultTest, check_state_transition_fault_state_precharge_fault)
{
    ...
    for (int i = 1; i <= 3; i++)
    {
        ...
        // force a pre-charge fault condition by making ts appear to be rising slowly
        LetTimePass(state_machine, PRECHARGE_COMPLETION_UPPER_BOUND + 10);
