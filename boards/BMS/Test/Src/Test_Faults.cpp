...
namespace FaultTest {
FAKE_VOID_FUNC(send_non_periodic_msg_BMS_STARTUP,
               const struct CanMsgs_bms_startup_t *);
FAKE_VOID_FUNC(send_non_periodic_msg_BMS_WATCHDOG_TIMEOUT,
...
FAKE_VALUE_FUNC(EEPROM_StatusTypeDef, read_page, uint16_t, uint8_t, uint8_t *,
                uint16_t);
FAKE_VALUE_FUNC(EEPROM_StatusTypeDef, write_page, uint16_t, uint8_t, uint8_t *,
                uint16_t);
FAKE_VALUE_FUNC(EEPROM_StatusTypeDef, page_erase, uint16_t);

static float cell_voltages[ACCUMULATOR_NUM_SEGMENTS]
                          [ACCUMULATOR_NUM_SERIES_CELLS_PER_SEGMENT];

static bool read_cell_voltages(
    float voltages[ACCUMULATOR_NUM_SEGMENTS]
                  [ACCUMULATOR_NUM_SERIES_CELLS_PER_SEGMENT]) {
  for (uint8_t segment = 0; segment < ACCUMULATOR_NUM_SEGMENTS; segment++) {
    for (uint8_t cell = 0; cell < ACCUMULATOR_NUM_SERIES_CELLS_PER_SEGMENT;
         cell++) {
...
static void set_cell_voltage(AccumulatorSegment segment, uint8_t cell,
                             float voltage) {
  if (segment < ACCUMULATOR_NUM_SEGMENTS &&
      cell < ACCUMULATOR_NUM_SERIES_CELLS_PER_SEGMENT) {
...
class BmsFaultTest : public BaseStateMachineTest {
 protected:
  void SetUp() override {
    ...
    imd = App_Imd_Create(get_pwm_frequency, IMD_FREQUENCY_TOLERANCE,
                         get_pwm_duty_cycle, get_seconds_since_power_on);
    ...
    charger = App_Charger_Create(enable_charger, disable_charger,
                                 is_charger_connected, has_charger_faulted);

    bms_ok =
        App_OkStatus_Create(enable_bms_ok, disable_bms_ok, is_bms_ok_enabled);

    imd_ok =
        App_OkStatus_Create(enable_imd_ok, disable_imd_ok, is_imd_ok_enabled);

    bspd_ok = App_OkStatus_Create(enable_bspd_ok, disable_bspd_ok,
                                  is_bspd_ok_enabled);

    accumulator = App_Accumulator_Create(
        configure_cell_monitors, write_cfg_registers, start_voltage_conv,
        read_cell_voltages, start_temp_conv, read_cell_temperatures,
        get_min_temp_degc, get_max_temp_degc, get_avg_temp_degc,
        enable_discharge, disable_discharge);

    precharge_relay =
        App_PrechargeRelay_Create(enable_pre_charge, disable_pre_charge);

    ts = App_TractiveSystem_Create(get_ts_voltage, get_high_res_current,
                                   get_low_res_current);

    airs = App_Airs_Create(is_air_positive_closed, is_air_negative_closed,
                           close_air_positive, open_air_positive);

    clock = App_SharedClock_Create();

    eeprom = App_Eeprom_Create(write_page, read_page, page_erase);

    world = App_BmsWorld_Create(imd, heartbeat_monitor, rgb_led_sequence,
                                charger, bms_ok, imd_ok, bspd_ok, accumulator,
                                airs, precharge_relay, ts, clock, eeprom);
    ...
    // A temperature in [0.0, 60.0] degC to prevent other tests from entering
    // the fault state
  ...
  std::vector<const struct State *> GetAllStates(void) {
    return std::vector<const struct State *>{
        App_GetInitState(),   App_GetPreChargeState(), App_GetDriveState(),
        App_GetChargeState(), App_GetFaultState(),
    };
  }

  void CheckInRangeCanSignalsInGivenState(
      const State *state, float min_value, float max_value, float &fake_value,
  ...
  void UpdateSignals(struct StateMachine *state_machine,
                     uint32_t current_time_ms) override {
...
TEST_F(BmsFaultTest,
       check_state_transition_to_fault_state_from_all_states_overvoltage) {
  ...
  for (uint8_t segment = 0; segment < ACCUMULATOR_NUM_SEGMENTS; segment++) {
    for (uint8_t cell = 0; cell < ACCUMULATOR_NUM_SERIES_CELLS_PER_SEGMENT;
      ...
      LetTimePass(state_machine, 1000);
      ASSERT_NE(App_GetFaultState(),
                App_SharedStateMachine_GetCurrentState(state_machine));

      // Set cell voltage critically high and confirm fault is set
      set_cell_voltage((AccumulatorSegment)segment, cell,
                       MAX_CELL_VOLTAGE + 0.1f);
      LetTimePass(state_machine, 10);
      ASSERT_EQ(App_GetFaultState(),
                App_SharedStateMachine_GetCurrentState(state_machine));
      LetTimePass(state_machine, 1000);
      ASSERT_EQ(App_GetFaultState(),
                App_SharedStateMachine_GetCurrentState(state_machine));
...
TEST_F(BmsFaultTest,
       check_state_transition_to_fault_state_from_all_states_undervoltage) {
...
TEST_F(
    BmsFaultTest,
    check_state_transition_to_fault_state_from_all_states_overtemp_drive_state) {
  ...
  // In Discharge state, acceptible temp range is (-20, 60), should be
  // unaffected by temp of 46 C
...
TEST_F(
    BmsFaultTest,
    check_state_transition_to_fault_state_from_all_states_overtemp_charge_state) {
...
TEST_F(
    BmsFaultTest,
    check_state_transition_to_fault_state_from_all_states_undertemp_drive_state) {
  // Set TS current positive to trigger discharging condition in tempertature
  // check
  get_high_res_current_fake.return_val = 10.0f;
  get_low_res_current_fake.return_val = 10.0f;
...
TEST_F(
    BmsFaultTest,
    check_state_transition_to_fault_state_from_all_states_undertemp_charge_state) {
  // Set charger conditions such that charger faults do not trigger
  is_charger_connected_fake.return_val = true;
  is_air_negative_closed_fake.return_val = true;
  has_charger_faulted_fake.return_val = false;
...
TEST_F(
    BmsFaultTest,
    check_state_transition_to_fault_state_from_all_states_ts_discharge_overcurrent) {
...
TEST_F(
    BmsFaultTest,
    check_state_transition_to_fault_state_from_all_states_ts_charge_overcurrent) {
  // Set charger conditions such that charger faults do not trigger
  is_charger_connected_fake.return_val = true;
  is_air_negative_closed_fake.return_val = true;
  has_charger_faulted_fake.return_val = false;
...
TEST_F(BmsFaultTest, check_precharge_fault) {
  // Possible combinations of is_charger_connected, is_ts_rising_slowly and
  // is_ts_rising_quickly
  bool inputs[8][3] = {{false, false, false}, {false, true, false},
  ...
  for (int i = 0; i < 8; i++) {
    ASSERT_EQ(expected_output[i],
              App_PrechargeRelay_CheckFaults(precharge_relay, inputs[i][0],
