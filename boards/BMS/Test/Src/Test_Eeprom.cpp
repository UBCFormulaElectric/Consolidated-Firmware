...
// callback stores byte_array input into Io_Eeprom_WriteByte in
// static_byte_array to mimic writing to memory
static EEPROM_StatusTypeDef write_byte_callback(uint16_t page, uint8_t offset,
                                                uint8_t *byte_arr,
                                                uint16_t size) {
...
// callback stores copies stored static_byte_array into array pointed to in
// argument list of Io_Eeprom_ReadByte to mimic reading from memory
static EEPROM_StatusTypeDef read_byte_callback(uint16_t page, uint8_t offset,
                                               uint8_t *byte_arr,
                                               uint16_t size) {
  for (int i = 0; i < size; i++) {
...
TEST_F(BmsEepromTest, test_float_converted_to_bytes_full_page) {
  ...
  // Check that after converting floats to bytes and back their values are the
  // same
...
TEST_F(BmsEepromTest, test_size_error) {
  ...
  EEPROM_StatusTypeDef return_val_write =
      App_Eeprom_WriteFloats(eeprom, page, offset, input_data, num_floats);
  EEPROM_StatusTypeDef return_val_read =
      App_Eeprom_ReadFloats(eeprom, page, offset, output_data, num_floats);
...
TEST_F(BmsEepromTest, test_invalid_offset) {
  ...
  EEPROM_StatusTypeDef return_status =
      App_Eeprom_WriteFloats(eeprom, page, offset, input_data, num_floats);
