...
TEST_F(SocVotingTest, max_difference_underflow) {
  ASSERT_EQ(EXIT_CODE_INVALID_ARGS,
...
TEST_F(SocVotingTest, three_numbers_underflow) {
  ASSERT_EQ(
      ...
      App_Soc_Vote(0.0f, UNDERFLOW_SOC, UNDERFLOW_SOC, UNDERFLOW_SOC, &result));
}
...
TEST_F(SocVotingTest, difference_is_smaller_than_max_difference) {
  ASSERT_EQ(EXIT_CODE_OK,
            App_Soc_Vote(
                std::nextafter(TEST_VALUE, std::numeric_limits<float>::max()),
...
TEST_F(SocVotingTest, difference_is_equal_to_max_difference) {
  ASSERT_EQ(EXIT_CODE_OK, App_Soc_Vote(TEST_VALUE, 0.0f, TEST_VALUE,
                                       2 * TEST_VALUE, &result));
  ASSERT_EQ(TEST_VALUE / 2.0f, result);
}
...
TEST_F(SocVotingTest, difference_is_larger_than_max_difference) {
  ASSERT_EQ(EXIT_CODE_OK,
            App_Soc_Vote(std::nextafter(TEST_VALUE,
