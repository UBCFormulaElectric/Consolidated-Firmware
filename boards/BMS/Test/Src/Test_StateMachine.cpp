...
FAKE_VOID_FUNC(heartbeat_timeout_callback, enum HeartbeatOneHot,
...
TEST_F(
  ...
  for (auto &state : GetAllStates()) {
    SetInitialState(state);
    ImdTest::SetImdCondition(imd, IMD_NORMAL,
                             get_pwm_frequency_fake.return_val);
...
TEST_F(
  ...
  for (auto &state : GetAllStates()) {
    SetInitialState(state);
    ImdTest::SetImdCondition(imd, IMD_UNDERVOLTAGE_DETECTED,
                             get_pwm_frequency_fake.return_val);
    ...
    EXPECT_EQ(IMD_UNDERVOLTAGE_DETECTED,
              App_CanTx_BMS_ImdStatus_Condition_Get());
    EXPECT_EQ(true, App_CanTx_BMS_ImdStatus_ValidDutyCycle_Get());
...
// BMS-37
TEST_F(BmsStateMachineTest,
       check_bms_ok_is_broadcasted_over_can_in_all_states) {
  // Enable BMS_OK
...
// BMS-37
TEST_F(BmsStateMachineTest,
       check_imd_ok_is_broadcasted_over_can_in_all_states) {
  // Enable IMD_OK
...
// BMS-37
TEST_F(BmsStateMachineTest,
       check_bspd_ok_is_broadcasted_over_can_in_all_states) {
  // Enable BSPD_OK
...
TEST_F(BmsStateMachineTest, charger_disconnects_in_charge_state) {
  ...
  ASSERT_EQ(true, App_CanAlerts_GetFault(
                      BMS_FAULT_CHARGER_DISCONNECTED_DURING_CHARGE));
...
// BMS-30
TEST_F(BmsStateMachineTest,
       check_state_transition_from_fault_to_init_with_no_faults_set) {
...
TEST_F(
    BmsStateMachineTest,
    check_remains_in_fault_state_until_fault_cleared_then_transitions_to_init) {
  SetInitialState(App_GetInitState());
  ...
  // Check that state machine remains in fault state without cycling to init
