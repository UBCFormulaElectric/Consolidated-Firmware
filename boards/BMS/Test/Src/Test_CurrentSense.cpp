...
TEST_F(CurrentSenseTest, low_res_main_current_calculation)
{
    ...
    ASSERT_EQ(
        ...
        Io_CurrentSense_ConvertToLowResolutionMainCurrent(
            adc_voltage, &low_res_main_current));

    // Zero current
    adc_voltage = HSNBV_D06_OFFSET_VOLTAGE / voltage_ratio;
    ASSERT_EQ(
        EXIT_CODE_OK, Io_CurrentSense_ConvertToLowResolutionMainCurrent(
                          adc_voltage, &low_res_main_current));
    ...
    ASSERT_EQ(
        ...
        Io_CurrentSense_ConvertToLowResolutionMainCurrent(adc_voltage, NULL));
}
...
TEST_F(CurrentSenseTest, high_res_main_current_calculation)
{
    ...
    ASSERT_EQ(
        EXIT_CODE_OUT_OF_RANGE,
        Io_CurrentSense_ConvertToHighResolutionMainCurrent(
            adc_voltage, &high_res_main_current));

    // Zero current
    adc_voltage = HSNBV_D06_OFFSET_VOLTAGE / voltage_ratio;
    ASSERT_EQ(
        EXIT_CODE_OK, Io_CurrentSense_ConvertToHighResolutionMainCurrent(
                          adc_voltage, &high_res_main_current));
    ...
    ASSERT_EQ(
        EXIT_CODE_INVALID_ARGS,
        Io_CurrentSense_ConvertToHighResolutionMainCurrent(adc_voltage, NULL));
}
...
TEST_F(CurrentSenseTest, air_loop_current_calculation)
{
    ...
    ASSERT_EQ(
        EXIT_CODE_OUT_OF_RANGE, Io_CurrentSense_ConvertToAirLoopCurrent(
    ...
    ASSERT_EQ(
        EXIT_CODE_INVALID_ARGS,
        Io_CurrentSense_ConvertToAirLoopCurrent(adc_voltage, NULL));
}
