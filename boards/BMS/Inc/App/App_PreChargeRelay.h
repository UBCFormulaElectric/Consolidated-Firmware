 ...
 * caller
 */
struct PrechargeRelay *App_PrechargeRelay_Create(void (*close_relay)(void),
 ...
 */
uint8_t App_PrechargeRelay_GetFaultCounterVal(
    const struct PrechargeRelay *const precharge_relay);
 ...
 */
void App_PrechargeRelay_ResetFaultCounterVal(
 ...
 * @param is_ts_rising_quickly True if tractive system voltage rising quickly,
 * false otherwise
 * @param is_ts_rising_slowly True if tractive system voltage rising slowly,
 * false otherwise
 * @param precharge_limit_exceeded Acts as second return val, set to True if 3
 * precharge faults are detected
 * @return True if faults present, false otherwise
 */
bool App_PrechargeRelay_CheckFaults(struct PrechargeRelay *precharge_relay,
