 ...
 * @param write_page A function that can be called to get write to a page on the
 * eeprom
 * @param read_page A function that can be called to get read from a page on the
 * eeprom
 ...
 * @note SHOULD ONLY BE CALLED ONCE EVERY 5ms, DOING SO MORE QUICKLY WILL
 * VIOLATE EEPROM SPECS
 ...
 * must be 0 or multiple of 4 to align with 4-byte size of float values
 ...
 * @param addresses array of 4 addresses to be chosen from. Should all contain
 * same value but may not due to data-corruption in EEPROM
 * @return EEPROM_StatusTypeDef returns success status for debug
 */
EEPROM_StatusTypeDef App_Eeprom_Read4CopiesOfAddress(struct Eeprom *eeprom,
                                                     uint16_t page,
                                                     uint16_t *addresses);
