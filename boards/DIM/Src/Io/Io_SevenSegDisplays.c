...
// LEDs are controlled by clocking commands into an 8-bit shift register
// (TPIC6C596PWR), which drives the 7-segs
...
static uint8_t formula_e_commands[] = {DIGIT_F,  DIGIT_O,  DIGIT_R,
                                       DIGIT_M1, DIGIT_M2, DIGIT_U,
                                       DIGIT_L,  DIGIT_A,  DIGIT_E};
...
void Io_SevenSegDisplays_Init() {
  ...
  HAL_GPIO_WritePin(SEVENSEGS_DIMMING_GPIO_Port, SEVENSEGS_DIMMING_Pin,
                    GPIO_PIN_RESET);
}
...
void Io_SevenSegDisplays_WriteCommands(void) {
  ...
  for (int display = 0; display < TEMP_NUM_DISPLAYS; display++) {
    ...
    const uint8_t display_data =
        formula_e_commands[TEMP_NUM_DISPLAYS - display - 1];  // invert order

    for (int i = 0; i < SHIFT_REGISTER_SIZE; i++) {
      const bool bit_state = IS_BIT_SET(display_data, i);

      // Write bit state to data line
      HAL_GPIO_WritePin(SEVENSEGS_SEROUT_GPIO_Port, SEVENSEGS_SEROUT_Pin,
                        bit_state ? GPIO_PIN_SET : GPIO_PIN_RESET);

      // Pulse clock to shift data in
      HAL_GPIO_WritePin(SEVENSEGS_SRCK_GPIO_Port, SEVENSEGS_SRCK_Pin,
                        GPIO_PIN_SET);
      HAL_GPIO_WritePin(SEVENSEGS_SRCK_GPIO_Port, SEVENSEGS_SRCK_Pin,
                        GPIO_PIN_RESET);
    }
