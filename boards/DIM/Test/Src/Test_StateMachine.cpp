...
FAKE_VOID_FUNC(send_non_periodic_msg_DIM_STARTUP,
               const struct CanMsgs_dim_startup_t *);
FAKE_VOID_FUNC(send_non_periodic_msg_DIM_WATCHDOG_TIMEOUT,
 ...
 protected:
  void SetUp() override {
    ...
    bms_status_led = App_SharedRgbLed_Create(
        turn_bms_status_led_red, turn_bms_status_led_green,
        turn_bms_status_led_blue, turn_off_bms_status_led);

    dcm_status_led = App_SharedRgbLed_Create(
        turn_dcm_status_led_red, turn_dcm_status_led_green,
        turn_dcm_status_led_blue, turn_off_dcm_status_led);

    dim_status_led = App_SharedRgbLed_Create(
        turn_dim_status_led_red, turn_dim_status_led_green,
        turn_dim_status_led_blue, turn_off_dim_status_led);

    fsm_status_led = App_SharedRgbLed_Create(
        turn_fsm_status_led_red, turn_fsm_status_led_green,
        turn_fsm_status_led_blue, turn_off_fsm_status_led);
    ...
    world = App_DimWorld_Create(seven_seg_displays, heartbeat_monitor,
                                ...
                                bspd_led, start_switch, traction_control_switch,
...
//    // always be true.
//    //    App_SharedErrorTable_SetError(error_table, (enum ErrorId)(10),
//    true);
//
//    LetTimePass(state_machine, 10);
//
...
//    //    App_SharedErrorTable_SetError(error_table, (enum ErrorId)(10),
//    true);
//    //    App_SharedErrorTable_SetError(error_table, (enum ErrorId)(11),
//    true);
//
//    // When an error ID shows up on the 7-segment displays, it will have an
//    // offset of 500 added to it. This is why we are asserting for the value
//    // of 10 + 500 = 510 below. The same applies for the rest of this test
//    case. LetTimePass(state_machine, 999); ASSERT_EQ(true,
//    set_left_hex_digit_fake.arg0_val.enabled); ASSERT_EQ(true,
//    set_middle_hex_digit_fake.arg0_val.enabled); ASSERT_EQ(true,
//    set_right_hex_digit_fake.arg0_val.enabled); ASSERT_EQ(0,
//    set_left_hex_digit_fake.arg0_val.value); ASSERT_EQ(1,
//    set_middle_hex_digit_fake.arg0_val.value); ASSERT_EQ(5,
//    set_right_hex_digit_fake.arg0_val.value);
//
...
// DIM-4
TEST_F(DimStateMachineTest,
       check_aux_switch_is_broadcasted_over_can_in_drive_state) {
...
TEST_F(DimStateMachineTest, dcm_board_status_led_control_with_warning) {
  ...
  App_CanRx_DCM_Warnings_DCM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1HZ_Update(
      true);
  LetTimePass(state_machine, 10);
...
TEST_F(DimStateMachineTest, fsm_board_status_led_control_with_warning) {
  ...
  App_CanRx_FSM_Warnings_FSM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1HZ_Update(
      true);
  LetTimePass(state_machine, 10);
...
TEST_F(DimStateMachineTest, pdm_board_status_led_control_with_warning) {
  ...
  App_CanRx_PDM_Warnings_PDM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1HZ_Update(
      true);
  LetTimePass(state_machine, 10);
