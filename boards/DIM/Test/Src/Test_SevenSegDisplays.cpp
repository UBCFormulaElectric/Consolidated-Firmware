...
TEST_F(SevenSegDisplaysTest, set_one_hexadecimal_digit) {
  ...
  for (input[0] = 0; input[0] < NUM_HEX_DIGITS; input[0]++, count++) {
    ExitCode exit_code = App_SevenSegDisplays_SetHexDigits(seven_seg_displays,
                                                           input, num_inputs);
    ...
    ASSERT_EQ(input[LEFT_SEVEN_SEG_DISPLAY],
...
TEST_F(SevenSegDisplaysTest, set_valid_hex_digits_invokes_callback_function) {
  ...
  for (size_t i = 1; i <= NUM_SEVEN_SEG_DISPLAYS; i++) {
    ExitCode exit_code =
        App_SevenSegDisplays_SetHexDigits(seven_seg_displays, hex_digits, i);
    ASSERT_EQ(EXIT_CODE_OK, exit_code);
    ASSERT_EQ(i, display_value_callback_fake.call_count);
  }
}

TEST_F(SevenSegDisplaysTest,
       set_invalid_hex_digits_does_not_invoke_callback_function) {
  uint8_t hex_digits[NUM_SEVEN_SEG_DISPLAYS] = {NUM_HEX_DIGITS, NUM_HEX_DIGITS,
                                                NUM_HEX_DIGITS};
