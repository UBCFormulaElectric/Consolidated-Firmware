...
float look_up_value(const float speed_rpm, const float torque_request, const float bus_voltage, const float motor_temp, const uint8_t table_number)
{
	...
	for(size_t i = 0; i < LUT_NUM_COLUMNS - 2; i++)
	{
        ...
        if (abs_torque_request >= shaft_torque_lut_array[0][0][speed_index + 1][i] &&
                abs_torque_request < shaft_torque_lut_array[0][0][speed_index + 1][i + 1])
        {
            torque_index = i;
            torque_inter_percentage = ((abs_torque_request - shaft_torque_lut_array[0][0][speed_index + 1][i])) /
	...
	if(bus_voltage < 400.0f)
	{
        ...
        val1 = intp_from_lut(voltage_index, 0, torque_index, speed_index, speed_inter_percentage, torque_inter_percentage, table_number);
        val2 = val1;
	}
	else if(bus_voltage >= 400.0f && bus_voltage < 500.0f)
	{
        ...
        val1 = intp_from_lut(voltage_index-1, 0, torque_index, speed_index, speed_inter_percentage, torque_inter_percentage, table_number);
        val2 = intp_from_lut(voltage_index, 0, torque_index, speed_index, speed_inter_percentage, torque_inter_percentage, table_number);
...
float intp_from_lut(const uint8_t voltage_index, const uint8_t temp_index, uint8_t torque_index, uint8_t speed_index, const float speed_inter_percentage, const float torque_inter_percentage, const uint8_t table_number)
{
    ...
    {
        case ID_PEAK:
            lut_x1y1 = id_peak_lut_array[voltage_index][temp_index][speed_index][torque_index];
            lut_x2y1 = id_peak_lut_array[voltage_index][temp_index][speed_index][torque_index + 1];
            lut_x1y2 = id_peak_lut_array[voltage_index][temp_index][speed_index + 1][torque_index];
            lut_x2y2 = id_peak_lut_array[voltage_index][temp_index][speed_index + 1][torque_index + 1];
            break;
        case IQ_PEAK:
            lut_x1y1 = iq_peak_lut_array[voltage_index][temp_index][speed_index][torque_index];
            lut_x2y1 = iq_peak_lut_array[voltage_index][temp_index][speed_index][torque_index + 1];
            lut_x1y2 = iq_peak_lut_array[voltage_index][temp_index][speed_index + 1][torque_index];
            lut_x2y2 = iq_peak_lut_array[voltage_index][temp_index][speed_index + 1][torque_index + 1];
            break;
        default:
            lut_x1y1 = 0;
            lut_x2y1 = 0;
