...
float look_up_value(
    ...
    //Interpolate between 400V/500V tables
        ...
        value_from_table = fabsf((intp2_val - intp1_val)*voltage_inter_percentage) + fabsf(intp1_val);
    }
...
float get_value_from_lut(uint8_t voltage_index, uint8_t torque_index, uint8_t speed_index, uint8_t table_number)
{
    ...
    if(table_number == ID_PEAK)
    {
        value = (float)(id_peak_lut_times100[voltage_index][torque_index][speed_index])/100.0f;
    }
    else if(table_number == IQ_PEAK)
    {
        value = (float)(iq_peak_lut_times100[voltage_index][torque_index][speed_index])/100.0f;
    }
