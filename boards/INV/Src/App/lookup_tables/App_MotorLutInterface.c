...
{
    float intp1_val = 0.0f;
    float intp2_val = 0.0f;
    ...
    {
        intp1_val = (float)(id_peak_lut_times100[voltage_index_high][speed_index][torque_index])/100.0f;
        intp2_val = (float)(id_peak_lut_times100[voltage_index_low][speed_index][torque_index])/100.0f;
        intp2p1_val = (intp2_val - intp1_val)*voltage_inter_percentage + intp1_val;
    }
    else if(table_number == IQ_PEAK)
    {
        intp1_val = (float)(iq_peak_lut_times100[voltage_index_high][speed_index][torque_index])/100.0f;
        intp2_val = (float)(iq_peak_lut_times100[voltage_index_low][speed_index][torque_index])/100.0f;
        intp2p1_val = (intp2_val - intp1_val)*voltage_inter_percentage + intp1_val;
    }
    else if(table_number == IS_PEAK)
    {
        intp1_val = (float)(is_peak_lut_times100[voltage_index_high][speed_index][torque_index])/100.0f;
        intp2_val = (float)(is_peak_lut_times100[voltage_index_low][speed_index][torque_index])/100.0f;
        intp2p1_val = (intp2_val - intp1_val)*voltage_inter_percentage + intp1_val;
    }
