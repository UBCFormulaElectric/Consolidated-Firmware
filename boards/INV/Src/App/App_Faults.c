...
bool App_Faults_FaultedNonCritical(const struct InvCanTxInterface* const can_tx_interface)
{
    if(App_CanTx_GetPeriodicSignal_STACK_WATERMARK_ABOVE_THRESHOLD_TASK1_HZ(can_tx_interface) ||
            App_CanTx_GetPeriodicSignal_STACK_WATERMARK_ABOVE_THRESHOLD_TASK100_HZ(can_tx_interface) ||
            App_CanTx_GetPeriodicSignal_STACK_WATERMARK_ABOVE_THRESHOLD_TASK1_KHZ(can_tx_interface) ||
            App_CanTx_GetPeriodicSignal_STACK_WATERMARK_ABOVE_THRESHOLD_TASKCANRX(can_tx_interface) ||
            App_CanTx_GetPeriodicSignal_STACK_WATERMARK_ABOVE_THRESHOLD_TASKCANTX(can_tx_interface) ||
...
bool App_Faults_FaultedMotorShutdown(const struct InvCanTxInterface * const can_tx_interface)
{
    if (App_CanTx_GetPeriodicSignal_PHA_HI_DIAG(can_tx_interface) ||
        ...
        App_CanTx_GetPeriodicSignal_MC_CUR_LIM_OUT_OF_RANGE(can_tx_interface) ||
        App_CanTx_GetPeriodicSignal_MC_TORQUE_REQUEST_IMPLAUSIBLE(can_tx_interface) ||
        App_CanTx_GetPeriodicSignal_MC_ROTOR_POSITION_IMPLAUSIBLE(can_tx_interface) ||
        App_CanTx_GetPeriodicSignal_MC_PHC_CUR_CALC_IMPLAUSIBLE(can_tx_interface) ||
        App_CanTx_GetPeriodicSignal_MC_FUND_FREQ_REQ_IMPLAUSIBLE(can_tx_interface) ||
        App_CanTx_GetPeriodicSignal_MC_ID_CONTROLLER_STABILITY(can_tx_interface) ||
        App_CanTx_GetPeriodicSignal_MC_IQ_CONTROLLER_STABILITY(can_tx_interface) ||
        App_CanTx_GetPeriodicSignal_MC_SPEED_CONTROLLER_STABILITY(can_tx_interface))
    {
        return 1;
    }
    else
