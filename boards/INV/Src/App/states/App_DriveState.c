...
static void DriveStateRunOnEntry(struct StateMachine *const state_machine)
{
    ...
    if (fabsf(App_PowerStage_GetPosCurrentLimit() - MAX_INVERTER_CURRENT) < 0.5f ||
        fabsf(App_PowerStage_GetNegCurrentLimit() + MAX_INVERTER_CURRENT) < 0.5f)
    {
        //TODO fault current limit out of range here
    }

    //Configure gate drive (STGAPs)
...
static void DriveStateRunOnTick100Hz(struct StateMachine *const state_machine)
{
    ...
    if(App_CanRx_INV_STATE_REQ_GetSignal_STATE_REQ(can_rx_interface) == CANMSGS_INV_STATE_MACHINE_STATE_STANDBY_CHOICE)
    {
