...
dqs_values generateRefCurrents(
		const dqs_values * const dqs_ref_currents, const double omega, const double vdc_sensor_val, bool * const fw_flag
		)
{
	...
	{
		*fw_flag = 1;
		new_dqs_ref_currents.d = (-lamda * D_INDUCTANCE +
		...
		new_dqs_ref_currents.q = (dqs_ref_currents->s * dqs_ref_currents->s - dqs_ref_currents->d * dqs_ref_currents->d > 0) ? sign_is * sqrt(dqs_ref_currents->s * dqs_ref_currents->s - dqs_ref_currents->d * dqs_ref_currents->d) : 0;
	}
	else
	{
		*fw_flag = 0;
