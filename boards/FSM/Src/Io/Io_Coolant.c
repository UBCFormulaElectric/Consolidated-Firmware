...
// source:
// https://www.adafruit.com/product/828#:~:text=7.5%20*%20Flow%20rate%20(L/min)
...
void Io_FlowMeter_Init(TIM_HandleTypeDef *htim) {
  ...
  flow_meter = Io_SharedFreqOnlyPwmInput_Create(
      htim, TIMx_FREQUENCY / TIM8_PRESCALER, TIM_CHANNEL_1,
      TIM8_AUTO_RELOAD_REG, HAL_TIM_ACTIVE_CHANNEL_1);
}
float Io_FlowMeter_GetFlowRate(void) {
  return Io_SharedFreqOnlyPwmInput_GetFrequency(flow_meter) /
         FLOW_RATE_CONVERSION_FACTOR;
}
void Io_FlowMeter_InputCaptureCallback(TIM_HandleTypeDef *htim) {
  if (htim == Io_SharedFreqOnlyPwmInput_GetTimerHandle(flow_meter) &&
      htim->Channel ==
          Io_SharedFreqOnlyPwmInput_GetTimerActiveChannel(flow_meter)) {
