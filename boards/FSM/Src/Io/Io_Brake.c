...
bool Io_Brake_IsActuated(void) {
  return HAL_GPIO_ReadPin(BSPD_BRAKE_PRESSED_5V_GPIO_Port,
                          BSPD_BRAKE_PRESSED_5V_Pin) == GPIO_PIN_SET;
}
bool Io_Brake_PressureVoltageAlarm(float pressure_voltage) {
  return !(BRAKE_PRESSURE_OC_THRESHOLD_V <= pressure_voltage &&
           pressure_voltage <= BRAKE_PRESSURE_SC_THRESHOLD_V);
}
...
float Io_Brake_GetPedalPercentTravel(void) {
  ...
  const float percent_per_volt =
      100 / (BRAKE_PEDAL_MAX_VOLTAGE - BRAKE_PEDAL_MIN_VOLTAGE);
  return (pedal_voltage - BRAKE_PEDAL_MIN_VOLTAGE) * percent_per_volt;
}
bool Io_Brake_PedalSensorOCSC(void) {
  return !(BRAKE_PEDAL_MIN_VOLTAGE <= Io_Brake_GetPedalSensorVoltage() &&
