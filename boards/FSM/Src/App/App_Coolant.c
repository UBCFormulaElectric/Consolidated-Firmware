...
struct Coolant *App_Coolant_Create(float (*get_flow_rate)(void),
                                   float (*get_temperature_A)(void),
                                   float (*get_temperature_B)(void),
                                   float (*get_pressure_A)(void),
                                   float (*get_pressure_B)(void)) {
  ...
  coolant->flow_rate_in_range_check = App_InRangeCheck_Create(
      get_flow_rate, MIN_FLOW_RATE_L_PER_MIN, MAX_FLOW_RATE_L_PER_MIN);
  // Temperature
  ...
  coolant->flow_in_range_signal = App_SharedSignal_Create(
      FLOW_METER_TIME_TO_FAULT, FLOW_METER_TIME_TO_CLEAR);
...
void App_Coolant_Broadcast(const struct FsmWorld *world) {
  ...
  enum InRangeCheck_Status coolant_status = App_InRangeCheck_GetValue(
      coolant->flow_rate_in_range_check, &coolant_flow_clamped);
  App_CanTx_FSM_Coolant_FlowRate_Set(coolant_flow_clamped);
  App_CanAlerts_SetWarning(FSM_WARNING_FLOW_RATE_OUT_OF_RANGE,
                           coolant_status != VALUE_IN_RANGE);
  ...
  SignalState flow_in_range_signal_state =
      App_SharedSignal_Update(coolant->flow_in_range_signal,
                              flow_rate_inRangeCheck_status == VALUE_UNDERFLOW,
                              flow_rate_inRangeCheck_status == VALUE_IN_RANGE);
  App_CanAlerts_SetFault(FSM_FAULT_FLOW_METER_HAS_UNDERFLOW,
