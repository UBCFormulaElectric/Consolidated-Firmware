...
struct Brake *App_Brake_Create(float (*get_front_pressure_psi)(void),
  ...
  brake->front_pressure_in_range_check = App_InRangeCheck_Create(
      get_front_pressure_psi, MIN_BRAKE_PRESSURE_PSI, MAX_BRAKE_PRESSURE_PSI);
  brake->rear_pressure_in_range_check = App_InRangeCheck_Create(
      get_rear_pressure_psi, MIN_BRAKE_PRESSURE_PSI, MAX_BRAKE_PRESSURE_PSI);
...
bool App_Brake_PressureElectricalFault(const struct Brake *brake) {
  return brake->front_pressure_sensor_ocsc() ||
         brake->rear_pressure_sensor_ocsc();
}
...
void App_Brake_Broadcast(const struct FsmWorld *world) {
  ...
  enum InRangeCheck_Status front_pressure_status = App_InRangeCheck_GetValue(
      brake->front_pressure_in_range_check, &front_pressure);
  App_CanTx_FSM_Brake_FrontBrakePressure_Set((uint32_t)front_pressure);
  App_CanAlerts_SetWarning(FSM_WARNING_FRONT_BRAKE_PRESSURE_OUT_OF_RANGE,
                           front_pressure_status != VALUE_IN_RANGE);

  float rear_pressure;
  enum InRangeCheck_Status rear_pressure_status = App_InRangeCheck_GetValue(
      brake->rear_pressure_in_range_check, &rear_pressure);
  App_CanTx_FSM_Brake_RearBrakePressure_Set((uint32_t)rear_pressure);
  App_CanAlerts_SetWarning(FSM_WARNING_REAR_BRAKE_PRESSURE_OUT_OF_RANGE,
                           rear_pressure_status != VALUE_IN_RANGE);

  App_CanTx_FSM_Brake_PressureSensorOpenShortCircuit_Set(
      App_Brake_PressureElectricalFault(brake));
  App_CanTx_FSM_Brake_PedalOpenShortCircuit_Set(
      brake->pedal_travel_sensor_ocsc());
