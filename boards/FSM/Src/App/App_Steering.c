...
struct Steering *App_Steering_Create(float (*get_steering_angle)(void),
  ...
  steering->steering_angle_in_range_check = App_InRangeCheck_Create(
...
void App_Steering_Broadcast(const struct FsmWorld *world) {
  ...
  App_CanAlerts_SetWarning(FSM_WARNING_STEERING_ANGLE_SENSOR_OCSC,
                           steering_sensor_ocsc);
  if (steering_sensor_ocsc) {
    App_CanTx_FSM_Steering_SteeringAngle_Set(0);
  }

  float steering_angle;
  enum InRangeCheck_Status steering_in_range = App_InRangeCheck_GetValue(
      steering->steering_angle_in_range_check, &steering_angle);
  App_CanTx_FSM_Steering_SteeringAngle_Set(steering_angle);
  App_CanAlerts_SetWarning(FSM_WARNING_STEERING_ANGLE_OUT_OF_RANGE,
                           steering_in_range != VALUE_IN_RANGE);
}
