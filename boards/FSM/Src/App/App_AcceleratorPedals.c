...
PERCENT_DEFLECTION + (float)encoder_unpressed_value;

    float high_count_deadzone_threshold =
        (float)(encoder_fully_pressed_value - encoder_unpressed_value) * (1.0f -
PERCENT_DEFLECTION) + (float)encoder_unpressed_value;
    ...
    return MAX_ACCELERATOR_PEDAL_PRESS * ((float)encoder_counter_value -
low_count_deadzone_threshold) / (high_count_deadzone_threshold -
...
PERCENT_DEFLECTION +
   ...
   // float high_count_deadzone_threshold =
   //    (float)(encoder_unpressed_value - encoder_fully_pressed_value) * (1.0f
- PERCENT_DEFLECTION) +
   //    (float)encoder_fully_pressed_value;
   ...
   //// (high end deadzone threshold - low end deadzone threshold)
   // return MAX_ACCELERATOR_PEDAL_PRESS * (high_count_deadzone_threshold -
(float)encoder_counter_value) /
...
struct AcceleratorPedals *App_AcceleratorPedals_Create(
  ...
  accelerator_pedals->papp_alarm_signal = App_SharedSignal_Create(
      PAPPS_OCSC_TIME_TO_FAULT, PAPPS_OCSC_TIME_TO_CLEAR);
  accelerator_pedals->sapp_alarm_signal = App_SharedSignal_Create(
      SAPPS_OCSC_TIME_TO_FAULT, SAPPS_OCSC_TIME_TO_CLEAR);
  accelerator_pedals->app_brake_signal =
      App_SharedSignal_Create(APP_BRAKE_TIME_TO_FAULT, APP_BRAKE_TIME_TO_CLEAR);
...
void App_AcceleratorPedals_Broadcast(const struct FsmWorld *world) {
  ...
  float papps_pedal_percentage =
      accelerator_pedals->get_primary_pedal_percent();
  float sapps_pedal_percentage =
      accelerator_pedals->get_secondary_pedal_percent();
  ...
  SignalState papp_signal_state =
      App_SharedSignal_Update(accelerator_pedals->papp_alarm_signal,
                              primary_pedal_ocsc, !primary_pedal_ocsc);
  ...
  SignalState sapp_signal_state =
      App_SharedSignal_Update(accelerator_pedals->sapp_alarm_signal,
                              secondary_pedal_ocsc, !secondary_pedal_ocsc);
  ...
  SignalState app_agreement_signal_state = App_SharedSignal_Update(
      accelerator_pedals->app_agreement_signal, (papp_sapp_diff) > 10.f,
      (papp_sapp_diff) <= 10.f);
  const bool apps_disagreement =
  ...
  SignalState app_brake_disagreement = App_SharedSignal_Update(
      accelerator_pedals->app_brake_signal,
      App_Brake_IsBrakeActuated(brake) &&
          accelerator_pedals->get_primary_pedal_percent() > 25,
      accelerator_pedals->get_primary_pedal_percent() < 5);
  const bool brake_acc_disagreement =
      app_brake_disagreement == SIGNAL_STATE_ACTIVE;
  App_CanAlerts_SetWarning(FSM_WARNING_BRAKE_ACC_DISAGREEMENT,
                           brake_acc_disagreement);
