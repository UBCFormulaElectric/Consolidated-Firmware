...
void App_AllStatesRunOnTick100Hz(struct StateMachine *const state_machine) {
  ...
  // TODO: JSONCAN
  // float left_torque_req =
  // (float)App_CanMsgs_dcm_invl_command_message_torque_command_invl_decode(
  //     App_CanRx_DCM_INVL_COMMAND_MESSAGE_GetSignal_TORQUE_COMMAND_INVL(can_rx));

  // float right_torque_req =
  // (float)App_CanMsgs_dcm_invr_command_message_torque_command_invr_decode(
  ...
  fsm_torque_limit = 0.01f *
                         App_CanTx_FSM_Apps_PappsMappedPedalPercentage_Get() *
