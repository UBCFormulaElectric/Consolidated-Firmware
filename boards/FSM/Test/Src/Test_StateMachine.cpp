  ...
  protected:
        ...
        uint8_t overflow_choice)
    {
        for (const auto &state : GetAllStates())
        {
            ...
            ASSERT_EQ(ok_choice, out_of_range_can_signal_getter());

            // Underflow range
            fake_value = std::nextafter(min_value, std::numeric_limits<float>::lowest());
            LetTimePass(fsm_state_machine, 10);
            ASSERT_EQ(underflow_choice, out_of_range_can_signal_getter());

            // Overflow range
            fake_value = std::nextafter(max_value, std::numeric_limits<float>::max());
            LetTimePass(fsm_state_machine, 10);
            ASSERT_EQ(overflow_choice, out_of_range_can_signal_getter());
        }
    }

    void CheckBinaryStatusCanSignalInAllStates(
        bool &fake_value,
        uint8_t (*can_signal_getter)(),
        uint8_t on_choice,
        uint8_t off_choice)
    {
        for (const auto &state : GetAllStates())
        {
            ...
            fake_value = true;
            LetTimePass(fsm_state_machine, 10);
            ASSERT_EQ(on_choice, can_signal_getter());

            fake_value = false;
            LetTimePass(fsm_state_machine, 10);
            ASSERT_EQ(off_choice, can_signal_getter());
        }
    }
    ...
    papps_alarm_error_sets_mapped_pedal_percentage_to_zero_and_sets_motor_shutdown_can_tx_signal)
{
    ...
    ASSERT_EQ(PAPPS_OCSC_IS_ACTIVE_FALSE_CHOICE, App_CanTx_FSM_MotorShutdownErrors_PappsOCSCIsActive_Get());
    ...
    ASSERT_EQ(PAPPS_OCSC_IS_ACTIVE_TRUE_CHOICE, App_CanTx_FSM_MotorShutdownErrors_PappsOCSCIsActive_Get());
}
// FSM-5, FSM-16
TEST_F(
    FsmStateMachineTest,
    sapps_alarm_error_sets_mapped_pedal_percentage_to_zero_and_sets_motor_shutdown_can_tx_signal)
{
    ...
    ASSERT_EQ(SAPPS_OCSC_IS_ACTIVE_FALSE_CHOICE, App_CanTx_FSM_MotorShutdownErrors_SappsOCSCIsActive_Get());
    ASSERT_NEAR(50, App_CanTx_FSM_Apps_PappsMappedPedalPercentage_Get(), 0.5f);

    // after
    LetTimePass(fsm_state_machine, 1);
    ASSERT_EQ(SAPPS_OCSC_IS_ACTIVE_TRUE_CHOICE, App_CanTx_FSM_MotorShutdownErrors_SappsOCSCIsActive_Get());
    ASSERT_FLOAT_EQ(0, App_CanTx_FSM_Apps_PappsMappedPedalPercentage_Get());
}
...
TEST_F(FsmStateMachineTest, papp_and_sapp_disagreement_error_on_can_and_torque_0)
{
    ...
    ASSERT_EQ(App_CanTx_FSM_MotorShutdownErrors_AppsDisagreementIsActive_Get(), APPS_HAS_DISAGREEMENT_FALSE_CHOICE);

    LetTimePass(fsm_state_machine, 1);
    ASSERT_EQ(0, App_CanTx_FSM_Apps_PappsMappedPedalPercentage_Get());
    ASSERT_EQ(App_CanTx_FSM_MotorShutdownErrors_AppsDisagreementIsActive_Get(), APPS_HAS_DISAGREEMENT_TRUE_CHOICE);
}
// FSM-3
TEST_F(FsmStateMachineTest, brake_is_actuated_sets_mapped_pedal_percentage_to_zero_in_drive_state)
{
    ...
    ASSERT_EQ(App_CanTx_FSM_Errors_BrakeAccDisagreement_Get(), BRAKE_ACC_DISAGREEMENT_FALSE_CHOICE);
    ...
    ASSERT_EQ(App_CanTx_FSM_Errors_BrakeAccDisagreement_Get(), BRAKE_ACC_DISAGREEMENT_TRUE_CHOICE);
...
TEST_F(FsmStateMachineTest, primary_flow_rate_underflow_sets_motor_shutdown_can_tx_signal)
{
    ...
    ASSERT_EQ(FLOW_METER_HAS_UNDERFLOW_FALSE_CHOICE, App_CanTx_FSM_MotorShutdownErrors_FlowMeterHasUnderflow_Get());

    LetTimePass(fsm_state_machine, 1);
    ASSERT_EQ(FLOW_METER_HAS_UNDERFLOW_TRUE_CHOICE, App_CanTx_FSM_MotorShutdownErrors_FlowMeterHasUnderflow_Get());
...
TEST_F(FsmStateMachineTest, steering_sensor_OCSC)
{
    ...
    LetTimePass(fsm_state_machine, 10);
    ASSERT_EQ(App_CanTx_FSM_Errors_SteeringAngleSensorOCSC_Get(), STEERING_SENSOR_OCSC_FALSE_CHOICE);

    steering_OCSC_fake.return_val = true;
    LetTimePass(fsm_state_machine, 10);
    ASSERT_EQ(App_CanTx_FSM_Errors_SteeringAngleSensorOCSC_Get(), STEERING_SENSOR_OCSC_TRUE_CHOICE);
}
...
TEST_F(FsmStateMachineTest, check_brake_can_signals_in_all_states)
{
    ...
    CheckBinaryStatusCanSignalInAllStates(
        ...
        BRAKE_IS_ACTUATED_TRUE_CHOICE, BRAKE_IS_ACTUATED_FALSE_CHOICE);
    ...
    CheckBinaryStatusCanSignalInAllStates(
        ...
        PRESSURE_SENSOR_IS_OPEN_OR_SHORT_CIRCUIT_TRUE_CHOICE, PRESSURE_SENSOR_IS_OPEN_OR_SHORT_CIRCUIT_FALSE_CHOICE);
    CheckBinaryStatusCanSignalInAllStates(
...
TEST_F(FsmStateMachineTest, check_primary_flow_rate_can_signals_in_all_states)
{
    CheckInRangeCanSignalsInAllStates(
        ...
        App_CanTx_FSM_Coolant_FlowRate_Get, (uint8_t(*)(void))App_CanTx_FSM_Errors_FlowRateOutOfRange_Get,
        FLOW_RATE_OUT_OF_RANGE_OK_CHOICE, FLOW_RATE_OUT_OF_RANGE_UNDERFLOW_CHOICE,
        FLOW_RATE_OUT_OF_RANGE_OVERFLOW_CHOICE);
}
...
TEST_F(FsmStateMachineTest, check_steering_angle_can_signals_in_all_states)
{
    CheckInRangeCanSignalsInAllStates(
        ...
        App_CanTx_FSM_Steering_SteeringAngle_Get, (uint8_t(*)(void))App_CanTx_FSM_Errors_SteeringAngleOutOfRange_Get,
        STEERING_ANGLE_OUT_OF_RANGE_OK_CHOICE, STEERING_ANGLE_OUT_OF_RANGE_WIDELEFT_CHOICE,
        STEERING_ANGLE_OUT_OF_RANGE_WIDERIGHT_CHOICE);
}
// FSM-9
TEST_F(FsmStateMachineTest, check_left_wheel_speed_can_signals_in_all_states)
{
    CheckInRangeCanSignalsInAllStates(
        ...
        App_CanTx_FSM_Wheels_LeftWheelSpeed_Get, (uint8_t(*)(void))(App_CanTx_FSM_Errors_LeftWheelSpeedOutOfRange_Get),
        LEFT_WHEEL_SPEED_OUT_OF_RANGE_OK_CHOICE, LEFT_WHEEL_SPEED_OUT_OF_RANGE_UNDERSPEED_CHOICE,
        LEFT_WHEEL_SPEED_OUT_OF_RANGE_OVERSPEED_CHOICE);
}
// FSM-9
TEST_F(FsmStateMachineTest, check_right_wheel_speed_can_signals_in_all_states)
{
    CheckInRangeCanSignalsInAllStates(
        ...
        App_CanTx_FSM_Wheels_RightWheelSpeed_Get, (uint8_t(*)(void))App_CanTx_FSM_Errors_RightWheelSpeedOutOfRange_Get,
        RIGHT_WHEEL_SPEED_OUT_OF_RANGE_OK_CHOICE, RIGHT_WHEEL_SPEED_OUT_OF_RANGE_UNDERSPEED_CHOICE,
        RIGHT_WHEEL_SPEED_OUT_OF_RANGE_OVERSPEED_CHOICE);
}
