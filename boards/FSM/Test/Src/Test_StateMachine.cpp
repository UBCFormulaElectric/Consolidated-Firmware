...
FAKE_VOID_FUNC(send_non_periodic_msg_FSM_STARTUP,
               const struct CanMsgs_fsm_startup_t *)
FAKE_VOID_FUNC(send_non_periodic_msg_FSM_AIR_SHUTDOWN,
               const struct CanMsgs_fsm_air_shutdown_t *)
FAKE_VOID_FUNC(send_non_periodic_msg_FSM_WATCHDOG_TIMEOUT,
...
class FsmStateMachineTest : public BaseStateMachineTest {
 protected:
  void SetUp() override {
    ...
    papps_and_sapps = App_AcceleratorPedals_Create(get_papps, papps_OCSC,
                                                   get_sapps, sapps_OCSC);
    brake = App_Brake_Create(brake_get_front_pressure,
                             brake_front_pressure_OCSC, brake_get_rear_pressure,
                             brake_rear_pressure_OCSC, brake_get_pedal_travel,
                             brake_pedal_OCSC, is_brake_actuated);
    coolant = App_Coolant_Create(coolant_get_flow_rate, coolant_get_temp_a,
                                 coolant_get_temp_b, coolant_get_pressure_a,
    ...
    world = App_FsmWorld_Create(heartbeat_monitor, papps_and_sapps, brake,
                                coolant, steering, wheels);
  ...
  void SetInitialState(const struct State *const initial_state) {
    ...
    ASSERT_EQ(initial_state,
              App_SharedStateMachine_GetCurrentState(fsm_state_machine));
  }
  static std::vector<const struct State *> GetAllStates() {
    return std::vector<const struct State *>{App_GetDriveState(),
                                             App_GetFaultState()};
  }

  template <typename T>
  void CheckInRangeCanSignalsInAllStates(float min_value, float max_value,
                                         float &fake_value,
  ...
  void CheckBinaryStatusCanSignalInAllStates(bool &fake_value,
...
TEST_F(FsmStateMachineTest,
       papp_and_sapp_disagreement_error_on_can_and_torque_0) {
  get_papps_fake.return_val = 50;
...
// FSM-3
TEST_F(FsmStateMachineTest,
       brake_is_actuated_sets_mapped_pedal_percentage_to_zero_in_drive_state) {
  RESET_FAKE(is_brake_actuated)
  ...
  LetTimePass(fsm_state_machine, 10);
  ASSERT_NEAR(10,
              std::round(App_CanTx_FSM_Apps_PappsMappedPedalPercentage_Get()),
              0.5f);
...
TEST_F(FsmStateMachineTest,
  ...
  coolant_get_flow_rate_fake.return_val =
      std::nextafter(flow_rate_threshold, std::numeric_limits<float>::lowest());
  LetTimePass(fsm_state_machine, 10);
...
TEST_F(FsmStateMachineTest, steering_sensor_OCSC) {
  ...
  LetTimePass(fsm_state_machine, 10);
  ASSERT_FALSE(
      App_CanAlerts_GetWarning(FSM_WARNING_STEERING_ANGLE_SENSOR_OCSC));
...
// FSM-3, FSM-4
TEST_F(FsmStateMachineTest,
       check_mapped_pedal_percentage_can_signals_in_drive_state) {
  ...
  GetPrimaryEncoderCounterFromPedalPercentage(0) - 1;
  get_sapps_encoder_counter_fake.return_val =
  GetSecondaryEncoderCounterFromPedalPercentage(0) + 1;
  ...
  get_papps_encoder_counter_fake.return_val =
  GetPrimaryEncoderCounterFromPedalPercentage(100) + 1;
  get_sapps_encoder_counter_fake.return_val =
  GetSecondaryEncoderCounterFromPedalPercentage(100) - 1;
  LetTimePass(state_machine, 10);
...
TEST_F(FsmStateMachineTest, check_brake_can_signals_in_all_states) {
  ...
  CheckInRangeCanSignalsInAllStates(
      ...
      brake_get_front_pressure_fake.return_val,
      App_CanTx_FSM_Brake_FrontBrakePressure_Get,
      (bool (*)(uint8_t))App_CanAlerts_GetWarning,
      FSM_WARNING_FRONT_BRAKE_PRESSURE_OUT_OF_RANGE);
  CheckInRangeCanSignalsInAllStates(
      MIN_BRAKE_PRESSURE_PSI, MAX_BRAKE_PRESSURE_PSI,
      brake_get_rear_pressure_fake.return_val,
  ...
  CheckBinaryStatusCanSignalInAllStates(
      ...
      (uint8_t(*)(void))App_CanTx_FSM_Brake_IsActuated_Get);
...
TEST_F(FsmStateMachineTest, check_primary_flow_rate_can_signals_in_all_states) {
  CheckInRangeCanSignalsInAllStates(
      MIN_FLOW_RATE_L_PER_MIN, MAX_FLOW_RATE_L_PER_MIN,
      coolant_get_flow_rate_fake.return_val, App_CanTx_FSM_Coolant_FlowRate_Get,
      (bool (*)(uint8_t))App_CanAlerts_GetWarning,
      FSM_WARNING_FLOW_RATE_OUT_OF_RANGE);
}

TEST_F(FsmStateMachineTest,
       check_coolant_pressure_temperature_can_signals_in_all_states) {
...
TEST_F(FsmStateMachineTest, check_steering_angle_can_signals_in_all_states) {
  CheckInRangeCanSignalsInAllStates(MIN_STEERING_ANGLE_DEG,
                                    MAX_STEERING_ANGLE_DEG,
                                    get_steering_angle_fake.return_val,
                                    App_CanTx_FSM_Steering_SteeringAngle_Get,
                                    (bool (*)(uint8_t))App_CanAlerts_GetWarning,
                                    FSM_WARNING_STEERING_ANGLE_OUT_OF_RANGE);
}
// FSM-9
TEST_F(FsmStateMachineTest, check_left_wheel_speed_can_signals_in_all_states) {
  CheckInRangeCanSignalsInAllStates(MIN_LEFT_WHEEL_SPEED_KPH,
                                    MAX_LEFT_WHEEL_SPEED_KPH,
                                    wheel_get_left_speed_fake.return_val,
...
TEST_F(FsmStateMachineTest, check_right_wheel_speed_can_signals_in_all_states) {
  CheckInRangeCanSignalsInAllStates(MIN_RIGHT_WHEEL_SPEED_KPH,
                                    MAX_RIGHT_WHEEL_SPEED_KPH,
                                    wheel_get_right_speed_fake.return_val,
