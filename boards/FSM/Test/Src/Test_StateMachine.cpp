  ...
  protected:
        ...
        uint8_t   false_choice)
    {
             ...
             pedal_percentage <= (size_t)MAX_ACCELERATOR_PEDAL_PRESS; pedal_percentage++)
        {
            ...
            // TODO: JSONCAN -> ASSERT_EQ(false_choice, App_CanTx_GetPeriodicSignal_APPS_HAS_DISAGREEMENT(can_tx_interface));

            LetTimePass(state_machine, 1);
            // TODO: JSONCAN -> ASSERT_EQ(true_choice, App_CanTx_GetPeriodicSignal_APPS_HAS_DISAGREEMENT(can_tx_interface));
        }
    }
...
TEST_F(FsmStateMachineTest, check_air_open_state_is_broadcasted_over_can)
{
    ...
    // TODO: JSONCAN -> EXPECT_EQ(CANMSGS_FSM_STATE_MACHINE_STATE_AIR_OPEN_CHOICE, App_CanTx_GetPeriodicSignal_STATE(can_tx_interface));
}
...
TEST_F(FsmStateMachineTest, check_air_closed_state_is_broadcasted_over_can)
{
    ...
    // TODO: JSONCAN -> EXPECT_EQ(CANMSGS_FSM_STATE_MACHINE_STATE_AIR_CLOSED_CHOICE, App_CanTx_GetPeriodicSignal_STATE(can_tx_interface));
}
...
TEST_F(FsmStateMachineTest, brake_is_actuated_sets_mapped_pedal_percentage_to_zero_in_all_states)
{
    for (const auto &state : GetAllStates())
    {
        ...
        LetTimePass(state_machine, 10);
        // TODO: JSONCAN -> ASSERT_NEAR(5, round(App_CanTx_GetPeriodicSignal_MAPPED_PEDAL_PERCENTAGE(can_tx_interface)), 0.5f);
