  ...
  protected:
    ...
    uint32_t GetPrimaryEncoderCounterFromPedalPercentage(float pedal_percentage)
    {
        return GetEncoderCounterFromPedalPercentage(
            pedal_percentage, PAPPS_ENCODER_FULLY_PRESSED_VALUE,
            PAPPS_ENCODER_UNPRESSED_VALUE, false);
    }

    uint32_t
        GetSecondaryEncoderCounterFromPedalPercentage(float pedal_percentage)
    {
        return GetEncoderCounterFromPedalPercentage(
            pedal_percentage, SAPPS_ENCODER_FULLY_PRESSED_VALUE,
            SAPPS_ENCODER_UNPRESSED_VALUE, true);
    }
        ...
        uint8_t   false_choice)
    {
             ...
             pedal_percentage++)
        {
            ...
            fake_larger_pedal_percentage_encoder_value =
                larger_pedal_percentage_unpressed_value;
            fake_smaller_pedal_percentage_encoder_value =
                smaller_pedal_percentage_unpressed_value;
            LetTimePass(state_machine, exit_high_ms);

            fake_larger_pedal_percentage_encoder_value =
                GetEncoderCounterFromPedalPercentage(
                    pedal_percentage, larger_pedal_percentage_max_pressed_value,
                    larger_pedal_percentage_unpressed_value,
            ...
            ASSERT_EQ(
                true_choice, App_CanTx_GetPeriodicSignal_APPS_HAS_DISAGREEMENT(
                                 can_tx_interface));
        }
    }
    ...
    check_mapped_pedal_percentage_can_signals_in_all_states)
{
    for (const auto &state : GetAllStates())
    {
        ...
        get_papps_encoder_counter_fake.return_val =
            GetPrimaryEncoderCounterFromPedalPercentage(50);
        get_sapps_encoder_counter_fake.return_val =
            GetSecondaryEncoderCounterFromPedalPercentage(50);

        LetTimePass(state_machine, 10);
        ASSERT_NEAR(
            50,
            App_CanTx_GetPeriodicSignal_MAPPED_PEDAL_PERCENTAGE(
                can_tx_interface),
            0.5f);
...
TEST_F(
    FsmStateMachineTest,
    brake_is_actuated_sets_mapped_pedal_percentage_to_zero_in_all_states)
{
    for (const auto &state : GetAllStates())
    {
        SetInitialState(state);
        ...
        get_papps_encoder_counter_fake.return_val =
            GetPrimaryEncoderCounterFromPedalPercentage(5);
        LetTimePass(state_machine, 10);
        ASSERT_NEAR(
            5,
            round(App_CanTx_GetPeriodicSignal_MAPPED_PEDAL_PERCENTAGE(
                can_tx_interface)),
            0.5f);
    ...
    papps_greater_than_apps_by_ten_percent_sets_motor_shutdown_can_tx_signal)
{
    CheckIfAppsDisagreementFaultIsSetWhenAppsHasDisagreement(
        get_papps_encoder_counter_fake.return_val,
        get_sapps_encoder_counter_fake.return_val,
        PAPPS_ENCODER_FULLY_PRESSED_VALUE, PAPPS_ENCODER_UNPRESSED_VALUE, false,
        SAPPS_ENCODER_FULLY_PRESSED_VALUE, SAPPS_ENCODER_UNPRESSED_VALUE, true,
        APPS_ENTRY_HIGH_MS, APPS_EXIT_HIGH_MS,
    ...
    apps_is_greater_than_25_percent_and_brake_is_not_actuated_does_not_set_motor_shutdown_can_tx_signal)
{
    const float fake_encoder_value_threshold =
        GetPrimaryEncoderCounterFromPedalPercentage(25);
    ...
    get_sapps_encoder_counter_fake.return_val =
        GetSecondaryEncoderCounterFromPedalPercentage(25);
    LetTimePass(state_machine, 10);
    ASSERT_EQ(
    ...
    apps_is_greater_than_25_percent_and_brake_is_actuated_sets_motor_shutdown_can_tx_signal)
{
    ...
    // Check CAN signals when the mapped pedal percentage is less than 25%
    get_papps_encoder_counter_fake.return_val =
        GetPrimaryEncoderCounterFromPedalPercentage(24);
    get_sapps_encoder_counter_fake.return_val =
        GetSecondaryEncoderCounterFromPedalPercentage(25);
    LetTimePass(state_machine, 10);
    ASSERT_EQ(
    ...
    // Check CAN signals when the mapped pedal percentage is greater than 25%
    get_papps_encoder_counter_fake.return_val =
        GetPrimaryEncoderCounterFromPedalPercentage(26);
    LetTimePass(state_machine, 1);
    ASSERT_EQ(
...
TEST_F(
    FsmStateMachineTest,
    apps_is_less_than_5_percent_and_brake_is_actuated_clears_motor_shutdown_can_tx_signal)
{
    is_brake_actuated_fake.return_val = true;
    ...
    get_papps_encoder_counter_fake.return_val =
        GetPrimaryEncoderCounterFromPedalPercentage(4);
    get_sapps_encoder_counter_fake.return_val =
        GetSecondaryEncoderCounterFromPedalPercentage(4);
    LetTimePass(state_machine, 10);
    ASSERT_EQ(
        CANMSGS_FSM_MOTOR_SHUTDOWN_ERRORS_PLAUSIBILITY_CHECK_HAS_FAILED_FALSE_CHOICE,
        App_CanTx_GetPeriodicSignal_PLAUSIBILITY_CHECK_HAS_FAILED(
...
TEST_F(
    FsmStateMachineTest,
    apps_is_less_than_5_percent_and_brake_is_not_actuated_clears_motor_shutdown_can_tx_signal)
{
    is_brake_actuated_fake.return_val = true;
