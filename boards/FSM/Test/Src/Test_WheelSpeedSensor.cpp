...
FAKE_VALUE_FUNC(enum WheelSpeedFaultStatus, get_wheel_speed_status);
...
TEST_F(WheelSpeedSensorTest, check_if_wheel_speed_non_critical_fault_is_updated)
{
    ...
    ASSERT_EQ(
        WHEEL_SPEED_NORMAL,
        App_WheelSpeedSensor_GetFaultStatus(wheel_speed_sensor));
    ...
    // 0x400000 is NaN; Check that wheel speed sensor non critical fault is
    // active
