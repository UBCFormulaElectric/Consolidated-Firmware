...
static void Io_CanTx_DCM_EllipseImuStatus_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DCM_ELLIPSE_IMU_STATUS_ID;
        tx_msg.dlc = CAN_MSG_DCM_ELLIPSE_IMU_STATUS_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DCM_EllipseImuStatus_Pack(App_CanTx_DCM_EllipseImuStatus_GetData(), tx_msg.data);
        vPortExitCritical();
...
static void Io_CanTx_DCM_EllipseImuTime_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DCM_ELLIPSE_IMU_TIME_ID;
        tx_msg.dlc = CAN_MSG_DCM_ELLIPSE_IMU_TIME_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DCM_EllipseImuTime_Pack(App_CanTx_DCM_EllipseImuTime_GetData(), tx_msg.data);
        vPortExitCritical();
...
static void Io_CanTx_DCM_EllipseImuAcceleration_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DCM_ELLIPSE_IMU_ACCELERATION_ID;
        tx_msg.dlc = CAN_MSG_DCM_ELLIPSE_IMU_ACCELERATION_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DCM_EllipseImuAcceleration_Pack(App_CanTx_DCM_EllipseImuAcceleration_GetData(), tx_msg.data);
        vPortExitCritical();
...
static void Io_CanTx_DCM_EllipseImuAngularVelocity_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DCM_ELLIPSE_IMU_ANGULAR_VELOCITY_ID;
        tx_msg.dlc = CAN_MSG_DCM_ELLIPSE_IMU_ANGULAR_VELOCITY_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DCM_EllipseImuAngularVelocity_Pack(App_CanTx_DCM_EllipseImuAngularVelocity_GetData(), tx_msg.data);
        vPortExitCritical();
...
static void Io_CanTx_DCM_EllipseImuEulerAngles_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DCM_ELLIPSE_IMU_EULER_ANGLES_ID;
        tx_msg.dlc = CAN_MSG_DCM_ELLIPSE_IMU_EULER_ANGLES_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DCM_EllipseImuEulerAngles_Pack(App_CanTx_DCM_EllipseImuEulerAngles_GetData(), tx_msg.data);
        vPortExitCritical();
...
void Io_CanTx_Enqueue100HzMsgs()
{
    ...
    Io_CanTx_DCM_EllipseImuAcceleration_SendPeriodic();
    Io_CanTx_DCM_EllipseImuAngularVelocity_SendPeriodic();
    Io_CanTx_DCM_EllipseImuEulerAngles_SendPeriodic();
}
...
void Io_CanTx_EnqueueOtherPeriodicMsgs(uint32_t time_ms)
{
    ...
    if (time_ms % CAN_MSG_DCM_ELLIPSE_IMU_STATUS_CYCLE_TIME_MS == 0)
    {
        Io_CanTx_DCM_EllipseImuStatus_SendPeriodic();
    }
    
    if (time_ms % CAN_MSG_DCM_ELLIPSE_IMU_TIME_CYCLE_TIME_MS == 0)
    {
        Io_CanTx_DCM_EllipseImuTime_SendPeriodic();
    }
