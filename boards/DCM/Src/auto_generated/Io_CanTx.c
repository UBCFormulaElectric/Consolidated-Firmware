...
void Io_CanTx_TransmitPeriodicMessages(struct CanTxInterface* can_tx_interface, const uint32_t current_ms)
{
    ...
    // Is it time to transmit this particular CAN message?
    if ((current_ms % CANMSGS_DCM_ERRORS_CYCLE_TIME_MS) == 0)
    {
        memset(&tx_message, 0, sizeof(tx_message));

        // Prepare CAN message header
        tx_message.std_id = CANMSGS_DCM_ERRORS_FRAME_ID;
        tx_message.dlc = CANMSGS_DCM_ERRORS_LENGTH;

        // Prepare CAN message payload (The packing function isn't thread-safe
        // so we must guard it)
        vPortEnterCritical();
        CanMsgs_dcm_errors_pack(
            &tx_message.data[0],
            App_CanTx_GetPeriodicMsgPointer_DCM_ERRORS(can_tx_interface),
            tx_message.dlc);
        vPortExitCritical();
...
void Io_CanTx_EnqueueNonPeriodicMsg_DCM_WATCHDOG_TIMEOUT(struct CanMsgs_dcm_watchdog_timeout_t* payload)
{
    ...
    struct CanMsg tx_msg;
    memset(&tx_msg, 0, sizeof(tx_msg));
    tx_msg.std_id = CANMSGS_DCM_WATCHDOG_TIMEOUT_FRAME_ID;
    tx_msg.dlc    = CANMSGS_DCM_WATCHDOG_TIMEOUT_LENGTH;
    memcpy(&tx_msg.data[0], &payload, CANMSGS_DCM_WATCHDOG_TIMEOUT_LENGTH); 
    CanMsgs_dcm_watchdog_timeout_pack(&tx_msg.data[0], payload, CANMSGS_DCM_WATCHDOG_TIMEOUT_LENGTH);
    Io_SharedCan_TxMessageQueueSendtoBack(&tx_msg);
}
