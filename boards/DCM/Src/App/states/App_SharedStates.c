...
void App_SharedStatesRunOnTick100Hz(struct StateMachine *const state_machine)
{
    ...
    App_BrakeLight_SetLightStatus(
        brake_light, App_CanRx_FSM_BRAKE_GetSignal_BRAKE_IS_ACTUATED(can_rx),
        is_regen_active);
    App_CanTx_SetPeriodicSignal_RIGHT_INVERTER_SWITCH(
        can_tx, App_InverterSwitches_IsRightOn(inverter_switches));
