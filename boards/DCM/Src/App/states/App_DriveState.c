...
void App_SetPeriodicCanSignals_TorqueRequests() {
  ...
  //       App_CanRx_BMS_AvailablePower_AvailablePower_Get();
  ...
  if ((right_motor_speed_rpm + left_motor_speed_rpm) > 0.0f) {
    // Estimate the maximum torque request to draw the maximum power available
    // from the BMS
    //        bms_torque_limit = bms_available_power * EFFICIENCY_ESTIMATE /
    //                           (RPM_TO_RADS(right_motor_speed_rpm) +
    //                           RPM_TO_RADS(left_motor_speed_rpm));
  }

  // Calculate the maximum torque request to scale pedal percentage off of
  //    const float max_torque_request = MIN(bms_torque_limit,
