...
bool App_AllStatesRunOnTick100Hz(struct StateMachine *const state_machine)
{
    ...
    const bool brake_actuated = App_CanRx_FSM_BRAKE_GetSignal_BRAKE_IS_ACTUATED(can_rx);
    App_BrakeLight_SetLightStatus(brake_actuated);

    App_CanTx_SetPeriodicSignal_RIGHT_INVERTER_SWITCH(can_tx, App_InverterSwitches_IsRightOn());
    App_CanTx_SetPeriodicSignal_LEFT_INVERTER_SWITCH(can_tx, App_InverterSwitches_IsLeftOn());
