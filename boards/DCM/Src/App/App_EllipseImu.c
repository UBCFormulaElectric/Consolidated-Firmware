...
void App_EllipseImu_HandleLogs() { sensor.handle_logs(); }
...
void App_EllipseImu_Broadcast() {
  ...
  const float forward_accel =
      sensor.get_sensor_output(ELLIPSE_OUTPUT_ACCELERATION_X);
  const float lateral_accel =
      sensor.get_sensor_output(ELLIPSE_OUTPUT_ACCELERATION_Y);
  const float vertical_accel =
      sensor.get_sensor_output(ELLIPSE_OUTPUT_ACCELERATION_Z);
  ...
  const float ang_vel_roll =
      sensor.get_sensor_output(ELLIPSE_OUTPUT_ANGULAR_VELOCITY_ROLL);
  const float ang_vel_pitch =
      sensor.get_sensor_output(ELLIPSE_OUTPUT_ANGULAR_VELOCITY_PITCH);
  const float ang_vel_yaw =
      sensor.get_sensor_output(ELLIPSE_OUTPUT_ANGULAR_VELOCITY_YAW);
  ...
  const float euler_pitch =
      sensor.get_sensor_output(ELLIPSE_OUTPUT_EULER_PITCH);
...
uint32_t App_EllipseImu_GetTimestampUs() { return sensor.get_timestamp(); }
