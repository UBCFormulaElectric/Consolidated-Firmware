...
struct DcmWorld *App_DcmWorld_Create(
    ...
    struct BrakeLight *const brake_light, struct Buzzer *const buzzer,
    struct Imu *const imu, struct Clock *const clock,
  ...
  struct WaitSignalCallback buzzer_callback = {
      .function = buzzer_complete_callback,
...
struct BrakeLight *App_DcmWorld_GetBrakeLight(
    const struct DcmWorld *const world) {
...
void App_DcmWorld_UpdateWaitSignal(const struct DcmWorld *const world,
                                   uint32_t current_ms) {
