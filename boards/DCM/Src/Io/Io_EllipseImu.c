...
static uint8_t uart_rx_buffer[UART_RX_PACKET_SIZE];  // Buffer to hold last RXed
                                                     // UART packet
...
float *sensor_output_map[NUM_SBG_OUTPUTS] = {
    ...
    [ELLIPSE_OUTPUT_ANGULAR_VELOCITY_ROLL] =
        &sensor_data.imu_data.angular_velocity.roll,
    [ELLIPSE_OUTPUT_ANGULAR_VELOCITY_PITCH] =
        &sensor_data.imu_data.angular_velocity.pitch,
    [ELLIPSE_OUTPUT_ANGULAR_VELOCITY_YAW] =
        &sensor_data.imu_data.angular_velocity.yaw,
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
  // NOTE: I previously tried handling logs within the interrupt itself, but
  // this was throwing errors. Not sure why but the error msg was related to
  // mallocing within an ISR, although I couldn't figure out where in SBG's
  // library anything was actually being malloced. This is why I push to a queue
  // here and handle the logs later in the 100Hz task.
...
/*
 * Create a serial interface that will be used by SBG's library. Requires
 * passing pointers to functions for all I/O operations, such as reading,
 * writing, flushing, etc. These I/O operations will use STM32's HAL drivers to
 * communicate to the sensor. We only need read operations, so that's all we
 * provide here.
 */
static void Io_EllipseImu_CreateSerialInterface(SbgInterface *interface) {
...
static SbgErrorCode Io_EllipseImu_Read(SbgInterface *interface, void *buffer,
  ...
  // Disable interrupts so UART RX callback won't push data to the queue while
 ...
 */
SbgErrorCode Io_EllipseImu_LogReceivedCallback(SbgEComHandle *handle,
...
static void Io_EllipseImu_ProcessMsg_Imu(const SbgBinaryLogData *log_data) {
  ...
  sensor_data.imu_data.angular_velocity.roll =
      RAD_TO_DEG(log_data->imuData.gyroscopes[0]);
  sensor_data.imu_data.angular_velocity.pitch =
      RAD_TO_DEG(log_data->imuData.gyroscopes[1]);
  sensor_data.imu_data.angular_velocity.yaw =
      RAD_TO_DEG(log_data->imuData.gyroscopes[2]);
}
...
static void Io_EllipseImu_ProcessMsg_EulerAngles(
  ...
  sensor_data.euler_data.euler_angles.roll =
      RAD_TO_DEG(log_data->ekfEulerData.euler[0]);
  sensor_data.euler_data.euler_angles.pitch =
      RAD_TO_DEG(log_data->ekfEulerData.euler[1]);
  sensor_data.euler_data.euler_angles.yaw =
      RAD_TO_DEG(log_data->ekfEulerData.euler[2]);
}
...
bool Io_EllipseImu_Init() {
  ...
  // Set the callback function (callback is called when a new log is
  // successfully received and parsed)
  sbgEComSetReceiveLogCallback(&com_handle, Io_EllipseImu_LogReceivedCallback,
                               NULL);
...
void Io_EllipseImu_HandleLogs() {
  // Handle logs. Calls the pReadFunc set in sbgInterfaceSerialCreate to read
  // data and parses all logs found in the data. Upon successfully parsing a
  // log, the the receive log callback function set in init is triggered.
  // Incomplete log data will be saved to a buffer in SBG's library to be used
  // once more data is received.
