...
void App_PdmWorld_Destroy(struct PdmWorld *const world) { free(world); }

struct InRangeCheck *App_PdmWorld_GetVbatVoltageInRangeCheck(
...
struct InRangeCheck *App_PdmWorld_GetAux1CurrentInRangeCheck(
    const struct PdmWorld *const world) {
  return world->aux1_current_in_range_check;
}

struct InRangeCheck *App_PdmWorld_GetAux2CurrentInRangeCheck(
    const struct PdmWorld *const world) {
...
struct InRangeCheck *App_PdmWorld_GetCanCurrentInRangeCheck(
    const struct PdmWorld *const world) {
...
struct HeartbeatMonitor *App_PdmWorld_GetHeartbeatMonitor(
    const struct PdmWorld *const world) {
  return world->heartbeat_monitor;
}

struct RgbLedSequence *App_PdmWorld_GetRgbLedSequence(
    const struct PdmWorld *const world) {
  return world->rgb_led_sequence;
}

struct LowVoltageBattery *App_PdmWorld_GetLowVoltageBattery(
    const struct PdmWorld *const world) {
