...
void Io_Efuse_SetChannel(EfuseChannel channel, bool enabled) {
  ...
  HAL_GPIO_WritePin(config[channel].in_gpio_port, config[channel].in_gpio_pin,
...
float Io_Efuse_GetChannelCurrent(EfuseChannel channel) {
  ...
  return Io_Adc_GetChannelVoltage(config[channel].cur_sns_adc_channel) *
         ADC_VOLTAGE_TO_CURRENT_A;
}
...
void Io_Efuse_StandbyReset(EfuseChannel channel) {
  ...
  HAL_GPIO_WritePin(config[channel].stby_reset_gpio_port,
                    config[channel].stby_reset_gpio_pin, GPIO_PIN_SET);
  HAL_GPIO_WritePin(config[channel].stby_reset_gpio_port,
                    config[channel].stby_reset_gpio_pin, GPIO_PIN_RESET);
  HAL_GPIO_WritePin(config[channel].stby_reset_gpio_port,
                    config[channel].stby_reset_gpio_pin, GPIO_PIN_SET);
}
