...
FAKE_VOID_FUNC(send_non_periodic_msg_PDM_STARTUP,
               const struct CanMsgs_pdm_startup_t *);
FAKE_VOID_FUNC(send_non_periodic_msg_PDM_AIR_SHUTDOWN,
               const struct CanMsgs_pdm_air_shutdown_t *);
FAKE_VOID_FUNC(send_non_periodic_msg_PDM_MOTOR_SHUTDOWN,
               const struct CanMsgs_pdm_motor_shutdown_t *);
FAKE_VOID_FUNC(send_non_periodic_msg_PDM_WATCHDOG_TIMEOUT,
 ...
 protected:
  void SetUp() override {
    ...
    energy_meter_current_in_range_check =
        App_InRangeCheck_Create(GetEnergyMeterCurrent, ENERGY_METER_MIN_CURRENT,
    ...
    air_shutdown_current_in_range_check =
        App_InRangeCheck_Create(GetAirShutdownCurrent, AIR_SHUTDOWN_MIN_CURRENT,
    ...
    world = App_PdmWorld_Create(
        vbat_voltage_in_range_check, _24v_aux_voltage_in_range_check,
        _24v_acc_voltage_in_range_check, aux1_current_in_range_check,
        aux2_current_in_range_check, left_inverter_current_in_range_check,
  ...
  void TearDown() override {
    ...
    TearDownObject(left_inverter_current_in_range_check,
                   App_InRangeCheck_Destroy);
    TearDownObject(right_inverter_current_in_range_check,
                   App_InRangeCheck_Destroy);
    TearDownObject(energy_meter_current_in_range_check,
                   App_InRangeCheck_Destroy);
    TearDownObject(can_current_in_range_check, App_InRangeCheck_Destroy);
    TearDownObject(air_shutdown_current_in_range_check,
                   App_InRangeCheck_Destroy);
  ...
  std::vector<const struct State *> GetAllStatesExceptForInit(void) {
    ...
    vec.erase(std::remove(vec.begin(), vec.end(), App_GetInitState()),
              vec.end());
  ...
  void CheckCurrentCanSignalsInNonInitStates(
      float min_current, float max_current, float &fake_current,
  ...
  void CheckVoltageCanSignalsInAllStates(
      float min_voltage, float max_voltage, float &fake_voltage,
    ...
    for (auto &state : GetAllStates()) {
      ...
      // Under-voltage
      fake_voltage =
          std::nextafter(min_voltage, std::numeric_limits<float>::lowest());
      LetTimePass(state_machine, 10);
      EXPECT_EQ(underflow_choice,
                out_of_range_can_signal_getter(can_tx_interface));
...
//    SetInitialState(App_GetInitState());
//
//    EXPECT_EQ(CANMSGS_PDM_STATE_MACHINE_STATE_INIT_CHOICE,
...
//
//    EXPECT_EQ(CANMSGS_PDM_STATE_MACHINE_STATE_AIR_OPEN_CHOICE,
//    App_CanTx_GetPeriodicSignal_STATE(can_tx_interface));
//}
//
//// PDM-21
...
//
//    EXPECT_EQ(CANMSGS_PDM_STATE_MACHINE_STATE_AIR_CLOSED_CHOICE,
//    App_CanTx_GetPeriodicSignal_STATE(can_tx_interface));
//}
//
//// PDM-6
...
//        Get24vAuxVoltage_fake.return_val,
//        App_CanTx_GetPeriodicSignal__24_V_AUX,
...
//        Get24vAccVoltage_fake.return_val,
//        App_CanTx_GetPeriodicSignal__24_V_ACC,
...
//
//// PDM-11, PDM-13
// TEST_F(PdmStateMachineTest,
// check_aux1_current_can_signals_in_non_init_states)
//{
...
//// PDM-11, PDM-13
// TEST_F(PdmStateMachineTest,
// check_aux2_current_can_signals_in_non_init_states)
//{
//    CheckCurrentCanSignalsInNonInitStates(
...
TEST_F(PdmStateMachineTest,
  ...
  ASSERT_EQ(App_GetAirClosedState(),
            App_SharedStateMachine_GetCurrentState(state_machine));
}
...
//
//        // Clear the fault to prevent false positive
//        App_CanTx_SetPeriodicSignal_BOOST_PGOOD_FAULT(can_tx_interface,
//        false);
//
//        do_low_voltage_battery_have_boost_controller_fault_fake.return_val =
//        true; LetTimePass(state_machine, 10);
//
...
//// PDM-5
// TEST_F(PdmStateMachineTest,
// clear_18650s_boost_controller_fault_in_all_states)
//{
//    for (auto &state : GetAllStates())
//    {
//        SetInitialState(state);
...
//
//        do_low_voltage_battery_have_boost_controller_fault_fake.return_val =
//        false; LetTimePass(state_machine, 10);
//
