...
class InRangeCheckTest : public testing::Test {
 protected:
  void SetUp() override {
    in_range_check =
        App_InRangeCheck_Create(get_value, DEFAULT_IN_RANGE_CHECK_MIN_VALUE,
...
TEST_F(InRangeCheckTest, value_underflow) {
  ...
  get_value_fake.return_val = std::nextafter(DEFAULT_IN_RANGE_CHECK_MIN_VALUE,
...
TEST_F(InRangeCheckTest, value_overflow) {
  ...
  get_value_fake.return_val = std::nextafter(DEFAULT_IN_RANGE_CHECK_MAX_VALUE,
