...
// Overcurrent profile specific. Dependent on CONF[x] pin state.
...
typedef enum
{
    ...
    ChannelConfigType_DirecControl_Disable = (1 << 5), //!< Direct Control Disable
    ChannelConfigType_OpenLoad_InOffState  = (1 << 6), //!< Open Load Detection in Off-state
    ChannelConfigType_OpenLoad_InOnState   = (1 << 7), //!< Open Load Detection in On-state
    ChannelConfigType_ShortCircuitDetection_Disable = (1 << 8) //!< Short-circuit to Vpwr Detection Disable
} ChannelConfigType_e;
...
struct E_Fuse
{
    ...
    struct E_FuseChipSelect
    {
        GPIO_TypeDef *GPIO_Port;
        uint16_t      GPIO_Pin;
    } chip_select;
    struct E_FuseCurrentSenseSync
    {
        GPIO_TypeDef *GPIO_Port;
        uint16_t      GPIO_Pin;
    } current_sense_sync;
...
/**
 * Configures the Efuse's registers using the values defined in: REGNAME_CONFIG
 * @param *e_fuse Pointer to E_Fuse structure being configured
 */
void Io_Efuse_22XS4200_configureEfuse(struct E_Fuse *e_fuse);

/**
 * Sets current/temperature monitoring option of CSNS pin
 * @param selection The desired monitoring function which can be selected by specifying the following values:
 ...
 *                  CSNS_FUNCTION_CURRENT_SUM - Current sensing for summed channels (for Parallel mode)
 * @param *e_fuse Pointer to E_Fuse structure being configured
 */
void Io_Efuse_22XS4200_configure_channel_monitoring(uint8_t selection, struct E_Fuse *e_fuse);

/**
 * Gets the value of the Efuse's Status Register over SPI and updates the Efuse handle's operating status
 * @param *e_fuse Pointer to E_Fuse structure being checked
 */
void Io_Efuse_22XS4200_UpdateStatus(struct E_Fuse *e_fuse);

/**
 * DGets the value of the Efuse's Fault Register over SPI and updates the Efuse handle's fault status
 * @param *e_fuse Pointer to E_Fuse structure being checked
 */
 ...
 * @param *e_fuse Pointer to E_Fuse structure being written to
 */
void Io_Efuse_22XS4200_WriteReg(uint8_t register_address, uint16_t register_value, struct E_Fuse *e_fuse);
 ...
 * @param *e_fuse Pointer to E_Fuse structure being read from
 * @return Contents of the addressed register
 */
uint16_t Io_Efuse_22XS4200_ReadReg(uint8_t register_address, struct E_Fuse *e_fuse);
 ...
 */
HAL_StatusTypeDef Io_Efuse_22XS4200_WriteToEfuse(uint16_t *TxData, GPIO_TypeDef *GPIO_Port, uint16_t GPIO_Pin);

/**
 * Read SPI data from Efuse
 * @param *TxData Pointer to data being sent to the Efuse
 ...
 * @return Status of the SPI transfer
 */
HAL_StatusTypeDef Io_Efuse_22XS4200_ReadFromEfuse(uint16_t *TxData, uint16_t *RxData, GPIO_TypeDef *GPIO_Port, uint16_t GPIO_Pin);
