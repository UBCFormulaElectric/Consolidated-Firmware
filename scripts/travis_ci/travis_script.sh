...
if [ "$RUN_BUILD" = "true" ]; then
    # Extract the CubeIDE directories so we have access to the executables in them
    cd $SCRIPT_DIR/../..
    ...
    # We assume this executable was extracted from the cube ide setup files
    travis_run ./headless-build.sh -build all
fi
...
# TODO: re-add this, probably via stm32cubeide somehow?
