...
class AppCanAlertsModule(CModule):
    def __init__(self, db: CanDatabase, node: str):
        ...
        self._node_name_and_alerts: List[Tuple[str, List[CanAlert]]] = [(k, v) for k, v in db.alerts.items()]
    ...
    def header_template(self) -> str:
        ...
        return template.render(
            ...
            node_name_and_alerts=self._node_name_and_alerts,
            # List[CanAlert] x 3
            tx_faults=[alert for alert in self._db.alerts[self._node_name] if alert.alert_type == CanAlertType.FAULT],
            tx_warnings=[alert for alert in self._db.alerts[self._node_name] if
                         alert.alert_type == CanAlertType.WARNING],
            tx_info=[alert for alert in self._db.alerts[self._node_name] if alert.alert_type == CanAlertType.INFO],
        )
