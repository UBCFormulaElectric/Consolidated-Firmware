...
struct IoCanRxModuleSourceCpp<'a> {
    messages_on_busses: &'a Vec<(String, Vec<CanMessage>)>,
}

#[derive(Template)]
#[template(path = "io_canRx.hpp.j2")]
struct IoCanRxModuleHeaderCpp<'a> {
    node: &'a String,
    messages_on_busses: &'a Vec<(String, Vec<CanMessage>)>,
}

pub struct IoCanRxModuleCpp<'a> {
    board: &'a String,
    messages_on_busses: Vec<(String, Vec<CanMessage>)>,
}
impl IoCanRxModuleCpp<'_> {
    pub fn new<'a>(
        can_db: &'a CanDatabase,
        rx_config: &'a CanRxConfig,
        board: &'a String,
...
impl CPPGenerator for IoCanRxModuleCpp<'_> {
    fn file_stem(&self) -> String {
        "io_canRx".to_string()
    }
    fn header_template(&self) -> Result<String, askama::Error> {
        IoCanRxModuleHeaderCpp {
            node: self.board,
            messages_on_busses: &self.messages_on_busses,
        }
        .render()
