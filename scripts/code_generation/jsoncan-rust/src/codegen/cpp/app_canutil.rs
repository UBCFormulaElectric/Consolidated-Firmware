...
struct AppCanUtilsModuleSourceCpp<'a> {
    messages: &'a Vec<CanMessage>,
}
impl AppCanUtilsModuleSourceCpp<'_> {
    fn signal_placement_comment(self: &Self, msg: &CanMessage) -> std::string::String {
        let chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
...
struct AppCanUtilsModuleHeaderCpp<'a> {
    messages: &'a Vec<CanMessage>,
    node_names: &'a Vec<String>,
    enums: &'a Vec<CanEnum>,
}

pub struct AppCanUtilsModuleCpp {
    messages: Vec<CanMessage>,
    node_names: Vec<String>,
    enums: Vec<CanEnum>,
}

impl AppCanUtilsModuleCpp {
    pub fn new(can_db: &CanDatabase) -> AppCanUtilsModuleCpp {
        AppCanUtilsModuleCpp {
            messages: can_db.get_all_msgs().expect("get all msgs should not fail"),
            node_names: can_db.nodes.iter().map(|n| n.name.clone()).collect(),
            enums: can_db
                .nodes
...
impl CPPGenerator for AppCanUtilsModuleCpp {
    fn file_stem(&self) -> String {
        "app_canUtils".to_string()
    }
    fn header_template(&self) -> Result<String, askama::Error> {
        AppCanUtilsModuleHeaderCpp {
            messages: &self.messages,
            node_names: &self.node_names,
            enums: &self.enums,
        }
