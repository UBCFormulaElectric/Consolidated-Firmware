...
static void Io_CanTx_DCM_StackRemaining_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DCM_STACK_REMAINING_ID;
        tx_msg.dlc = CAN_MSG_DCM_STACK_REMAINING_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DCM_StackRemaining_Pack(App_CanTx_DCM_StackRemaining_GetData(), tx_msg.data);
        vPortExitCritical();
...
void Io_CanTx_EnqueueOtherPeriodicMsgs(uint32_t time_ms)
{
    ...
    if (time_ms % CAN_MSG_DCM_STACK_REMAINING_CYCLE_TIME_MS == 0)
    {
        Io_CanTx_DCM_StackRemaining_SendPeriodic();
    }
