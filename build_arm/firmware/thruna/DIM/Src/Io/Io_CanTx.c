...
static void Io_CanTx_DIM_StackRemaining_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DIM_STACK_REMAINING_ID;
        tx_msg.dlc = CAN_MSG_DIM_STACK_REMAINING_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DIM_StackRemaining_Pack(App_CanTx_DIM_StackRemaining_GetData(), tx_msg.data);
        vPortExitCritical();
...
static void Io_CanTx_DIM_CommitInfo_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DIM_COMMIT_INFO_ID;
        tx_msg.dlc = CAN_MSG_DIM_COMMIT_INFO_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DIM_CommitInfo_Pack(App_CanTx_DIM_CommitInfo_GetData(), tx_msg.data);
        vPortExitCritical();
...
static void Io_CanTx_DIM_WarningsCounts_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DIM_WARNINGS_COUNTS_ID;
        tx_msg.dlc = CAN_MSG_DIM_WARNINGS_COUNTS_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DIM_WarningsCounts_Pack(App_CanTx_DIM_WarningsCounts_GetData(), tx_msg.data);
        vPortExitCritical();
...
static void Io_CanTx_DIM_FaultsCounts_SendPeriodic()
{
    if (can_mode & (CAN_MODE_DEFAULT))
    {
        // Prepare msg header
        struct CanMsg tx_msg;
        memset(&tx_msg, 0, sizeof(struct CanMsg));
        tx_msg.std_id = CAN_MSG_DIM_FAULTS_COUNTS_ID;
        tx_msg.dlc = CAN_MSG_DIM_FAULTS_COUNTS_BYTES;
        
        // Prepare CAN msg payload (The packing function isn't thread-safe so we must guard it)
        vPortEnterCritical();
        App_CanUtils_DIM_FaultsCounts_Pack(App_CanTx_DIM_FaultsCounts_GetData(), tx_msg.data);
        vPortExitCritical();
...
void Io_CanTx_Enqueue1HzMsgs()
{
    ...
    Io_CanTx_DIM_WarningsCounts_SendPeriodic();
}
...
void Io_CanTx_EnqueueOtherPeriodicMsgs(uint32_t time_ms)
{
    ...
    if (time_ms % CAN_MSG_DIM_STACK_REMAINING_CYCLE_TIME_MS == 0)
    {
        Io_CanTx_DIM_StackRemaining_SendPeriodic();
    }
    
    if (time_ms % CAN_MSG_DIM_COMMIT_INFO_CYCLE_TIME_MS == 0)
    {
        Io_CanTx_DIM_CommitInfo_SendPeriodic();
    }
    ...
    if (time_ms % CAN_MSG_DIM_FAULTS_COUNTS_CYCLE_TIME_MS == 0)
    {
        Io_CanTx_DIM_FaultsCounts_SendPeriodic();
    }
