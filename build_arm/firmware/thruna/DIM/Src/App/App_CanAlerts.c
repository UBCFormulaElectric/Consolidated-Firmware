...
void App_CanAlerts_SetWarning(DIM_WarningId alert_id, bool set_alert)
{
    switch (alert_id)
    {
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1HZ:
        {
            App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1HZ_Set(set_alert);
            break;
        }
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_100HZ:
        {
            App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_100HZ_Set(set_alert);
            break;
        }
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1KHZ:
        {
            App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1KHZ_Set(set_alert);
            break;
        }
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANRX:
        {
            App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANRX_Set(set_alert);
            break;
        }
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANTX:
        {
            App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANTX_Set(set_alert);
            break;
        }
        case DIM_WARNING_WATCHDOG_TIMEOUT:
        {
            App_CanTx_DIM_Warnings_DIM_WARNING_WATCHDOG_TIMEOUT_Set(set_alert);
            break;
        }
        case DIM_WARNING_TX_OVERFLOW:
        {
            App_CanTx_DIM_Warnings_DIM_WARNING_TX_OVERFLOW_Set(set_alert);
            break;
        }
        case DIM_WARNING_RX_OVERFLOW:
        {
            App_CanTx_DIM_Warnings_DIM_WARNING_RX_OVERFLOW_Set(set_alert);
            break;
        }
        default:
        {
...
void App_CanAlerts_SetFault(DIM_FaultId alert_id, bool set_alert)
{
    switch (alert_id)
    {
        case DIM_FAULT_MISSING_HEARTBEAT:
        {
            App_CanTx_DIM_Faults_DIM_FAULT_MISSING_HEARTBEAT_Set(set_alert);
            break;
        }
        default:
        {
...
bool App_CanAlerts_GetWarning(DIM_WarningId alert_id)
{
    switch (alert_id)
    {
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1HZ:
        {
            return App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1HZ_Get();
            break;
        }
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_100HZ:
        {
            return App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_100HZ_Get();
            break;
        }
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1KHZ:
        {
            return App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1KHZ_Get();
            break;
        }
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANRX:
        {
            return App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANRX_Get();
            break;
        }
        case DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANTX:
        {
            return App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANTX_Get();
            break;
        }
        case DIM_WARNING_WATCHDOG_TIMEOUT:
        {
            return App_CanTx_DIM_Warnings_DIM_WARNING_WATCHDOG_TIMEOUT_Get();
            break;
        }
        case DIM_WARNING_TX_OVERFLOW:
        {
            return App_CanTx_DIM_Warnings_DIM_WARNING_TX_OVERFLOW_Get();
            break;
        }
        case DIM_WARNING_RX_OVERFLOW:
        {
            return App_CanTx_DIM_Warnings_DIM_WARNING_RX_OVERFLOW_Get();
            break;
        }
        default:
        {
...
bool App_CanAlerts_GetFault(DIM_FaultId alert_id)
{
    switch (alert_id)
    {
        case DIM_FAULT_MISSING_HEARTBEAT:
        {
            return App_CanTx_DIM_Faults_DIM_FAULT_MISSING_HEARTBEAT_Get();
            break;
        }
        default:
        {
...
bool App_CanAlerts_BoardHasWarning(CanAlertBoard board)
{
    switch (board)
    {
        ...
        {
            if (App_CanRx_DCM_Warnings_DCM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1HZ_Get())
            {
                return true;
            }
            
            if (App_CanRx_DCM_Warnings_DCM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_100HZ_Get())
            {
                return true;
            }
            
            if (App_CanRx_DCM_Warnings_DCM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1KHZ_Get())
            {
                return true;
            }
            
            if (App_CanRx_DCM_Warnings_DCM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANRX_Get())
            {
                return true;
            }
            
            if (App_CanRx_DCM_Warnings_DCM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANTX_Get())
            {
                return true;
            }
            
            if (App_CanRx_DCM_Warnings_DCM_WARNING_WATCHDOG_TIMEOUT_Get())
            {
                return true;
            }
            
            if (App_CanRx_DCM_Warnings_DCM_WARNING_TX_OVERFLOW_Get())
            {
                return true;
            }
            
            if (App_CanRx_DCM_Warnings_DCM_WARNING_RX_OVERFLOW_Get())
            {
                return true;
            }
        ...
        case FSM_ALERT_BOARD:
        {
            ...
            if (App_CanRx_FSM_Warnings_FSM_WARNING_APPS_HAS_DISAGREEMENT_Get())
            {
                return true;
            }
        ...
        {
            if (App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1HZ_Get())
            {
                return true;
            }
            
            if (App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_100HZ_Get())
            {
                return true;
            }
            
            if (App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_1KHZ_Get())
            {
                return true;
            }
            
            if (App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANRX_Get())
            {
                return true;
            }
            
            if (App_CanTx_DIM_Warnings_DIM_WARNING_STACK_WATERMARK_ABOVE_THRESHOLD_TASK_CANTX_Get())
            {
                return true;
            }
            
            if (App_CanTx_DIM_Warnings_DIM_WARNING_WATCHDOG_TIMEOUT_Get())
            {
                return true;
            }
            
            if (App_CanTx_DIM_Warnings_DIM_WARNING_TX_OVERFLOW_Get())
            {
                return true;
            }
            
            if (App_CanTx_DIM_Warnings_DIM_WARNING_RX_OVERFLOW_Get())
            {
                return true;
            }
...
bool App_CanAlerts_BoardHasFault(CanAlertBoard board)
{
    switch (board)
    {
        case BMS_ALERT_BOARD:
        {
            ...
            if (App_CanRx_BMS_Faults_BMS_FAULT_LATCHED_BMS_FAULT_Get())
            {
                return true;
            }
        ...
        case DCM_ALERT_BOARD:
        {
            if (App_CanRx_DCM_Faults_DCM_FAULT_STATE_FAULT_Get())
            {
                return true;
            }
            
            if (App_CanRx_DCM_Faults_DCM_FAULT_MISSING_HEARTBEAT_Get())
            {
                return true;
            }
        ...
        case FSM_ALERT_BOARD:
        {
            ...
            if (App_CanRx_FSM_Faults_FSM_FAULT_TORQUE_PLAUSABILITY_FAILED_Get())
            {
                return true;
            }
        ...
        case DIM_ALERT_BOARD:
        {
            if (App_CanTx_DIM_Faults_DIM_FAULT_MISSING_HEARTBEAT_Get())
            {
                return true;
            }
