...
#define PRIMARY_APPS_DEADZONE \
    (float)PRIMARY_APPS_MAX_VALUE *APPS_PERCENT_DEADZONE
#define SECONDARY_APPS_DEADZONE \
    (float)SECONDARY_APPS_MAX_VALUE *APPS_PERCENT_DEADZONE
...
#define MAX_APPS_FAULTS \
    (float)APPS_IMPLAUSIBILITY_DELAY *(float)CONTROL_LOOP_FREQUENCY
...
/**
  *@brief	  Gets the latest accelerator pedal position
  ...
  values) *4. Checks for implausible APPS readings (>10% difference between the
  primary and secondary readings as per EV2.3.6) *5. Checks for APPS/Brake Pedal
  Plausibility Check, where accelerator pedal is pushed >25% while brake is
  pushed (EV2.5) *6. Checks for accelerator pedal stuck at max. torque for
  longer than 10 secs *7. Maps the APPS readings to a 10-bit number (0 =
