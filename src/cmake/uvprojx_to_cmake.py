...
def generate_cmake_file_str(c_source_file_paths: [str], include_dirs: [str], 
        file_path_prefix: str, cmake_variable_prefix: str) -> str:
...
list(APPEND {cmake_variable_prefix}INCLUDE_DIRS {include_dirs})
set({cmake_variable_prefix}SRC_C_FILES {src_c_files})
            '''
    return CMAKE_FILE_FORMAT.format(
            include_dirs=include_dirs_str,
            src_c_files=src_c_files_str,
            cmake_variable_prefix=cmake_variable_prefix
            )
...
def main():
    ...
    parser.add_option("", "--cmake-variable-prefix", dest="cmake_variable_prefix",
            default="",
            help="specifies the prefix to append to the start of all generated\
                    cmake variables")
