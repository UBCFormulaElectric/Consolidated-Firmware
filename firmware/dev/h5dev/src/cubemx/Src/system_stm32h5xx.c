...
#define VECT_TAB_OFFSET                           \
    0x20400U /*!< Vector Table base offset field. \
...
/* FPU settings ------------------------------------------------------------*/
    ...
    RCC->CR &=
        ~(RCC_CR_HSEON | RCC_CR_HSECSSON | RCC_CR_HSEBYP | RCC_CR_HSEEXT | RCC_CR_HSIDIV | RCC_CR_HSIKERON |
            ...
            */
            ...
            switch (pllsource)
            {
                case 0x01UL: /* HSI used as PLL clock source */
                    ...
                    pllvco =
                        ((float_t)hsivalue / (float_t)pllm) * ((float_t)(uint32_t)(RCC->PLL1DIVR & RCC_PLL1DIVR_PLL1N) +
                                                               (fracn1 / (float_t)0x2000) + (float_t)1);
                    break;

                case 0x02UL: /* CSI used as PLL clock source */
                    pllvco = ((float_t)CSI_VALUE / (float_t)pllm) *
                             ((float_t)(uint32_t)(RCC->PLL1DIVR & RCC_PLL1DIVR_PLL1N) + (fracn1 / (float_t)0x2000) +
                              (float_t)1);
                    break;
