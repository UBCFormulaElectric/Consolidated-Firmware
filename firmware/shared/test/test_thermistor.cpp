...
static_assert(app::therm::ThermistorLUT(0.0f, 1.0f, simple_resistances).size() == 5U, "ThermistorLUT size mismatch at compile time");
...
TEST(ThermistorLUTTest, compile_time_constructor_and_accessors)
{
    ...
    constexpr app::therm::ThermistorLUT lut(starting, resolution, simple_resistances);

    EXPECT_EQ(lut.size(), 5u);
    EXPECT_FLOAT_EQ(lut.starting_temp(), starting);
    EXPECT_FLOAT_EQ(lut.resolution(), resolution);
    EXPECT_EQ(lut.resistances(), simple_resistances);
}

TEST(ThermistorLUTTest, exact_match)
{
    app::therm::ThermistorLUT lut(0.0f, 1.0f, simple_resistances);

    // Temperature = starting_temp + index * resolution
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(100.0f), 0.0f); // idx 0
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(80.0f), 1.0f);  // idx 1
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(60.0f), 2.0f);  // idx 2
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(40.0f), 3.0f);  // idx 3
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(20.0f), 4.0f);  // idx 4
}
...
TEST(ThermistorLUTTest, basic_interpolation_value_checks)
{
    ...
    EXPECT_NEAR(lut.resistanceToTemp(90.0f), 0.5f, 1e-3f);
    EXPECT_NEAR(lut.resistanceToTemp(70.0f), 1.5f, 1e-3f);
    EXPECT_NEAR(lut.resistanceToTemp(50.0f), 2.5f, 1e-3f);
    EXPECT_NEAR(lut.resistanceToTemp(30.0f), 3.5f, 1e-3f);
    ...
    EXPECT_NEAR(lut.resistanceToTemp(85.0f), 0.75f, 1e-3f);
}
...
TEST(ThermistorLUTTest, out_of_range)
{
    ...
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(100.1f), -1.0f); 
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(19.9f), -1.0f);
}
...
TEST(ThermistorLUTTest, single_entry)
{
    ...
    app::therm::ThermistorLUT lut(5.0f, 1.0f, single);

    EXPECT_FLOAT_EQ(lut.resistanceToTemp(100.0f), 5.0f);
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(100.1f), -1.0f);
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(99.9f), -1.0f);
}
...
TEST(ThermistorLUTTest, nan_and_inf)
{
    ...
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(std::numeric_limits<float>::quiet_NaN()), -1.0f);
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(std::numeric_limits<float>::infinity()), -1.0f);
    EXPECT_FLOAT_EQ(lut.resistanceToTemp(-std::numeric_limits<float>::infinity()), -1.0f);
}
...
TEST(ThermistorLUTTest, degenerate_adjacent_equal)
{
    ...
    app::therm::ThermistorLUT lut(0.0f, 1.0f, degenerate);
...
TEST(ThermistorLUTTest, binary_search_edge_cases)
{
    ...
    app::therm::ThermistorLUT lut(0.0f, 1.0f, binary_res);

    // Pick points exactly between array entries specifically to trace through low/mid/high index shifts thoroughly
    EXPECT_NEAR(lut.resistanceToTemp(95.0f), 0.5f, 1e-3f);
    EXPECT_NEAR(lut.resistanceToTemp(75.0f), 2.5f, 1e-3f);
    EXPECT_NEAR(lut.resistanceToTemp(25.0f), 7.5f, 1e-3f);
}
...
TEST(ThermistorLUTTest, precondition_decreasing_resistances)
{
    ...
    app::therm::ThermistorLUT lut(0.0f, 1.0f, non_monotonic);
    ...
    EXPECT_GE(t, 0.0f) << "Violation of monotonicity leads to potentially confusing output just mathematically applied.";
}
