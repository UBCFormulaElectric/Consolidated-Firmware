...
inline constexpr float M_2PI_F = 2.0f * M_PI_F;
...
[[nodiscard]] inline constexpr uint32_t S_TO_MS(const float time_s)
{
...
[[nodiscard]] inline constexpr uint32_t HZ_TO_MS(const uint32_t hz)
{
    return 1000U / hz;
}
...
[[nodiscard]] inline constexpr float RAD_TO_DEG(const float radians)
{
    return radians * 180.0f / M_PI_F;
}

// Convert degrees to radians
[[nodiscard]] inline constexpr float DEG_TO_RAD(const float degrees)
{
    return degrees * M_PI_F / 180.0f;
}

// Convert RPM to radians per second
[[nodiscard]] inline constexpr float RPM_TO_RADS(const float rpm)
{
...
[[nodiscard]] inline constexpr int MOTOR_KMH_TO_RPM(const float kmh)
{
