...
namespace detail
{
     ...
     * Cholesky decomposition : Rewriting an SPD matrix into triangular factors to solve Ax=b efficiently without
     * explicit inverse.
     */
     ...
     */
    template <typename Mat> auto is_positive_definite(const Mat &matrix) -> bool
    {
        const Mat       symmetric = symmetrize(matrix);
    ...
    void assert_covariance_constraints(const StateCov &P0, const StateCov &Q, const MeasurementCov &R)
    {
        ...
        assert(
            is_positive_semidefinite(Q) &&
            "Kalman filter constructor: Q must be positive semidefinite (all eigenvalues must be >= 0).");
...
/**
 * @brief The kalman filter class will serve as a simplification of the extended kalman filter. The idea is that for
 * linear systems (or systems that can be easily linearized) we use the kalman filter rather than the EKF.
 */
 ...
 * those being how many states our system estimates, number of measurements from MEASUREMENTS and how many inputs we
 * have F (NxN) : represents how our states evolve with respect to each other covariance matrixes are always square as
 * they relate variables to each other B (NxU) : represents how our state evolves with respect to the input Q (NxN) :
 * the noise in the model (how the noise in one state effect others) R (MxM) : the noise in the measurements (and how
 * the noises of a measurement affect another measurement) H (MxN) : maps state prediction to measurement inputs S (MxM)
 * : residuial covariacne of how uncertain measurement error is (freaky) K (NxM) : basically decides how much to trust
 * measurement vs prediction
 */
template <typename T, std::size_t STATES, std::size_t MEASUREMENTS, std::size_t INPUTS> class kf
{
  public:
     ...
     */
    explicit kf(const N_N &F, const N_U &B, const N_N &Q, const M_N &H, const M_M &R, const N_1 &x0, const N_N &P0)
...
template <typename T, std::size_t STATES, std::size_t MEASUREMENTS, std::size_t INPUTS> class ekf
{
  ...
  private:
    ...
    void compute_F_eval_states(const U_1 &u)
    {
        ...
        vector valued function of the state functions where the output is a an array of vectors evaluated at the scalar
        input. In our case it is an array of scalar inputs .. p
        */
