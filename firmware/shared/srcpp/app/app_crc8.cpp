// GENERATED WITH: pycrc --model crc-8 --algorithm table-driven --generate c
// C++ port of firmware/shared/src/app/app_crc8.c.
//
// Table and update() must remain byte-identical to the C implementation so
// that on-disk CAN logs written by C++ targets (e.g. Hexray DAM) stay
// decodable by the existing Quintuna-format Python decoder.
#include "app_crc8.hpp"

#include <array>
#include <cstdint>
#include <cstddef>

namespace app::crc8
{
static constexpr std::array<uint8_t, 256> crc_table = { {
    ,
} };

uint8_t update(uint8_t crc, const void *data, size_t data_len)
{
    const auto *d = static_cast<const uint8_t *>(data);
    while (data_len--)
    {
        const uint8_t tbl_idx = static_cast<uint8_t>(crc ^ *d);
        crc                   = crc_table[tbl_idx];
        ++d;
    }
    return crc;
}
} // namespace app::crc8
