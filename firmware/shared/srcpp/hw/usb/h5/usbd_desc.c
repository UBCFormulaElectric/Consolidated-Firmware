...
#define USBD_CONFIGURATION_HS_STRING USBD_CONFIGURATION_FS_STRING
#endif
...
#define USBD_PRODUCT_HS_STRING USBD_PRODUCT_FS_STRING
#endif
#ifndef USBD_PRODUCT_FS_STRING
#error "Please check your user_product_string"
#endif
...
#pragma data_alignment = 4
#endif /* __ICCARM__ */
__ALIGN_BEGIN uint8_t USBD_BOSDesc[USB_SIZ_BOS_DESC] __ALIGN_END = { 0x5, USB_DESC_TYPE_BOS, 0xC, 0x0,
...
static void IntToUnicode(uint32_t value, uint8_t *pbuf, uint8_t len)
{
    ...
    for (idx = 0U; idx < len; idx++)
    {
        if (((value >> 28)) < 0xAU)
        {
            pbuf[2U * idx] = (uint8_t)((value >> 28) + (uint32_t)'0');
        }
        else
        {
            pbuf[2U * idx] = (uint8_t)((value >> 28) + (uint32_t)'A' - 10U);
        }
