...
std::expected<void, ErrorCode> FmacIir::init(const FmacIirConfig& config)
{
    ...
    RETURN_IF_ERR_SILENT(hw_utils_convertHalStatus(HAL_FMAC_FilterPreload(m_handle, const_cast<int16_t*>(config.coefB), config.numBTaps, const_cast<int16_t*>(config.coefA), config.numATaps)));
