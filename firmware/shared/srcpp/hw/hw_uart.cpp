...
{
    return hw::utils::convertHalStatus(HAL_UART_Transmit_DMA(handle, data.data(), static_cast<uint16_t>(data.size())));
}

std::expected<void, ErrorCode> Uart::receiveDma(std::span<uint8_t> data) const
{
    return hw::utils::convertHalStatus(HAL_UART_Receive_DMA(handle, data.data(), static_cast<uint16_t>(data.size())));
}
...
{
    return hw::utils::convertHalStatus(HAL_UART_Transmit_IT(handle, data.data(), static_cast<uint16_t>(data.size())));
}
