...
namespace hw::flash
{
    ...
    inline std::expected<void, ErrorCode>
        programFlash(const uint32_t address, const std::span<const std::byte, 16> buffer)
    {
        return programFlashRetry(address, buffer);
    }
    inline std::expected<void, ErrorCode>
        programFlash(const uint32_t address, const std::span<const std::byte, 32> buffer)
    {
        return programFlashRetry(address, buffer);
    }
} // namespace hw::flash
