...
void hw_pwmInputFreqOnly_tick(PwmInputFreqOnly *const pwm_input)
{
    ...
    else
    {
        ...
        if (curr_rising_edge > prev_rising_edge)
            setFrequency(pwm_input, pwm_input->config->tim_frequency_hz / (float)(curr_rising_edge - prev_rising_edge));
        else if (curr_rising_edge < prev_rising_edge)
            // Occurs when the counter rolls over
            setFrequency(
                pwm_input, pwm_input->config->tim_frequency_hz / (float)(pwm_input->config->tim_auto_reload_reg -
