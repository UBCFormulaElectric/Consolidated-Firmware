...
bool hw_i2c_transmit(const I2cInterface *i2c, uint8_t *tx_buffer, uint16_t tx_buffer_size) {
    if (hw_i2c_is_target_ready(i2c)) {
        return HAL_I2C_Master_Transmit(i2c->i2c_handle, i2c->target_address << 1, tx_buffer, tx_buffer_size, i2c->timeout_ms) == HAL_OK;
    }
    return false;
}
...
bool hw_i2c_receive(const I2cInterface *i2c, uint8_t *rx_buffer, uint16_t rx_buffer_size) {
    if (hw_i2c_is_target_ready(i2c)) {
        return HAL_I2C_Master_Receive(i2c->i2c_handle, i2c->target_address << 1, rx_buffer, rx_buffer_size, i2c->timeout_ms) == HAL_OK;
    }
    return false;
}
