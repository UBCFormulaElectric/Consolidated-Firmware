...
void io_chimera_init(const uint32_t name_gpio, const uint32_t name_adc)
{
    ...
    {
        hw_uart_receive(chimera_uart_device, data, DEBUG_SIZE_MSG_BUF_SIZE);
    }
}
...
void io_chimera_msgRxCallback(void)
{
    if (is_mid_debug_msg)
    {
        ...
        hw_uart_transmit(chimera_uart_device, &tx_packet_size, DEBUG_SIZE_MSG_BUF_SIZE);
        hw_uart_transmit(chimera_uart_device, data, tx_packet_size);
    ...
    hw_uart_receiveCallback(chimera_uart_device, data, rx_packet_size);
}
