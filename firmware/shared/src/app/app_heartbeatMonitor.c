...
void app_heartbeatMonitor_checkIn(const HeartbeatMonitor* hbm) {
    ...
    for (int i = 0; i < hbm->board_count; i++) app_heartbeatBoard_checkIn(&hbm->boards[i]);
}

void app_heartbeatMonitor_broadcastFaults(const HeartbeatMonitor* hbm) {
    if (hbm->block_faults)
        return;
    for (int i = 0; i < hbm->board_count; i++) app_heartbeatBoard_broadcastFaults(&hbm->boards[i]);
}
...
void app_heartbeatMonitor_clearFaults(const HeartbeatMonitor* hbm) {
    for (int i = 0; i < hbm->board_count; i++) app_heartbeatBoard_clearFaults(&hbm->boards[i]);
}
