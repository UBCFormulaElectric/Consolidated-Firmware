...
{
    constexpr uint32_t bms_avail_power_w = 80000;
    constexpr uint32_t inverter_rpm      = 1000;
    ...
    constexpr float torque_limit     = static_cast<float>(bms_avail_power_w / (4 * inverter_rpm));
    const float     torque_to_motors = fminf(torque_limit, MAX_TORQUE_REQUEST_NM);
