...
static void can2RxCallback(const CanMsg *msg)
{
    io_bootHandler_processBootRequest(msg);
...
static void can3RxCallback(const CanMsg *msg)
{
    io_bootHandler_processBootRequest(msg);
    if (io_canRx_filterMessageId_can3(msg->std_id))
    {
        io_canQueue_pushRx(msg);
    }
}
...
CanHandle can2 = { .hcan = &hfdcan3, .bus_num = 2, .receive_callback = can2RxCallback };
CanHandle can3 = { .hcan = &hfdcan2, .bus_num = 3, .receive_callback = can3RxCallback };
