...
namespace app
{
    PID::PID(const Config& conf) :
        ...
        out_max(conf.out_max), out_min(conf.out_min), max_integral(conf.max_integral), min_integral(conf.min_integral), 
        clamp_output(conf.clamp_output), clamp_integral(conf.clamp_integral), back_calculation(conf.back_calculation), feed_forward(conf.feed_forward), 
     ...
     * - Integral anti-wind up in the form of clamping and optional back calculation (requires valid min and max output values)
