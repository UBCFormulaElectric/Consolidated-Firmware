...
void io_power_monitor_refresh()
{
    ...
    hw_i2c_transmit(&pwr_mtr, buffer, 1);
}
...
float io_power_monitor_read_voltage(const PowerRail *voltage_address)
{
    ...
    hw_i2c_transmit(&pwr_mtr, buffer, 1); // change register pointer address

    hw_i2c_receive(&pwr_mtr, buffer, 2); // get raw register data and store it in buffer
...
float io_power_monitor_read_current(const PowerRail *current_address)
{
    ...
    hw_i2c_transmit(&pwr_mtr, buffer, 1); // change register pointer address (Vsense)
...
uint32_t io_power_monitor_alert_status()
{
    ...
    hw_i2c_transmit(&pwr_mtr, buffer, 1); // change register pointer to alert_status

    hw_i2c_receive(&pwr_mtr, buffer, 3); // Gets all the alert statuses
