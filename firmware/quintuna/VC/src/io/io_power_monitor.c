...
{
    voltage_bus voltage_address = {
        .VBAT = 0x0B, .V_BOOST = 0x0C, .V_ACC = 0x0D, .V_EXT = 0x0E
    }; // is this an ok way of doing it?
    return hw_i2c_isTargetReady(&pwr_mon);
}

uint8_t io_power_monitor_read(
    voltage_bus voltage_address) // another way of doing it could be
