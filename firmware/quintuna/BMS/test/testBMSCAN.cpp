...
//     {
//         fakes::irs::setNegativeClosed(metadata.requires_irs_negative_closed);
//         ASSERT_EQ(io_irs_isNegativeClosed(), metadata.requires_irs_negative_closed)
//             << "Expected IRS negative state to be " << (metadata.requires_irs_negative_closed ? "CLOSED" : "OPEN")
//             << " in state: " << metadata.state->name;
//         app_canAlerts_BMS_Fault_CellUndervoltage_set(metadata.requires_fault);
...
//         app_canRx_Debug_CellBalancingRequest_update(metadata.state == app_balancingState_get());
//         app_canRx_Debug_StartCharging_update(metadata.state == app_chargeState_get());
//     }
// };
...
// // {
// //     //
// //     ASSERT_EQ(io_faultLatch_getCurrentStatus(&bms_ok_latch), true) << "Expected BMS OK latch to be OK";
// //     ASSERT_EQ(io_faultLatch_getCurrentStatus(&imd_ok_latch), true) << "Expected IMD OK latch to be OK";
// //     ASSERT_EQ(io_faultLatch_getCurrentStatus(&bspd_ok_latch), true) << "Expected BSPD OK latch to be OK";

// //     LetTimePass(10);

// //     fakes::faultLatches::updateFaultLatch(&bms_ok_latch, false);
// //     fakes::faultLatches::updateFaultLatch(&imd_ok_latch, false);
// //     fakes::faultLatches::updateFaultLatch(&bspd_ok_latch, false);

// //     ASSERT_EQ(io_faultLatch_getCurrentStatus(&bms_ok_latch), false) << "Expected BMS OK latch to be FAULT ";
// //     ASSERT_EQ(io_faultLatch_getLatchedStatus(&bms_ok_latch), false) << "Expected BMS OK latch to be latched as
// //     FAULT"; ASSERT_EQ(io_faultLatch_getCurrentStatus(&imd_ok_latch), false) << "Expected IMD OK latch to be FAULT
// ";
// //     ASSERT_EQ(io_faultLatch_getLatchedStatus(&imd_ok_latch), false) << "Expected IMD OK latch to be latched as
// //     FAULT"; ASSERT_EQ(io_faultLatch_getCurrentStatus(&bspd_ok_latch), false) << "Expected BSPD OK latch to be
// FAULT";
// //     ASSERT_EQ(io_faultLatch_getLatchedStatus(&bspd_ok_latch), false) << "Expected BSPD OK latch to be latched as
// //     FAULT";
...
// //     fakes::faultLatches::updateFaultLatch(&bms_ok_latch, true);
// //     fakes::faultLatches::updateFaultLatch(&imd_ok_latch, true);
// //     fakes::faultLatches::updateFaultLatch(&bspd_ok_latch, true);

// //     ASSERT_EQ(io_faultLatch_getCurrentStatus(&bms_ok_latch), true) << "Expected BMS OK latch to be OK";
// //     ASSERT_EQ(io_faultLatch_getLatchedStatus(&bms_ok_latch), false) << "Expected BMS OK latch to be latched as
// //     FAULT"; ASSERT_EQ(io_faultLatch_getCurrentStatus(&imd_ok_latch), true) << "Expected IMD OK latch to be OK";
// //     ASSERT_EQ(io_faultLatch_getLatchedStatus(&imd_ok_latch), false) << "Expected IMD OK latch to be latched as
// //     FAULT"; ASSERT_EQ(io_faultLatch_getCurrentStatus(&bspd_ok_latch), true) << "Expected BSPD OK latch to be OK";
// //     ASSERT_EQ(io_faultLatch_getLatchedStatus(&bspd_ok_latch), false) << "Expected BSPD OK latch to be latched as
// //     FAULT";
...
//         LetTimePass(10);
//         ASSERT_EQ(io_irs_isPositiveClosed(), true) << "Expected contactors closed in state: " <<
//         metadata.state->name;

//         io_irs_openPositive();
//         LetTimePass(10);
//         ASSERT_EQ(io_irs_isPositiveClosed(), false) << "Expected contactors open in state: " << metadata.state->name;
//     }
// }
