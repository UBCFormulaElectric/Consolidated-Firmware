...
void PwmInputs::tick()
{
    uint32_t ic_rising_edge = HAL_TIM_ReadCapturedValue(htim, rising_edge_tim_channel);

    if (ic_rising_edge != 0U)
    {
        uint32_t ic_falling_edge = HAL_TIM_ReadCapturedValue(htim, falling_edge_tim_channel);
