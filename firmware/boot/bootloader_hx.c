...
void bootloader_boardSpecific_program(uint32_t address, uint64_t data)
{
    ...
    // data. So we keep track of the data to program in a buffer in RAM, until the buffer is full, and then we can write
    // the entire flash word. This implementation only works if the binary size is a multiple of 32 bytes, or the buffer
    // won't fill up for the last few bytes so won't be written into flash. This is guaranteed by canup.
