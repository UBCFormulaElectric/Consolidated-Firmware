...
template <Decimal T>
inline float inner_wheel_ang_rad(const T steer_ang_rad)
{
...
template <Decimal T>
inline float outer_wheel_ang_rad(const T steer_ang_rad)
{
...
{
template <Decimal T>
[[nodiscard]] wheel_set<T> wheel_steer_angles(const T steer_ang_rad)
{
    const T steer_ang_clamped_rad =
        std::clamp(steer_ang_rad, -static_cast<T>(vd_constants::STEER_WHEEL_RANGE_rad), static_cast<T>(vd_constants::STEER_WHEEL_RANGE_rad));
    /**
