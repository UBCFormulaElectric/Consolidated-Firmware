...
std::expected<void, ErrorCode> update()
{
    ...
    if (mask.has_value()) {
        ...
        LOG_INFO("Balance status: Difference =%umV  Balancing mask=0x%04X", static_cast<unsigned>(diff_mV), static_cast<uint16_t>(mask.value()));
    }

    const auto cell_balancing_time = io::batteryMonitoring::read_balacing_time_seconds();
    cell_balancing_time.has_value() ?  LOG_INFO("Cell Balance in seconds=%u", static_cast<unsigned>(cell_balancing_time.value())) : LOG_WARN("Cell Balance read failed");
