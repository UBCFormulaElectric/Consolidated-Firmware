...
static inline tire_forces app_frictionCircle_calcMaxTractiveForce(road_tire_fric_coeff friction_coeffs, tire_forces normal_forces);
...
VD_frictionCircle_outputs app_frictionCircle_estimate()
{
    ...
    return (VD_frictionCircle_outputs){
        .maximum_tractive_forces = app_frictionCircle_calcMaxTractiveForce(inputs.tire_fric_coeffs, inputs.tire_normal_forces),
        .required_Fy = app_frictionCircle_calcRequired_Fy(inputs),
        .avail_Fx = app_frictionCircle_calcAvailable_Fx(max_tractive_forces, required_Fy)
    };
}

/************************* Static Function Definition *****************************/
static VD_frictionCircle_inputs app_frictionCircle_inputs(ImuData imu_data, double desired_yawRate, double desired_yawMoment, double vehicle_velocity, double body_slip, road_tire_fric_coeff tire_fric_coeffs, tire_forces tire_normal_forces)
{
    return (VD_frictionCircle_inputs){
        .imu_data = imu_data,
...
static tire_forces app_frictionCircle_calcRequired_Fy(VD_frictionCircle_inputs inputs)
{
    ...
    const double body_lat_force = (CAR_MASS_AT_CG_KG * inputs.vehicle_velocity * (inputs.imu_data->yaw_rate - body_slip_rate_of_change));
