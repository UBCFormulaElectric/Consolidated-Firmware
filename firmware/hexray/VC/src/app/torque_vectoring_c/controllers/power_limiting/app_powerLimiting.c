...
static void app_powerLimiting_broadCast(powerLimiting_outputs out) static inline double app_powerLimiting_calcMaxPower(
    bool isRegen_on);
...
static powerLimiting_inputs app_powerLimiting_getInputs(VD_TorqueToInv requested_torques, VD_WheelRpms wheel_rpms)
{
    const powerLimiting_inputs inputs = {
        ...
        .bms_power_limit =
            inputs.isRegen_on ? app_canRx_BMS_ChargePowerLimit_get() : app_canRx_BMS_DischargePowerLimit_get(),
    };
