...
void broadcast()
{
    ...
    // Status msg
    app::can::rx::VC_EllipseGeneralStatusBitmask_set(io_sbgEllipse_getGeneralStatus());
    app::can::rx::VC_EllipseComStatusBitmask_set(io_sbgEllipse_getComStatus());
    app::can::rx::VC_EllipseQueueOverflowCount_set(io_sbgEllipse_getOverflowCount());

    // Time msg
    constexpr uint32_t timestamp_us = io_sbgEllipse_getTimestampUs();
    app::can::rx::VC_EllipseTimestamp_set(timestamp_us);

    const float ekf_vel_N_accuracy = 0;
    const float ekf_vel_E_accuracy = 0;
    const float ekf_vel_D_accuracy = 0;

    app::can::rx::VC_VelocityNorth_set(ekf_vel_N);
    app::can::rx::VC_VelocityEast_set(ekf_vel_E);
    app::can::rx::VC_VelocityDown_set(ekf_vel_D);

    app::can::rx::VC_VelocityNorthAccuracy_set(ekf_vel_N_accuracy);
    app::can::rx::VC_VelocityEastAccuracy_set(ekf_vel_E_accuracy);
    app::can::rx::VC_VelocityDownAccuracy_set(ekf_vel_D_accuracy);
    ...
    {
        //app::can::rx::VC_EkfSolutionMode_set(ekf_solution_mode);
    }
    ...
    app::can::rx::VC_EulerAnglesRoll_set(euler_roll);
    app::can::rx::VC_EulerAnglesPitch_set(euler_pitch);
    app::can::rx::VC_EulerAnglesYaw_set(euler_yaw);
}
