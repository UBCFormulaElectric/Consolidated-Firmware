...
class RSMChimeraConfig : public chimera_v2::config
{
  public:
    ...
    std::optional<std::reference_wrapper<const hw::i2c>> id_to_i2c(const _I2cNetName *inn) const override
    {
        ...
        {
            case rsm_I2cNetName_I2C_R_PUMP return std::cref(r_pump); default:
...
public:
            RSMChimeraConfig()
            {
                ...
                i2c_net_name_tag = I2cNetName_rsm_net_name_tag spi_net_name_tag = SpiNetName_rsm_net_name_tag;
            }
        }
        rsm_config;
