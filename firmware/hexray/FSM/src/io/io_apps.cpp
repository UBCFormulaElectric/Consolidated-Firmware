...
constexpr float PAPPS_LEN_A = 175.0f; // Primary sensor (PAPPS) distance
constexpr float SAPPS_LEN_A = 165.0f; // Secondary sensor (SAPPS) distance
constexpr float LEN_B       = 90.61f; // Horizontal distance to sensors

// Potentiometer length limits (in mm)
constexpr float MAX_POT_LENGTH = 220.0f;
constexpr float MIN_POT_LENGTH = 145.0f;

// ADC voltage limits (in V)
constexpr float MAX_POT_V = 3.30f;
// Note: Minimum pot voltage is assumed to be 0V

// Raw Voltage Calibration Values for PAPPS (Primary)
constexpr float PAPPS_FULL_PRESSED_POT_V = 2.57f; // Fully pressed position voltage
constexpr float PAPPS_UNPRESSED_POT_V    = 0.86f; // Unpressed position voltage

// Raw Voltage Calibration Values for SAPPS (Secondary)
constexpr float SAPPS_FULL_PRESSED_POT_V = 3.08f;
constexpr float SAPPS_UNPRESSED_POT_V    = 1.38f;

// Cosine Law Pre-computed Parameters for Angle Calculation
constexpr float PAPPS_COS_LAW_DENOMINATOR = 2.0f * PAPPS_LEN_A * LEN_B; // For PAPPS sensor
constexpr float PAPPS_COS_LAW_COEFFICIENT = (PAPPS_LEN_A * PAPPS_LEN_A + LEN_B * LEN_B) / PAPPS_COS_LAW_DENOMINATOR;
constexpr float SAPPS_COS_LAW_DENOMINATOR = 2.0f * SAPPS_LEN_A * LEN_B; // For SAPPS sensor
constexpr float SAPPS_COS_LAW_COEFFICIENT = (SAPPS_LEN_A * SAPPS_LEN_A + LEN_B * LEN_B) / SAPPS_COS_LAW_DENOMINATOR;
...
constexpr float PAPPS_MAX_V = PAPPS_FULL_PRESSED_POT_V + 0.5f;
constexpr float SAPPS_MIN_V = SAPPS_UNPRESSED_POT_V - 0.5f;
constexpr float SAPPS_MAX_V = SAPPS_FULL_PRESSED_POT_V + 0.5f;
