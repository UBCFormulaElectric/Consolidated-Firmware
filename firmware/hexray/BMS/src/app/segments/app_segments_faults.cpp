  ...
  public:
    ...
    bool check(bool blanking_expired)
    {
        ...
        for (size_t seg = 0; seg < io::NUM_SEGMENTS; ++seg)
        {
            ...
            {
                comm_err = comm_err || !app::segments::cell_voltage_success[seg][cell] || !app::segments::filtered_cell_voltage_success[seg][cell];
        ...
        const bool uv = under_voltage_timer_.runIfCondition(under_voltage_condition) ==
                Timer::TimerState::EXPIRED &&
            blanking_expired;
        const bool ov =
            over_voltage_timer_.runIfCondition(over_voltage_condition) ==
                Timer::TimerState::EXPIRED &&
            blanking_expired;
        const bool ot =
            over_temp_timer_.runIfCondition(over_temp_condition) ==
                Timer::TimerState::EXPIRED &&
            blanking_expired;
        const bool ce =
            comm_err_timer_.runIfCondition(comm_err) == Timer::TimerState::EXPIRED &&
            blanking_expired;
        const bool cow =
            cell_owc_timer_.runIfCondition(any_cell_owc) == Timer::TimerState::EXPIRED &&
            blanking_expired;
        const bool tow =
            therm_owc_enabled &&
...
{
    const bool settled =
        startup_blanking_timer.updateAndGetState() == Timer::TimerState::EXPIRED;
