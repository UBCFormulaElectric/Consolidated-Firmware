...
{
    array<array<array<uint16_t, io::adbms::THERM_GPIOS_PER_SEGMENT>, io::NUM_SEGMENTS>, static_cast<size_t>(ThermistorMux::THERMISTOR_MUX_COUNT)> regs;
    array<array<array<expected<void, ErrorCode>, io::adbms::THERM_GPIOS_PER_SEGMENT>, io::NUM_SEGMENTS>, static_cast<size_t>(ThermistorMux::THERMISTOR_MUX_COUNT)> regs_success;
        ...
        for (size_t mux = 0U; mux < static_cast<size_t>(ThermistorMux::THERMISTOR_MUX_COUNT); mux++) {
            for (size_t gpio = 0U; gpio < io::adbms::THERM_GPIOS_PER_SEGMENT; gpio++) {
                ...
                if (therm < io::THERMISTORS_PER_SEGMENT) {
                    out_temps[seg][therm] = regs_success[mux][seg][gpio] ? convertRegToVoltage(regs[mux][seg][gpio]) : -0.1f;
    ...
    CellSuccess &out_success)
{
    ...
    for (size_t seg = 0; seg < io::NUM_SEGMENTS; seg++) {
        for (size_t cell = 0; cell < io::CELLS_PER_SEGMENT; cell++) {
            ...
            float owc_v                 = (cell % 2 == 0) ? convertRegToVoltage(owc_odd_regs[seg][cell]) : convertRegToVoltage(owc_even_regs[seg][cell]);
