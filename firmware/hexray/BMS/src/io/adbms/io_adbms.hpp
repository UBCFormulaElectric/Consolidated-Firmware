...
namespace adbms
{
     ...
     */
    result<void> writeConfigurationRegisters(const SegmentConfig config[io::NUM_SEGMENTS]);
    ...
    void readVoltageRegisters(
        ...
        result<void> comm_success[io::NUM_SEGMENTS][io::CELLS_PER_SEGMENTS]);

    /**
     * sends a command to read all voltages from all segments
     * @return success of the operation
     */
    result<void> startCellsAdcConversion(void);
     ...
     * @return success of the operation
     */
    result<void> pollAdcConversions(void);
