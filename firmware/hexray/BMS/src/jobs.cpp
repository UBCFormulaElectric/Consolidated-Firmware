...
void jobs_run100Hz_tick()
{
    ...
    const bool imd_latched_status  = imd_ok_latch.getLatchedStatus() == FaultLatchState::OK;
    const bool bspd_latched_status = bspd_ok_latch.getLatchedStatus() == FaultLatchState::OK;
    const bool bms_latched_status  = bms_ok_latch.getLatchedStatus() == FaultLatchState::OK;

    app::can_alerts::faults::ImdLatched_set(not imd_latched_status);
    app::can_alerts::faults::HardwareBspdLatched_set(not bspd_latched_status);
    ...
    app::can_tx::BMS_BmsLatchOk_set(bms_latched_status);
    app::can_tx::BMS_ImdLatchOk_set(imd_latched_status);
    app::can_tx::BMS_BspdLatchOk_set(bspd_latched_status);
