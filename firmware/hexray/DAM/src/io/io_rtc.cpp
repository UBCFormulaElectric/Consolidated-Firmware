...
result<void> set_time(const Time &time)
{
    ...
    result<void> status = hw::utils::convertHalStatus(HAL_RTC_SetTime(&hrtc, &rtcTime, RTC_FORMAT_BCD));
    if (!status)
    {
...
result<void> set_date(const Date &date)
{
    ...
    result<void> status = hw::utils::convertHalStatus(HAL_RTC_SetDate(&hrtc, &rtcDate, RTC_FORMAT_BCD));
    if (!status)
    {
...
result<Time> get_time(Time &time)
{
    ...
    result<void> status = hw::utils::convertHalStatus(HAL_RTC_GetTime(&hrtc, &rtcTime, RTC_FORMAT_BCD));
    if (!status)
        return std::unexpected(status.error());

    status = hw::utils::convertHalStatus(HAL_RTC_GetDate(&hrtc, &rtcDate, RTC_FORMAT_BCD));
    if (!status)
        return std::unexpected(status.error());
...
result<Date> get_date(Date &date)
{
    ...
    // You must call HAL_RTC_GetDate() after HAL_RTC_GetTime() to unlock the values in the
    // higher-order calendar shadow registers to ensure consistency between the time and date values.
    result<void> status = hw::utils::convertHalStatus(HAL_RTC_GetTime(&hrtc, &dummy, RTC_FORMAT_BCD));
    if (!status)
        return std::unexpected(status.error());
