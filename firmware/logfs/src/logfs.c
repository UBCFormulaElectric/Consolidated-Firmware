...
LogFsErr logfs_write(LogFs *fs, LogFsFile *file, void *buf, const uint32_t size)
{
    CHECK_ARG(fs);
    CHECK_ARG(file);
    CHECK_ARG(buf);
...
LogFsErr logfs_writeMetadata(LogFs *fs, LogFsFile *file, void *buf, const uint32_t size)
{
    CHECK_ARG(fs);
    CHECK_ARG(file);
    CHECK_ARG(buf);
