...
TEST_F(YawRateControllerTest, torque_distribution_lambda_is_one)
{
    ...
    yrc.real_yaw_rate_deg = RAD_TO_DEG(
        KMH_TO_MPS(vehicle_speed) /
        ((WHEELBASE_mm * MM_TO_M) / (tanf(DEG_TO_RAD(steering_angle * APPROX_STEERING_TO_WHEEL_ANGLE)))));
    ...
    float torque_fl_Nm_test = (0.25f * yrc.requested_torque) + (0.5f * (1 - lambda) * yrc.yaw_moment);
    float torque_fr_Nm_test = (0.25f * yrc.requested_torque) - (0.5f * (1 - lambda) * yrc.yaw_moment);
    float torque_rl_Nm_test = (0.25f * yrc.requested_torque) + (0.5f * lambda * yrc.yaw_moment);
    float torque_rr_Nm_test = (0.25f * yrc.requested_torque) - (0.5f * lambda * yrc.yaw_moment);
    // std::cout << "ref_yaw_rate: " << yrc.ref_yaw_rate_deg << "real_yaw_rate" << yrc.real_yaw_rate_deg <<
    // "yaw_rate_diff" << yaw_rate_diff << "\n"; std::cout << "ref_yaw_rate: " << yrc.ref_yaw_rate_deg << " yaw_moment:
    // " << yrc.yaw_moment << " lambda: " << lambda << " torque_left: " << torque_left_Nm_test << " torque_right: " <<
    // torque_right_Nm_test << "\n";
    ...
    ASSERT_FLOAT_EQ(torque_fl_Nm_test, yrc.torque_fl_Nm);
    ASSERT_FLOAT_EQ(torque_fr_Nm_test, yrc.torque_fr_Nm);
    ASSERT_FLOAT_EQ(torque_rl_Nm_test, yrc.torque_rl_Nm);
    ASSERT_FLOAT_EQ(torque_rr_Nm_test, yrc.torque_rr_Nm);
};
