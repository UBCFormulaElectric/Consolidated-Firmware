...
static TVInteralStates torqueVectoringState;
static TVInteralStates torqueVectoringNextState;
...
static void app_torqueVectoring_stateMachine(float accelerator_pedal_percentage)
{
    ...
    // if (accelerator_pedal_percent <= PEDAL_NOT_PRESSED && torqueVectoringState != OFF)
    // {
    //     torqueVectoringState = noPedalInput();
    //     return;
    // }

    // switch (torqueVectoringState)
    // {
    //     case OFF:

    //         if (accelerator_pedal_percent > 0.0f)
    //         {
    //             if (TRACTION_CONTROL_LOWER_BOUND <= steering_angle_deg &&
    //                 steering_angle_deg <= TRACTION_CONTROL_UPPER_BOUND)
    //             {
    //                 torqueVectoringState = runTractionControl(power_limit);
    //             }
    //             else
    //             {
    //                 torqueVectoringState = runActiveDiff(power_limit);
    //             }
    //         }
    ...
    //         // we do not nessecarily want hystersis to enter active diff from traction control as if we increase the
    //         // window for to be activate for a larger angle window we would be further assuming that there are little
    //         // lateral forces in play at a steering angle where they very much may be in play
    //         if (TRACTION_CONTROL_LOWER_BOUND <= steering_angle_deg &&
    //             steering_angle_deg <= TRACTION_CONTROL_UPPER_BOUND)
    //         {
    //             torqueVectoringState = runTractionControl(power_limit);
    ...
    //             if ((TRACTION_CONTROL_LOWER_BOUND + STEERING_ANG_HYSTERSIS) <= steering_angle_deg &&
    //                 steering_angle_deg <= (TRACTION_CONTROL_UPPER_BOUND - STEERING_ANG_HYSTERSIS))
    //             {
    //                 torqueVectoringState = runTractionControl(power_limit);
    //             }
    //         }
    ...
    case OFF:
        if (pedal > 0) {
            if(abs(steer) < 10)
                torqueVectoringNextState = TRACTION_CONTROL;
            else
                torqueVectoringNextState = ACTIVE_DIFF;
        }
        break;
    case ACTIVE_DIFF:
    ...
    //handle edges
    if(torqueVectoringNextState != torqueVectoringState) {
    ...
    case OFF:
        ...
        break;
    case ACTIVE_DIFF:
        runActiveDiff(power_limit);
        break;
    case TRACTION_CONTROL:
        runTractionControl(power_limit);
        break;
    default:
        break;
    }

    app_canTx_VC_TVInternalState_set(torqueVectoringState);
}
...
static TVInteralStates runTractionControl(float power_limit)
{
    if (torqueVectoringState != TRACTION_CONTROL)
    {
        pid_traction_control.integral =
            0; // WE DO NOT WANT TO KEEP RESETING ACCUMULATED ERROR TO 0 WHILE WE ARE CONTINUOUSLY IN TRACTION CONTORL
    }
