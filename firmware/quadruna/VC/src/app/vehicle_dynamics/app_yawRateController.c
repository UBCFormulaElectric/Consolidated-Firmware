...
{
    float ref_yaw_rate_rad = (KMH_TO_MPS(yrc->vehicle_velocity_kmh) * DEG_TO_RAD(yrc->wheel_angle_deg * APPROX_STEERING_TO_WHEEL_ANGLE)) /
           ((WHEELBASE_mm * MM_TO_M) + (yrc->ku * (KMH_TO_MPS(yrc->vehicle_velocity_kmh)) * (KMH_TO_MPS(yrc->vehicle_velocity_kmh))));
...
void app_yawRateController_computeTorque(YawRateController *yrc)
{
    ...
    yrc->torque_left_Nm  = (0.5f * yrc->requested_torque) + 0.5f * lambda * yrc->yaw_moment;
    yrc->torque_right_Nm = (0.5f * yrc->requested_torque) - 0.5f * lambda * yrc->yaw_moment;
}   