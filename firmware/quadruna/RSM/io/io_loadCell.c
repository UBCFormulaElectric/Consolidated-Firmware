...
bool io_loadCell_OCSC(int adcPin_toCheck)
{
    const float voltage = hw_adc_getVoltage(ADC1_IN10_LC3_OUT);
    return !(LOADCELL_MINVOLT <= voltage && voltage <= LOADCELL_MAXVOLT);
}

float io_loadCell_getVoltage3(void)
{
    return hw_adc_getVoltage(ADC1_IN10_LC3_OUT);
}

float io_loadCell_getVoltage4(void)
{
    return hw_adc_getVoltage(ADC1_IN10_LC4_OUT);
}
