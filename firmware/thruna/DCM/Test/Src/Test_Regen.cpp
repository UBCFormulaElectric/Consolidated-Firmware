...
TEST_F(TestRegen, active_differential)
{
    ...
    float actual_delta                = TRACK_WIDTH_mm * tanf(DEG_TO_RAD(steering_angle)) / (2 * WHEELBASE_mm);
    float actual_left_torque_request  = torque_request * (1 + actual_delta);
    float actual_right_torque_request = torque_request * (1 - actual_delta);

    App_ActiveDifferential_ComputeNegativeTorque(&inputs, &regenAttributes, torque_request);

    ASSERT_FLOAT_EQ(actual_left_torque_request, regenAttributes.left_inverter_torque_Nm);
    ASSERT_FLOAT_EQ(actual_right_torque_request, regenAttributes.right_inverter_torque_Nm);
}
...
TEST_F(TestRegen, battery_over_temp_torque_request)
{
    ...
    App_Run_Regen(&prev_torque_request, pedal_percentage);
    ...
    ASSERT_FLOAT_EQ(0.0, prev_torque_request);
...
TEST_F(TestRegen, battery_full_torque_request)
{
    ...
    ASSERT_FLOAT_EQ(4.1f, regenAttributes.current_battery_level);
}

TEST_F(TestRegen, regular_active_differential_torque_request)
{
    static float prev_torque_request = -20.0f;
    ...
    // make sure RegenNotAvailable alert occurs
    bool  alert                      = App_CanAlerts_DCM_Warning_RegenNotAvailable_Get();
    float actual_torque_request      = MAX_REGEN_nm * pedal_percentage / MAX_PEDAL_POSITION;
    float actual_delta                = TRACK_WIDTH_mm * tanf(DEG_TO_RAD(steering_angle)) / (2 * WHEELBASE_mm);
    float actual_left_torque_request  = actual_torque_request * (1 + actual_delta);
    float actual_right_torque_request = actual_torque_request * (1 - actual_delta);

    ASSERT_TRUE(alert == false);
    ASSERT_FLOAT_EQ(actual_torque_request, prev_torque_request);
    ASSERT_FLOAT_EQ(actual_left_torque_request, App_CanTx_DCM_LeftInverterTorqueCommand_Get());
    ASSERT_FLOAT_EQ(actual_right_torque_request, App_CanTx_DCM_RightInverterTorqueCommand_Get());
}

// tapers torque request due to being too close to a full battery
TEST_F(TestRegen, limit_change_in_torque)
{
  static float prev_torque_request = -15.0f;
  float        pedal_percentage    = -65.0f; 
  ...
  float actual_torque_request      = MAX_REGEN_nm * pedal_percentage / MAX_PEDAL_POSITION; 
  float torqueChange = actual_torque_request - prev_torque_request;
  actual_torque_request = prev_torque_request + (torqueChange) / (float)(fabs(torqueChange)) * MAX_TORQUE_CHANGE;
...
TEST_F(TestRegen, taper_torque_request)
{
    ...
    // make sure RegenNotAvailable alert occurs
    bool  alert                      = App_CanAlerts_DCM_Warning_RegenNotAvailable_Get();
    float actual_torque_request      = MAX_REGEN_nm * pedal_percentage / MAX_PEDAL_POSITION * 0.75f;
    float actual_delta                = TRACK_WIDTH_mm * tanf(DEG_TO_RAD(steering_angle)) / (2 * WHEELBASE_mm);
    float actual_left_torque_request  = actual_torque_request * (1 + actual_delta);
    float actual_right_torque_request = actual_torque_request * (1 - actual_delta);
...
TEST_F(TestRegen, active_differential_exceeds_max) {   static RegenBraking              regenAttributes = { .enable_active_differential = true };
    static ActiveDifferential_Inputs inputs;

    float pedal_percentage    = -100.0f;
    float torque_request      = MAX_REGEN_nm * pedal_percentage / MAX_PEDAL_POSITION;
    float steering_angle      = -43.0f;
    inputs.steering_angle_deg = steering_angle;

    float actual_delta                = TRACK_WIDTH_mm * tanf(DEG_TO_RAD(steering_angle)) / (2 * WHEELBASE_mm);
    float actual_left_torque_request  = torque_request * (1 + actual_delta);
    float actual_right_torque_request = torque_request * (1 - actual_delta);
    float torque_max_Nm   = fmaxf(actual_left_torque_request, actual_right_torque_request);

    float scale = MAX_REGEN_nm / torque_max_Nm;

    App_ActiveDifferential_ComputeNegativeTorque(&inputs, &regenAttributes, torque_request);

    ASSERT_FLOAT_EQ(actual_left_torque_request * scale, regenAttributes.left_inverter_torque_Nm);
    ASSERT_FLOAT_EQ(actual_right_torque_request * scale, regenAttributes.right_inverter_torque_Nm);
}