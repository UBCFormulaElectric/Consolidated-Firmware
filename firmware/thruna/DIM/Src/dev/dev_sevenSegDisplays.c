...
typedef struct
{
    ...
    Gpio *   dimming_pin;
    Gpio *   ser_out_pin;
    const SevenSegDigitsConfig* config;
...
static SevenSegDigits seven_seg_digits;

void dev_sevenSegDigits_init(const SevenSegDigitsConfig* config)
{
    seven_seg_digits.config = config;
    memset(seven_seg_digits.commands, 0U, sizeof(seven_seg_digits.commands));

    // SRCK rising edge transfers 1 bit, so start low
    hw_gpio_writePin(seven_seg_digits.config->srck_pin, false);

    // RCK pin is normally held low
    hw_gpio_writePin(seven_seg_digits.config->rck_pin, false);

    // Set 7-segs to full brightness
    hw_gpio_writePin(seven_seg_digits.config->dimming_pin, false);
}
...
void dev_sevenSegDigits_writeCommands(void)
{
    ...
    for (int display = NUM_SEVEN_SEGS - 1; display >= 0; display++)
    {
        ...
        {
            const bool bit_state = IS_BIT_SET(seven_seg_digits.commands[display], i);

            // Write bit state to data line
            hw_gpio_writePin(seven_seg_digits.config->ser_out_pin, bit_state);

            // Pulse clock to shift data in
            hw_gpio_writePin(seven_seg_digits.config->srck_pin, true);
            hw_gpio_writePin(seven_seg_digits.config->srck_pin, false);
        }
    }
    ...
    hw_gpio_writePin(seven_seg_digits.config->rck_pin, true);
    hw_gpio_writePin(seven_seg_digits.config->rck_pin, false);
}
...
void dev_sevenSegDigits_setValue(SevenSegPosition position, HexDigit digit, bool decimal_point)
{
    ...
    seven_seg_digits.commands[position] = command_value;
}

void dev_sevenSegDigits_disable(SevenSegPosition position)
{
    seven_seg_digits.commands[position] = command_lookup_table.disable;
}