...
void App_FaultStateRunOnEntry(struct StateMachine *const state_machine)
{
    ...
    App_CanAlerts_FSM_Fault_StateMachine_Set(true);
}
...
void App_FaultStateRunOnTick100Hz(struct StateMachine *state_machine)
{
    ...
    const bool hb_ok = !App_CanAlerts_FSM_Fault_MissingHeartbeat_Get();
    if (hb_ok)
    {
...
{
    UNUSED(state_machine);
    App_CanAlerts_FSM_Fault_StateMachine_Set(false);
}
