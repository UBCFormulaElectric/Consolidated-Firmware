...
void App_FaultStateRunOnEntry(struct StateMachine *const state_machine)
{
    ...
    App_CanAlerts_SetFault(StateMachineFault, true);
}
...
void App_FaultStateRunOnTick100Hz(struct StateMachine *state_machine)
{
    ...
    const bool hb_ok = !App_CanAlerts_GetFault(MissingHeartbeatFault);
    if (hb_ok)
    {
...
{
    UNUSED(state_machine);
    App_CanAlerts_SetFault(StateMachineFault, false);
}
