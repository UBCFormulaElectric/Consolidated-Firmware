...
class BmsStateMachineTest : public BaseStateMachineTest
{
  protected:
    ...
    {
        return std::vector<const struct State *>{ App_GetInitState(),  App_GetPreChargeState(),
                                                  App_GetDriveState(), App_GetChargeState(),
                                                  App_GetFaultState(), App_GetInverterOnState() };
    }
...
TEST_F(BmsStateMachineTest, check_inverter_on_state_is_broadcasted_over_can)
{
    ...
    EXPECT_EQ(BMS_INVERTER_ON_STATE, App_CanTx_BMS_Vitals_CurrentState_Get());
}
...
TEST_F(BmsStateMachineTest, check_state_transition_from_init_to_inverter_to_precharge)
{
    ...
    EXPECT_EQ(BMS_INVERTER_ON_STATE, App_CanTx_BMS_Vitals_CurrentState_Get()) << "Expected state: BMS_INVERTER_STATE";
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (test_params[i].expect_precharge_starts)
        {
            ...
            else
            {
                ...
                // TODO: failing when i = 3 it stays in drive state should be init
                LetTimePass(state_machine, 200);
                ASSERT_EQ(App_GetInitState(), App_SharedStateMachine_GetCurrentState(state_machine));
                ASSERT_EQ(close_air_positive_fake.call_count, 0);
