...
static void ChargeStateRunOnTick100Hz(struct StateMachine *const state_machine)
{
    if (App_AllStatesRunOnTick100Hz(state_machine))
    {
        ...
        App_CanAlerts_ChargerFault_Set(has_charger_faulted);
        ...
        if (!is_charger_connected || external_shutdown_occurred)
        {
            App_SharedStateMachine_SetNextState(state_machine, App_GetFaultState());
            App_CanRx_Debug_StartCharging_Update(false);
            App_CanAlerts_ChargerDisconnectedDuringChargeFault_Set(!is_charger_connected);
            App_CanAlerts_ChargerExternalShutdownFault_Set(external_shutdown_occurred);
        }
