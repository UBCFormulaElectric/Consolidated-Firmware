...
void Squircle::paint(QPainter *p) {
  ...
  const double radius = std::clamp(static_cast<double>(this->m_radius), 0.0, std::min(bounds.width(), bounds.height()) / 2.0);
  ...
  const double corner_dim = radius * (1 + smoothness),
               ...
               s = sin(M_PI_4 * smoothness), c = cos(M_PI_4 * smoothness),
               f_1 = radius * (1 - s),
               f_2 = radius *
                     (1 - c), // f1 is the long side, f2 is the short side
  ...
  for (const struct {
    ...
    path.lineTo(startPos.x() + isH * straightWidthLength * -xt,
                startPos.y() + !isH * straightHeightLength * -yt);

    // define the points
    const QPointF p1_1(bx + corner_dim * xt, by),
        p4_1(bx + f_1 * xt, by + f_2 * yt), p3_1(bx + three_shift * xt, by),
        p2_1((p1_1.x() + 2 * p3_1.x()) / 3, by), p1_2(bx, by + corner_dim * yt),
        p4_2(bx + f_2 * xt, by + f_1 * yt), p3_2(bx, by + three_shift * yt),
        p2_2(bx, (p1_2.y() + 2 * p3_2.y()) / 3);

    const double unCorrectedArcAngle =
                     RAD_TO_DEG(atan2(abs(by + radius * yt - p4_1.y()),
                                      abs(bx + radius * xt - p4_1.x()))),
    ...
    if (isH) {
      ...
      path.arcTo(bx + (xt - 1) * radius, by + (yt - 1) * radius, radius * 2,
                 radius * 2, arcAngle + angleOffset, (arcAngle - 45) * -2);
