...
def find_sd_card(sd_device: str):
	...
	candidate_partitions = list(filter(lambda d: d.device == sd_device and "removable" in d.opts.split(","), psutil.disk_partitions()))
	if len(candidate_partitions) != 1:
		if len(candidate_partitions) == 0:
			raise NoSDCard()
		else:
			logger.critical(f"Found multiple for {sd_device}, namely {candidate_partitions}")
...
def list_sd_cards():
	removable_cards = [i for i in psutil.disk_partitions() if "removable" in i.opts.split(",")]
