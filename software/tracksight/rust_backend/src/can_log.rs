//! Parsing of the raw CAN logs written to the SD card by `firmware/shared/src/io/io_canLogging.c`.
//! Kept free of crate-internal dependencies so the `logfs_cmd` binary can include it too.
...
pub fn parse_can_log_frames(metadata: &[u8], data: &[u8]) -> Vec<CanLogFrame> {
    ...
    while i < data.len() {
        ...
        if msg_end >= data.len() {
            // Not enough bytes for the payload + CRC; resync on the next magic byte.
            i += 1;
            continue;
        }
        ...
        if CRC8.checksum(&data[i..msg_end]) != rxed_crc {
            // Corrupt frame (or a stray 0xBA in a payload); resync on the next magic byte.
            i += 1;
            continue;
        }
        i = msg_end + 1;
