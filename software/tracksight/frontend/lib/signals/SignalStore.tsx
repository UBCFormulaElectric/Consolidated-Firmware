...
export type SignalStoreReturnType<T extends SignalMetadata> = T["type"] extends SignalType.ENUM ? LODAwareEnumSeries : T["type"] extends SignalType.NUMERICAL ? LODAwareNumericalSeries : T["type"] extends SignalType.ALERT ? LODAwareAlertSeries : never;
...
abstract class SignalStore {
    ...
    mergeHistoricalPoints(signalName: string, resolutionMs: number, requestStartMs: number, requestEndMs: number, points: LevelPoint[]): void {
        ...
        const existingValues = lod.data instanceof SeriesData ? Array.from({ length: lod.data.length }, (_, i) => (lod.data as SeriesData).get(i)) : (lod.data as number[]);
