...
export function useSubscribers(socket: Socket, pruneSignalData: (name: string) => void, clearAllData: () => void) {
    ...
    const activeSignals = useMemo(() => Object.keys(signalSubscriberCount.current).filter((k) => signalSubscriberCount.current[k] > 0), [tick]);
        ...
        const onConnect = () => {
            ...
            names.forEach((name) => {
                fetch(`${BACKEND_URL}/api/v1/subscribe`, {
                    ...
                    .catch((e) => {
                        DEBUG && console.warn(`[subs] resubscribe error for ${name}`, e);
                    });
            });
        };
