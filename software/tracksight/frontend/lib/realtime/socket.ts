...
import subscribeToSignal from './subscribeToSignal';
import unsubscribeFromSignal from './unsubscribeFromSignal';
...
const processPendingOperations = async () => {
  ...
  if (pendingUnsubscriptions.size > 0) {
    ...
    const unsubscriptionsToProcess = Array
      .from(pendingUnsubscriptions)
      .forEach(unsubscribeFromSignal);
