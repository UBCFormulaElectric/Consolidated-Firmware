...
const EnumerationGraphComponent: React.FC<DynamicSignalGraphProps> = ({ signalName, onDelete }) => {
  ...
  const computeBars = useCallback((hist: { state: string; startTime: number }[]) => {
    const windowMs = 5000;
    ...
    return hist
      .map((item, i) => {
        ...
        const bar = w > 0
          ? { state: item.state, startTime: item.startTime, width: w, xOffset: offset }
          : null;
        if (bar) offset += bar.width;
        return bar;
      })
      .filter(Boolean) as { state: string; startTime: number; width: number; xOffset: number }[];
  }, [isPaused, pausedTime]);

  const bars = computeBars(history);
  const enumVals = getEnumValues(signalName);
  const lastCode = history.length ? history[history.length - 1].state : undefined;
        ...
        {enumVals.map((label, i) => {
          ...
          return (
            <div key={label} className="inline-flex items-center gap-1">
              ...
              <span className={active ? 'font-bold text-black' : 'text-gray-500'}>
                {label}
              </span>
            </div>
          );
