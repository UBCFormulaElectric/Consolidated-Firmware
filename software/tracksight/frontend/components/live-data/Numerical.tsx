...
const NumericalGraphComponent: React.FC<DynamicSignalGraphProps> = ({ signalName, onDelete }) => {
  ...
  const availableOptions = React.useMemo(
    () => availableSignals.filter(s => !activeSignals.includes(s.name) && isNumericalSignal(s.name)),
  ...
  const width = Math.max(chartData.length * pixelPerPoint, 100) * (scaleFactor / 100);
  ...
  const handleUnsub = useCallback((name: string) => unsubscribeFromSignal(name), [unsubscribeFromSignal]);
  ...
  return (
    <div className="mb-6 p-4 relative">
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        {availableOptions.length > 0 && (
          <div className="relative">
            <PlusButton onClick={() => setIsSearchOpen(o => !o)} variant="rowSide" />
            {isSearchOpen && (
      ...
      {chartData.length === 0
        ? <div className="w-full h-[256px] flex items-center justify-center text-gray-500">No data</div>
        : (
          <div style={{ width, height: chartHeight, transition: 'width 100ms ease-out' }}>
            <AreaChart width={width} height={chartHeight} data={chartData}>
              <XAxis dataKey="time" tickFormatter={t => new Date(t).toLocaleTimeString()} interval={Math.ceil(chartData.length/10)} />
              <YAxis domain={[0, 'auto']} />
              <Tooltip isAnimationActive={false} labelFormatter={time => new Date(time).toLocaleString()} />
              {numericalSignals.map((sig, i) => (
                <Area
                  key={sig}
                  type="monotone"
