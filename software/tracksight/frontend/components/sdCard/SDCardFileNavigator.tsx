...
const SDCardFileNavigator = (props: SDCardFileNavigatorProps) => {
        ...
        setSelectedFiles((prev) => {
            if (selectRange && anchor && availableFiles.data) {
                ...
                if (anchorIndex !== -1 && fileIndex !== -1) {
                    const range = availableFiles.data.slice(Math.min(anchorIndex, fileIndex), Math.max(anchorIndex, fileIndex) + 1);
    ...
    const handleToggleAll = () => {
        setSelectedFiles((prev) => (prev.length === availableFiles.data?.length ? [] : (availableFiles.data ?? [])));
        selectionAnchor.current = null;
    };
    ...
    const handleDump = async (filesToDump: SDCardFile[], overwrite: boolean = false) => {
        ...
        for (let index = 0; index < filesToDump.length; index += 1) {
            ...
            try {
                ...
                setDumpError({
                    message: SD_CARD_ERROR_MESSAGES[statusCode]?.replace("{{fileName}}", file) ?? message,
                    statusCode,
                    fileName: file,
                });
    ...
    return (
        <div className="w-full h-full flex flex-row gap-8">
            <SDCardFileList files={files} selectedFiles={selectedFiles} onToggleFile={handleToggleFile} onToggleAll={handleToggleAll} isLoading={isLoadingFiles} noSDCardSelected={noSDCardSelected} hasError={hasError} />
            <SDCardFileInfo
                            ...
                            ? ([
                                  {
                                      ...
                                      onClick: () => {
                                          ...
                                          setDumpError(null);
                                          setSelectedFiles((currentSelectedFiles) => currentSelectedFiles.filter((selectedFile) => selectedFile !== pendingDump.fileName));
                                          setPendingDump(null);
