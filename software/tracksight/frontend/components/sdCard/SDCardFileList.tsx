...
const SDCardFileListFrame = (props: { children: React.ReactNode; headerAction?: React.ReactNode }) => {
    return (
        <div className="border border-black rounded h-full flex flex-col overflow-hidden" style={PANEL_SIZE}>
            ...
            <div className="flex-1 min-h-0 overflow-y-scroll overscroll-contain scrollbar-hidden -mb-px">{props.children}</div>
        </div>
    );
};
...
const SDCardFileListMessage = (props: { children: React.ReactNode; tone?: "muted" | "error" }) => {
    ...
    return (
        <SDCardFileListFrame>
            <div className={`p-4 h-full flex items-center justify-center ${toneClasses}`}>{props.children}</div>
        </SDCardFileListFrame>
    );
};
...
const SDCardFileListSkeleton = () => {
    return (
        <SDCardFileListFrame>
            <div className="flex flex-col items-center justify-center gap-2 p-4 animate-pulse">Loading files...</div>
        </SDCardFileListFrame>
    );
};

const SDCardFileList = (props: SDCardFileListProps) => {
    const { files, selectedFiles, onToggleFile, onToggleAll, isLoading, noSDCardSelected, hasError } = props;
    ...
    if (hasError) {
        return <SDCardFileListMessage tone="error">Failed to read files from this SD card. Please reconnect it and try again.</SDCardFileListMessage>;
    }
        ...
        >
            <div className="flex flex-col">
                {files.map((file) => {
                    ...
                    return (
                        <div key={file} className={`h-min p-4 select-none border-b flex flex-row items-center gap-4 border-black w-full hover:cursor-pointer ${isSelected ? "bg-blue-100" : "hover:bg-blue-100/50"}`} onClick={(event) => onToggleFile(file, event.shiftKey)}>
