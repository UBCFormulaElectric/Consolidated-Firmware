    ...
    return (
        <div className="flex flex-row gap-3">
            {sdCards.map((sdCard) => {
                ...
                return (
                    <button key={sdCard} onClick={() => onSelect(sdCard)} className={`rounded px-4 py-4 text-left flex gap-2 items-center transition-colors hover:cursor-pointer ${isSelected ? "border-blue-500 border bg-blue-100" : "border-black hover:border-black/75 border"}`}>
            ...
            <FormatSDCardButton sdCard={selected || undefined} />
        </div>
    );
};
