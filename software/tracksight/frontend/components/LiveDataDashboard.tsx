...
function LiveDataDashboard() {
  ...
  return (
    <div className="w-full h-fit overflow-x-scroll scrollbar-hidden overscroll-x-none">
      <div className="flex h-full min-w-[200vw] flex-col gap-16" ref={containerRef}>
        {widgets.map((widget, index) => (
          <div className="sticky left-0 w-screen" style={{ zIndex: widgets.length - index }} key={index}>
            <Widget {...widget} />
          </div>
        ))}
      </div>
