...
export const EnumSignalPicker = memo(function EnumSignalPicker(props: { query: string; selectedSignalName: string | null; onQueryChange: (value: string) => void; onSelectSignal: (signal: EnumSignalMetadata | BooleanSignalMetadata) => void }) {
    ...
    const searchableSignals = useMemo(() => {
        return availableSignals.map((signal) => ({
            signal,
            searchableText: isEnumSignalMetadata(signal) ? normalizeSearchText([signal.name, signal.msg_name, signal.tx_node, signal?.enum_signal?.enum_name].filter(Boolean).join(" ")) : isBooleanSignalMetadata(signal) ? normalizeSearchText([signal.name, signal.msg_name, signal.tx_node].filter(Boolean).join(" ")) : "",
        }));
    }, [availableSignals]);
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="flex items-center justify-between gap-3 w-full">
                                                ...
                                                {isEnumSignalMetadata(signal) ? <span className="text-xs uppercase min-w-0 truncate tracking-wide text-gray-500">{signal.enum_signal.enum_name}</span> : <span className="text-xs uppercase min-w-0 truncate tracking-wide text-gray-500">BOOLEAN</span>}
                                            </div>
