...
const MockGraph: React.FC<MockGraphProps> = React.memo(
  ({ onDelete, config }) => {
    ...
    const [newSignalType, setNewSignalType] = useState<SignalType>(
    ...
    const visiblePointsCount = React.useMemo(() => {
      ...
      const earliestTime = globalTimeRange
        ? globalTimeRange.min
        : data.timestamps[0];
      const latestTime = globalTimeRange
        ? globalTimeRange.max
    ...
    }, [data.timestamps, zoomLevel, scrollProgress, globalTimeRange]);
