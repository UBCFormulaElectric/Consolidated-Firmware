...
const processAlertTimelineData = (data: { [signalName: string]: AlertSeries }, cursorIndices: CursorIndices, previousState: AlertTimelineRenderState, populationMap: boolean[], slipStreamLanes: RefObject<number>) => {
...
const hitTestAlerts = (mousePos: MousePosition, state: AlertTimelineRenderState, width: number, height: number, leftEdge: number, rightEdge: number, liveTime: number, slipStreamLanes: number, ctx: CanvasRenderingContext2D): HoverInfo => {
...
const renderAlertTimeline = (ctx: CanvasRenderingContext2D, state: AlertTimelineRenderState, width: number, height: number, leftEdge: number, rightEdge: number, bottom: number, liveTime: number, slipStreamLanes: number, hover: HoverInfo) => {
    ...
    state.forEach((alert) => {
        ...
        ctx.roundRect(alertStartX, laneY, alertEndX - alertStartX, heightPerLane, [isOffLeftEdge ? 0 : SLIP_STREAM_ROUNDING_RADIUS, rightRadius, rightRadius, isOffLeftEdge ? 0 : SLIP_STREAM_ROUNDING_RADIUS]);
        ctx.fill();
        ...
        ctx.fillText(alert.signal, visibleStartX + visibleWidth / 2, laneY + heightPerLane / 2);
        ctx.restore();
    });
...
function AlertTimeline() {
    ...
    const { globalTimeRangeRef, XToTime, timeToX, hoverTimestampRef: externalHoverTimestampRef } = useSyncedGraph();
    ...
    useEffect(() => {
        ...
        const renderSlipStream = () => {
            ...
            hoverInfo.current = hitTestAlerts(mousePos.current, renderState.current, rect.width, rect.height, leftEdge, rightEdge, liveTime, slipStreamLanes.current, ctx);
            ...
            renderAlertTimeline(ctx, renderState.current, rect.width, rect.height, leftEdge, rightEdge, bottom, liveTime, slipStreamLanes.current, hoverInfo.current);
            render_markers(ctx, rect.width, rect.height, getVisibleTelemetryMarkers(XToTime(rect.left), XToTime(rect.right)), timeToXRef.current);

            if (externalHoverTimestampRef.current !== null) {
                render_hover_line(ctx, rect.width, rect.height, externalHoverTimestampRef.current, timeToXRef.current, false);
            }
    ...
    useEffect(() => {
        ...
        const processRenderData = () => {
            ...
            const { previousState: newRenderState, populationMap: newSlipstreamPopulation, cursorIndices: newCursorIndices } = processAlertTimelineData(data.current, cursorIndices.current, renderState.current, slipstreamPopulated.current, slipStreamLanes);
