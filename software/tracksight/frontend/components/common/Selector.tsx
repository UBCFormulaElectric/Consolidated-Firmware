...
function Selector<T extends any>(props: SelectorProps<T>) {
    const { options, selectedOption, onSelect, ItemRenderer, getSearchableText, buttonElement } = props;
    ...
    useEffect(() => {
        ...
        const handleButtonClick = (event: MouseEvent) => {
            if (!buttonElement.current || !buttonElement.current.contains(event.target as Node) || (dropdownRef.current && dropdownRef.current.contains(event.target as Node))) return;
    ...
    return (
        isOpen && (
            <div className="scrollbar-hidden absolute top-full z-10 mt-1 max-h-60 min-w-96 overflow-y-auto rounded border bg-white shadow-lg hover:cursor-auto" ref={dropdownRef}>
                    ...
                    filteredOptions.length > 0 ? (
                        filteredOptions.map(({ item, path: itemPath }, index) => (
                            <div key={index} className="border-b last:border-b-0">
                                {itemPath.length > 0 && <div className="px-4 pt-2 pb-1 text-xs text-gray-500">{itemPath.join(" / ")}</div>}
                                <div
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        onSelect(item);
