/**
 * @note This file is auto-generated. Do not modify!
 */
// clang-format off


/* ------------------------------- Includes ------------------------------- */

#include "app_canAlerts.h"
#include "app_canTx.h"
#include "app_canRx.h"

/* ------------------------- Function Definitions ------------------------- */


// warning setter functions

void app_canAlerts_VC_Warning_StackWaterMarkHighTask1Hz_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_StackWaterMarkHighTask1Hz_get())
    {
        app_canTx_VC_Warning_StackWaterMarkHighTask1HzCount_set(app_canTx_VC_Warning_StackWaterMarkHighTask1HzCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_StackWaterMarkHighTask1Hz_set(set_alert);
}

void app_canAlerts_VC_Warning_StackWaterMarkHighTask100Hz_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_StackWaterMarkHighTask100Hz_get())
    {
        app_canTx_VC_Warning_StackWaterMarkHighTask100HzCount_set(app_canTx_VC_Warning_StackWaterMarkHighTask100HzCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_StackWaterMarkHighTask100Hz_set(set_alert);
}

void app_canAlerts_VC_Warning_StackWaterMarkHighTask1kHz_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_StackWaterMarkHighTask1kHz_get())
    {
        app_canTx_VC_Warning_StackWaterMarkHighTask1kHzCount_set(app_canTx_VC_Warning_StackWaterMarkHighTask1kHzCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_StackWaterMarkHighTask1kHz_set(set_alert);
}

void app_canAlerts_VC_Warning_StackWaterMarkHighTaskCanRx_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_StackWaterMarkHighTaskCanRx_get())
    {
        app_canTx_VC_Warning_StackWaterMarkHighTaskCanRxCount_set(app_canTx_VC_Warning_StackWaterMarkHighTaskCanRxCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_StackWaterMarkHighTaskCanRx_set(set_alert);
}

void app_canAlerts_VC_Warning_StackWaterMarkHighTaskCanTx_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_StackWaterMarkHighTaskCanTx_get())
    {
        app_canTx_VC_Warning_StackWaterMarkHighTaskCanTxCount_set(app_canTx_VC_Warning_StackWaterMarkHighTaskCanTxCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_StackWaterMarkHighTaskCanTx_set(set_alert);
}

void app_canAlerts_VC_Warning_WatchdogTimeout_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_WatchdogTimeout_get())
    {
        app_canTx_VC_Warning_WatchdogTimeoutCount_set(app_canTx_VC_Warning_WatchdogTimeoutCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_WatchdogTimeout_set(set_alert);
}

void app_canAlerts_VC_Warning_TxOverflow_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_TxOverflow_get())
    {
        app_canTx_VC_Warning_TxOverflowCount_set(app_canTx_VC_Warning_TxOverflowCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_TxOverflow_set(set_alert);
}

void app_canAlerts_VC_Warning_RxOverflow_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_RxOverflow_get())
    {
        app_canTx_VC_Warning_RxOverflowCount_set(app_canTx_VC_Warning_RxOverflowCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_RxOverflow_set(set_alert);
}

void app_canAlerts_VC_Warning_RegenNotAvailable_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_RegenNotAvailable_get())
    {
        app_canTx_VC_Warning_RegenNotAvailableCount_set(app_canTx_VC_Warning_RegenNotAvailableCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_RegenNotAvailable_set(set_alert);
}

void app_canAlerts_VC_Warning_ImuInitFailed_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_ImuInitFailed_get())
    {
        app_canTx_VC_Warning_ImuInitFailedCount_set(app_canTx_VC_Warning_ImuInitFailedCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_ImuInitFailed_set(set_alert);
}

void app_canAlerts_VC_Warning_FlowRateInputOutOfRange_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_FlowRateInputOutOfRange_get())
    {
        app_canTx_VC_Warning_FlowRateInputOutOfRangeCount_set(app_canTx_VC_Warning_FlowRateInputOutOfRangeCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_FlowRateInputOutOfRange_set(set_alert);
}

void app_canAlerts_VC_Warning_BatteryRailOvercurrentFault_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_BatteryRailOvercurrentFault_get())
    {
        app_canTx_VC_Warning_BatteryRailOvercurrentFaultCount_set(app_canTx_VC_Warning_BatteryRailOvercurrentFaultCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_BatteryRailOvercurrentFault_set(set_alert);
}

void app_canAlerts_VC_Warning_AccumulatorRailOvercurrentFault_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_AccumulatorRailOvercurrentFault_get())
    {
        app_canTx_VC_Warning_AccumulatorRailOvercurrentFaultCount_set(app_canTx_VC_Warning_AccumulatorRailOvercurrentFaultCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_AccumulatorRailOvercurrentFault_set(set_alert);
}

void app_canAlerts_VC_Warning_LvChargeFault_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_LvChargeFault_get())
    {
        app_canTx_VC_Warning_LvChargeFaultCount_set(app_canTx_VC_Warning_LvChargeFaultCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_LvChargeFault_set(set_alert);
}

void app_canAlerts_VC_Warning_BoostControllerFault_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_BoostControllerFault_get())
    {
        app_canTx_VC_Warning_BoostControllerFaultCount_set(app_canTx_VC_Warning_BoostControllerFaultCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_BoostControllerFault_set(set_alert);
}

void app_canAlerts_VC_Warning_HighNumberOfCanDataLogs_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_HighNumberOfCanDataLogs_get())
    {
        app_canTx_VC_Warning_HighNumberOfCanDataLogsCount_set(app_canTx_VC_Warning_HighNumberOfCanDataLogsCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_HighNumberOfCanDataLogs_set(set_alert);
}

void app_canAlerts_VC_Warning_CanLoggingSdCardNotPresent_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_CanLoggingSdCardNotPresent_get())
    {
        app_canTx_VC_Warning_CanLoggingSdCardNotPresentCount_set(app_canTx_VC_Warning_CanLoggingSdCardNotPresentCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_CanLoggingSdCardNotPresent_set(set_alert);
}

void app_canAlerts_VC_Warning_CanLoggingErrored_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_CanLoggingErrored_get())
    {
        app_canTx_VC_Warning_CanLoggingErroredCount_set(app_canTx_VC_Warning_CanLoggingErroredCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_CanLoggingErrored_set(set_alert);
}

void app_canAlerts_VC_Warning_BrakeAppsDisagreement_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_BrakeAppsDisagreement_get())
    {
        app_canTx_VC_Warning_BrakeAppsDisagreementCount_set(app_canTx_VC_Warning_BrakeAppsDisagreementCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_BrakeAppsDisagreement_set(set_alert);
}

void app_canAlerts_VC_Warning_SbgInitFailed_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Warning_SbgInitFailed_get())
    {
        app_canTx_VC_Warning_SbgInitFailedCount_set(app_canTx_VC_Warning_SbgInitFailedCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Warning_SbgInitFailed_set(set_alert);
}

void app_canAlerts_VC_Fault_MissingBMSHeartbeat_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Fault_MissingBMSHeartbeat_get())
    {
        app_canTx_VC_Fault_MissingBMSHeartbeatCount_set(app_canTx_VC_Fault_MissingBMSHeartbeatCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Fault_MissingBMSHeartbeat_set(set_alert);
}

void app_canAlerts_VC_Fault_MissingFSMHeartbeat_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Fault_MissingFSMHeartbeat_get())
    {
        app_canTx_VC_Fault_MissingFSMHeartbeatCount_set(app_canTx_VC_Fault_MissingFSMHeartbeatCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Fault_MissingFSMHeartbeat_set(set_alert);
}

void app_canAlerts_VC_Fault_MissingRSMHeartbeat_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Fault_MissingRSMHeartbeat_get())
    {
        app_canTx_VC_Fault_MissingRSMHeartbeatCount_set(app_canTx_VC_Fault_MissingRSMHeartbeatCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Fault_MissingRSMHeartbeat_set(set_alert);
}

void app_canAlerts_VC_Fault_MissingCRITHeartbeat_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Fault_MissingCRITHeartbeat_get())
    {
        app_canTx_VC_Fault_MissingCRITHeartbeatCount_set(app_canTx_VC_Fault_MissingCRITHeartbeatCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Fault_MissingCRITHeartbeat_set(set_alert);
}

void app_canAlerts_VC_Fault_LeftInverterFault_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Fault_LeftInverterFault_get())
    {
        app_canTx_VC_Fault_LeftInverterFaultCount_set(app_canTx_VC_Fault_LeftInverterFaultCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Fault_LeftInverterFault_set(set_alert);
}

void app_canAlerts_VC_Fault_RightInverterFault_set(bool set_alert)
{
    // Increment alert counter.
    if (set_alert && !app_canTx_VC_Fault_RightInverterFault_get())
    {
        app_canTx_VC_Fault_RightInverterFaultCount_set(app_canTx_VC_Fault_RightInverterFaultCount_get() + 1);
    }
    
    // Set alert.
    app_canTx_VC_Fault_RightInverterFault_set(set_alert);
}



// warning getter functions

bool app_canAlerts_VC_Warning_StackWaterMarkHighTask1Hz_get()
{
    return app_canTx_VC_Warning_StackWaterMarkHighTask1Hz_get();
}

bool app_canAlerts_VC_Warning_StackWaterMarkHighTask100Hz_get()
{
    return app_canTx_VC_Warning_StackWaterMarkHighTask100Hz_get();
}

bool app_canAlerts_VC_Warning_StackWaterMarkHighTask1kHz_get()
{
    return app_canTx_VC_Warning_StackWaterMarkHighTask1kHz_get();
}

bool app_canAlerts_VC_Warning_StackWaterMarkHighTaskCanRx_get()
{
    return app_canTx_VC_Warning_StackWaterMarkHighTaskCanRx_get();
}

bool app_canAlerts_VC_Warning_StackWaterMarkHighTaskCanTx_get()
{
    return app_canTx_VC_Warning_StackWaterMarkHighTaskCanTx_get();
}

bool app_canAlerts_VC_Warning_WatchdogTimeout_get()
{
    return app_canTx_VC_Warning_WatchdogTimeout_get();
}

bool app_canAlerts_VC_Warning_TxOverflow_get()
{
    return app_canTx_VC_Warning_TxOverflow_get();
}

bool app_canAlerts_VC_Warning_RxOverflow_get()
{
    return app_canTx_VC_Warning_RxOverflow_get();
}

bool app_canAlerts_VC_Warning_RegenNotAvailable_get()
{
    return app_canTx_VC_Warning_RegenNotAvailable_get();
}

bool app_canAlerts_VC_Warning_ImuInitFailed_get()
{
    return app_canTx_VC_Warning_ImuInitFailed_get();
}

bool app_canAlerts_VC_Warning_FlowRateInputOutOfRange_get()
{
    return app_canTx_VC_Warning_FlowRateInputOutOfRange_get();
}

bool app_canAlerts_VC_Warning_BatteryRailOvercurrentFault_get()
{
    return app_canTx_VC_Warning_BatteryRailOvercurrentFault_get();
}

bool app_canAlerts_VC_Warning_AccumulatorRailOvercurrentFault_get()
{
    return app_canTx_VC_Warning_AccumulatorRailOvercurrentFault_get();
}

bool app_canAlerts_VC_Warning_LvChargeFault_get()
{
    return app_canTx_VC_Warning_LvChargeFault_get();
}

bool app_canAlerts_VC_Warning_BoostControllerFault_get()
{
    return app_canTx_VC_Warning_BoostControllerFault_get();
}

bool app_canAlerts_VC_Warning_HighNumberOfCanDataLogs_get()
{
    return app_canTx_VC_Warning_HighNumberOfCanDataLogs_get();
}

bool app_canAlerts_VC_Warning_CanLoggingSdCardNotPresent_get()
{
    return app_canTx_VC_Warning_CanLoggingSdCardNotPresent_get();
}

bool app_canAlerts_VC_Warning_CanLoggingErrored_get()
{
    return app_canTx_VC_Warning_CanLoggingErrored_get();
}

bool app_canAlerts_VC_Warning_BrakeAppsDisagreement_get()
{
    return app_canTx_VC_Warning_BrakeAppsDisagreement_get();
}

bool app_canAlerts_VC_Warning_SbgInitFailed_get()
{
    return app_canTx_VC_Warning_SbgInitFailed_get();
}

bool app_canAlerts_VC_Fault_MissingBMSHeartbeat_get()
{
    return app_canTx_VC_Fault_MissingBMSHeartbeat_get();
}

bool app_canAlerts_VC_Fault_MissingFSMHeartbeat_get()
{
    return app_canTx_VC_Fault_MissingFSMHeartbeat_get();
}

bool app_canAlerts_VC_Fault_MissingRSMHeartbeat_get()
{
    return app_canTx_VC_Fault_MissingRSMHeartbeat_get();
}

bool app_canAlerts_VC_Fault_MissingCRITHeartbeat_get()
{
    return app_canTx_VC_Fault_MissingCRITHeartbeat_get();
}

bool app_canAlerts_VC_Fault_LeftInverterFault_get()
{
    return app_canTx_VC_Fault_LeftInverterFault_get();
}

bool app_canAlerts_VC_Fault_RightInverterFault_get()
{
    return app_canTx_VC_Fault_RightInverterFault_get();
}



bool app_canAlerts_BoardHasWarning(CanAlertBoard board)
{
    switch (board)
    {
        
        case BMS_ALERT_BOARD :
        {
            if (app_canRx_BMS_Warning_StackWaterMarkHighTask1Hz_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_StackWaterMarkHighTask100Hz_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_StackWaterMarkHighTask1kHz_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_StackWaterMarkHighTaskCanRx_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_StackWaterMarkHighTaskCanTx_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_WatchdogTimeout_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_TxOverflow_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_RxOverflow_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_OpenWireCheckFault_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_OpenWireCheck_Segment0_GND_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_OpenWireCheck_Segment1_GND_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_OpenWireCheck_Segment2_GND_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_OpenWireCheck_Segment3_GND_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_OpenWireCheck_Segment4_GND_get()) 
            {
                return true;
            }if (app_canRx_BMS_Warning_MissingVCHeartbeat_get()) 
            {
                return true;
            }
            break;
        }
        
        case FSM_ALERT_BOARD :
        {
            if (app_canRx_FSM_Warning_WatchdogTimeout_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_TxOverflow_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_RxOverflow_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_StackWaterMarkHighTask1Hz_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_StackWaterMarkHighTask100Hz_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_StackWaterMarkHighTask1kHz_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_StackWaterMarkHighTaskCanRx_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_StackWaterMarkHighTaskCanTx_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_SteeringAngleOCSC_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_SteeringAngleOutOfRange_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_LeftWheelSpeedOutOfRange_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_RightWheelSpeedOutOfRange_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_AppsDisagreement_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_FrontBrakePressureOutOfRange_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_RearBrakePressureOutOfRange_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_FrontBrakePressureOcSc_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_RearBrakePressureOcSc_get()) 
            {
                return true;
            }if (app_canRx_FSM_Warning_BrakeOcScNotOk_get()) 
            {
                return true;
            }
            break;
        }
        
        case VC_ALERT_BOARD :
        {
            if (app_canTx_VC_Warning_StackWaterMarkHighTask1Hz_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_StackWaterMarkHighTask100Hz_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_StackWaterMarkHighTask1kHz_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_StackWaterMarkHighTaskCanRx_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_StackWaterMarkHighTaskCanTx_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_WatchdogTimeout_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_TxOverflow_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_RxOverflow_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_RegenNotAvailable_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_ImuInitFailed_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_FlowRateInputOutOfRange_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_BatteryRailOvercurrentFault_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_AccumulatorRailOvercurrentFault_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_LvChargeFault_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_BoostControllerFault_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_HighNumberOfCanDataLogs_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_CanLoggingSdCardNotPresent_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_CanLoggingErrored_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_BrakeAppsDisagreement_get()) 
            {
                return true;
            }if (app_canTx_VC_Warning_SbgInitFailed_get()) 
            {
                return true;
            }
            break;
        }
        
        
        default:
        {
            // Do nothing
            break;
        }
    }
    
    return false;
}

bool app_canAlerts_BoardHasFault(CanAlertBoard board)
{
    switch (board)
    {
        case BMS_ALERT_BOARD :
        {
            if (app_canRx_BMS_Fault_StateMachine_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_CellUndervoltage_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_CellOvervoltage_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_ModuleCommunicationError_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_CellUndertemp_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_CellOvertemp_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_ChargerReportedError_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_ChargerDisconnectedDuringCharge_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_ChargerShutdownLoopOpen_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_TractiveSystemOvercurrent_get()) 
            {
                return true;
            }if (app_canRx_BMS_Fault_PrechargeFailure_get()) 
            {
                return true;
            }
            break;
        }
        
        case FSM_ALERT_BOARD :
        {
            if (app_canRx_FSM_Fault_PappsOCSC_get()) 
            {
                return true;
            }if (app_canRx_FSM_Fault_SappsOCSC_get()) 
            {
                return true;
            }
            break;
        }
        
        case VC_ALERT_BOARD :
        {
            if (app_canTx_VC_Fault_MissingBMSHeartbeat_get()) 
            {
                return true;
            }if (app_canTx_VC_Fault_MissingFSMHeartbeat_get()) 
            {
                return true;
            }if (app_canTx_VC_Fault_MissingRSMHeartbeat_get()) 
            {
                return true;
            }if (app_canTx_VC_Fault_MissingCRITHeartbeat_get()) 
            {
                return true;
            }if (app_canTx_VC_Fault_LeftInverterFault_get()) 
            {
                return true;
            }if (app_canTx_VC_Fault_RightInverterFault_get()) 
            {
                return true;
            }
            break;
        }
        
        
        default:
        {
            // Do nothing
            break;
        }
    }
    
    return false;
}

bool app_canAlerts_AnyBoardHasWarning()
{
    if (app_canAlerts_BoardHasWarning(VC_ALERT_BOARD))
    {
        return true;
    }
    
    
    return false;
}

bool app_canAlerts_AnyBoardHasFault()
{
    if (app_canAlerts_BoardHasFault(VC_ALERT_BOARD))
    {
        return true;
    }
    
    
    return false;
}

uint8_t app_canAlerts_WarningInfo(Fault_Warning_Info *alert_array)
{
    uint8_t element_num = 0;
    // tx faults
    
    if (app_canRx_BMS_Warning_StackWaterMarkHighTask1Hz_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_StackWaterMarkHighTask1Hz";
        alert_array[element_num].description = "1Hz task stack watermark exceeded.";
        alert_array[element_num].id = 101;
        element_num++;
    }if (app_canRx_BMS_Warning_StackWaterMarkHighTask100Hz_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_StackWaterMarkHighTask100Hz";
        alert_array[element_num].description = "100Hz task stack watermark exceeded.";
        alert_array[element_num].id = 102;
        element_num++;
    }if (app_canRx_BMS_Warning_StackWaterMarkHighTask1kHz_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_StackWaterMarkHighTask1kHz";
        alert_array[element_num].description = "1kHz task stack watermark exceeded.";
        alert_array[element_num].id = 103;
        element_num++;
    }if (app_canRx_BMS_Warning_StackWaterMarkHighTaskCanRx_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_StackWaterMarkHighTaskCanRx";
        alert_array[element_num].description = "CAN RX task stack watermark exceeded.";
        alert_array[element_num].id = 104;
        element_num++;
    }if (app_canRx_BMS_Warning_StackWaterMarkHighTaskCanTx_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_StackWaterMarkHighTaskCanTx";
        alert_array[element_num].description = "CAN TX task stack watermark exceeded.";
        alert_array[element_num].id = 105;
        element_num++;
    }if (app_canRx_BMS_Warning_WatchdogTimeout_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_WatchdogTimeout";
        alert_array[element_num].description = "Watchdog timeout detected.";
        alert_array[element_num].id = 106;
        element_num++;
    }if (app_canRx_BMS_Warning_TxOverflow_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_TxOverflow";
        alert_array[element_num].description = "CAN TX queue has overflowed.";
        alert_array[element_num].id = 107;
        element_num++;
    }if (app_canRx_BMS_Warning_RxOverflow_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_RxOverflow";
        alert_array[element_num].description = "CAN RX queue has overflowed.";
        alert_array[element_num].id = 108;
        element_num++;
    }if (app_canRx_BMS_Warning_OpenWireCheckFault_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_OpenWireCheckFault";
        alert_array[element_num].description = "Open wire cell detected (cell voltage tap is open circuit).";
        alert_array[element_num].id = 109;
        element_num++;
    }if (app_canRx_BMS_Warning_OpenWireCheck_Segment0_GND_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_OpenWireCheck_Segment0_GND";
        alert_array[element_num].description = "Open wire cell detected in segment 0 (cell voltage tap is open circuit).";
        alert_array[element_num].id = 110;
        element_num++;
    }if (app_canRx_BMS_Warning_OpenWireCheck_Segment1_GND_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_OpenWireCheck_Segment1_GND";
        alert_array[element_num].description = "Open wire cell detected in segment 1 (cell voltage tap is open circuit).";
        alert_array[element_num].id = 111;
        element_num++;
    }if (app_canRx_BMS_Warning_OpenWireCheck_Segment2_GND_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_OpenWireCheck_Segment2_GND";
        alert_array[element_num].description = "Open wire cell detected in segment 2 (cell voltage tap is open circuit).";
        alert_array[element_num].id = 112;
        element_num++;
    }if (app_canRx_BMS_Warning_OpenWireCheck_Segment3_GND_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_OpenWireCheck_Segment3_GND";
        alert_array[element_num].description = "Open wire cell detected in segment 3 (cell voltage tap is open circuit).";
        alert_array[element_num].id = 113;
        element_num++;
    }if (app_canRx_BMS_Warning_OpenWireCheck_Segment4_GND_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_OpenWireCheck_Segment4_GND";
        alert_array[element_num].description = "Open wire cell detected in segment 4 (cell voltage tap is open circuit).";
        alert_array[element_num].id = 114;
        element_num++;
    }if (app_canRx_BMS_Warning_MissingVCHeartbeat_get()) // rx BMS faults
    {
        alert_array[element_num].name = "BMS_Warning_MissingVCHeartbeat";
        alert_array[element_num].description = "Missing heartbeat from the VC.";
        alert_array[element_num].id = 115;
        element_num++;
    }
    if (app_canRx_FSM_Warning_WatchdogTimeout_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_WatchdogTimeout";
        alert_array[element_num].description = "Watchdog timed out.";
        alert_array[element_num].id = 301;
        element_num++;
    }if (app_canRx_FSM_Warning_TxOverflow_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_TxOverflow";
        alert_array[element_num].description = "CanTx queue overflowed.";
        alert_array[element_num].id = 302;
        element_num++;
    }if (app_canRx_FSM_Warning_RxOverflow_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_RxOverflow";
        alert_array[element_num].description = "CanRx queue overflowed.";
        alert_array[element_num].id = 303;
        element_num++;
    }if (app_canRx_FSM_Warning_StackWaterMarkHighTask1Hz_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_StackWaterMarkHighTask1Hz";
        alert_array[element_num].description = "Stack watermark failed on 1Hz task.";
        alert_array[element_num].id = 304;
        element_num++;
    }if (app_canRx_FSM_Warning_StackWaterMarkHighTask100Hz_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_StackWaterMarkHighTask100Hz";
        alert_array[element_num].description = "Stack watermark failed on 100Hz task.";
        alert_array[element_num].id = 305;
        element_num++;
    }if (app_canRx_FSM_Warning_StackWaterMarkHighTask1kHz_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_StackWaterMarkHighTask1kHz";
        alert_array[element_num].description = "Stack watermark failed on 1kHz task.";
        alert_array[element_num].id = 306;
        element_num++;
    }if (app_canRx_FSM_Warning_StackWaterMarkHighTaskCanRx_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_StackWaterMarkHighTaskCanRx";
        alert_array[element_num].description = "Stack watermark failed on canRx task.";
        alert_array[element_num].id = 307;
        element_num++;
    }if (app_canRx_FSM_Warning_StackWaterMarkHighTaskCanTx_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_StackWaterMarkHighTaskCanTx";
        alert_array[element_num].description = "Stack watermark failed on canTx task.";
        alert_array[element_num].id = 308;
        element_num++;
    }if (app_canRx_FSM_Warning_SteeringAngleOCSC_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_SteeringAngleOCSC";
        alert_array[element_num].description = "Steering sensor voltage out of acceptable range (OCSC).";
        alert_array[element_num].id = 309;
        element_num++;
    }if (app_canRx_FSM_Warning_SteeringAngleOutOfRange_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_SteeringAngleOutOfRange";
        alert_array[element_num].description = "Steering angle out of range.";
        alert_array[element_num].id = 310;
        element_num++;
    }if (app_canRx_FSM_Warning_LeftWheelSpeedOutOfRange_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_LeftWheelSpeedOutOfRange";
        alert_array[element_num].description = "Left wheel speed out of range.";
        alert_array[element_num].id = 311;
        element_num++;
    }if (app_canRx_FSM_Warning_RightWheelSpeedOutOfRange_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_RightWheelSpeedOutOfRange";
        alert_array[element_num].description = "Right wheel speed out of range.";
        alert_array[element_num].id = 312;
        element_num++;
    }if (app_canRx_FSM_Warning_AppsDisagreement_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_AppsDisagreement";
        alert_array[element_num].description = "Conflict between primary and secondary accelerator pedals.";
        alert_array[element_num].id = 313;
        element_num++;
    }if (app_canRx_FSM_Warning_FrontBrakePressureOutOfRange_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_FrontBrakePressureOutOfRange";
        alert_array[element_num].description = "Front brake pressure out of range.";
        alert_array[element_num].id = 314;
        element_num++;
    }if (app_canRx_FSM_Warning_RearBrakePressureOutOfRange_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_RearBrakePressureOutOfRange";
        alert_array[element_num].description = "Rear brake pressure out of range.";
        alert_array[element_num].id = 315;
        element_num++;
    }if (app_canRx_FSM_Warning_FrontBrakePressureOcSc_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_FrontBrakePressureOcSc";
        alert_array[element_num].description = "Front brake pressure sensor voltage out of acceptable range (OCSC).";
        alert_array[element_num].id = 316;
        element_num++;
    }if (app_canRx_FSM_Warning_RearBrakePressureOcSc_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_RearBrakePressureOcSc";
        alert_array[element_num].description = "Rear brake pressure sensor voltage out of acceptable range (OCSC).";
        alert_array[element_num].id = 322;
        element_num++;
    }if (app_canRx_FSM_Warning_BrakeOcScNotOk_get()) // rx FSM faults
    {
        alert_array[element_num].name = "FSM_Warning_BrakeOcScNotOk";
        alert_array[element_num].description = "BRAKE_OCSC_OK line routed to BMS for BSPD is not OK.";
        alert_array[element_num].id = 323;
        element_num++;
    }
    
    if (app_canTx_VC_Warning_StackWaterMarkHighTask1Hz_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_StackWaterMarkHighTask1Hz";
        alert_array[element_num].description = "1Hz task stack watermark exceeded.";
        alert_array[element_num].id = 501;
        element_num++;
    }
    if (app_canTx_VC_Warning_StackWaterMarkHighTask100Hz_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_StackWaterMarkHighTask100Hz";
        alert_array[element_num].description = "100Hz task stack watermark exceeded.";
        alert_array[element_num].id = 502;
        element_num++;
    }
    if (app_canTx_VC_Warning_StackWaterMarkHighTask1kHz_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_StackWaterMarkHighTask1kHz";
        alert_array[element_num].description = "1kHz task stack watermark exceeded.";
        alert_array[element_num].id = 503;
        element_num++;
    }
    if (app_canTx_VC_Warning_StackWaterMarkHighTaskCanRx_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_StackWaterMarkHighTaskCanRx";
        alert_array[element_num].description = "CAN RX task stack watermark exceeded.";
        alert_array[element_num].id = 504;
        element_num++;
    }
    if (app_canTx_VC_Warning_StackWaterMarkHighTaskCanTx_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_StackWaterMarkHighTaskCanTx";
        alert_array[element_num].description = "CAN TX task stack watermark exceeded.";
        alert_array[element_num].id = 505;
        element_num++;
    }
    if (app_canTx_VC_Warning_WatchdogTimeout_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_WatchdogTimeout";
        alert_array[element_num].description = "Watchdog timeout detected.";
        alert_array[element_num].id = 506;
        element_num++;
    }
    if (app_canTx_VC_Warning_TxOverflow_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_TxOverflow";
        alert_array[element_num].description = "CAN TX queue has overflowed.";
        alert_array[element_num].id = 507;
        element_num++;
    }
    if (app_canTx_VC_Warning_RxOverflow_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_RxOverflow";
        alert_array[element_num].description = "CAN RX queue has overflowed.";
        alert_array[element_num].id = 508;
        element_num++;
    }
    if (app_canTx_VC_Warning_RegenNotAvailable_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_RegenNotAvailable";
        alert_array[element_num].description = "Regen is not currently available.";
        alert_array[element_num].id = 509;
        element_num++;
    }
    if (app_canTx_VC_Warning_ImuInitFailed_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_ImuInitFailed";
        alert_array[element_num].description = "Initializing the embedded IMU failed.";
        alert_array[element_num].id = 516;
        element_num++;
    }
    if (app_canTx_VC_Warning_FlowRateInputOutOfRange_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_FlowRateInputOutOfRange";
        alert_array[element_num].description = "Input to app_pumpControl_setFlowRate out of range";
        alert_array[element_num].id = 517;
        element_num++;
    }
    if (app_canTx_VC_Warning_BatteryRailOvercurrentFault_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_BatteryRailOvercurrentFault";
        alert_array[element_num].description = "LV rail from the 18650 batteries was overcurrent.";
        alert_array[element_num].id = 557;
        element_num++;
    }
    if (app_canTx_VC_Warning_AccumulatorRailOvercurrentFault_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_AccumulatorRailOvercurrentFault";
        alert_array[element_num].description = "LV rail from the HV DCDC was overcurrent.";
        alert_array[element_num].id = 558;
        element_num++;
    }
    if (app_canTx_VC_Warning_LvChargeFault_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_LvChargeFault";
        alert_array[element_num].description = "LV battery charger has faulted.";
        alert_array[element_num].id = 559;
        element_num++;
    }
    if (app_canTx_VC_Warning_BoostControllerFault_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_BoostControllerFault";
        alert_array[element_num].description = "Boost controller has faulted.";
        alert_array[element_num].id = 560;
        element_num++;
    }
    if (app_canTx_VC_Warning_HighNumberOfCanDataLogs_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_HighNumberOfCanDataLogs";
        alert_array[element_num].description = "High number of CAN data logs (>200) on the SD card, which slows down mounting the filesystem.";
        alert_array[element_num].id = 561;
        element_num++;
    }
    if (app_canTx_VC_Warning_CanLoggingSdCardNotPresent_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_CanLoggingSdCardNotPresent";
        alert_array[element_num].description = "CAN logging SD card is not detected.";
        alert_array[element_num].id = 562;
        element_num++;
    }
    if (app_canTx_VC_Warning_CanLoggingErrored_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_CanLoggingErrored";
        alert_array[element_num].description = "CAN logging received multiple errors and has been disabled.";
        alert_array[element_num].id = 563;
        element_num++;
    }
    if (app_canTx_VC_Warning_BrakeAppsDisagreement_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_BrakeAppsDisagreement";
        alert_array[element_num].description = "Brakes and apps are active at same time.";
        alert_array[element_num].id = 564;
        element_num++;
    }
    if (app_canTx_VC_Warning_SbgInitFailed_get()) // tx VC faults
    {
        alert_array[element_num].name = "VC_Warning_SbgInitFailed";
        alert_array[element_num].description = "Initializing the SBG Ellipse GPS failed";
        alert_array[element_num].id = 565;
        element_num++;
    }return element_num;
    
}

uint8_t app_canAlerts_FaultInfo(Fault_Warning_Info *alert_array)
{
    uint8_t element_num = 0;
    // tx faults
    
    
    
    return element_num;
    
}
