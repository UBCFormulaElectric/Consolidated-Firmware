...
ID | Title | Description | Associated Competition Rule(s)
--- | --- | --- | ---
BMS-34 | SoC calculation and storage | - The BMS must transmit SoC over CAN at 100Hz or faster. <br/> - The BMS must perform coulomb counting and calculate SoC at 100Hz. <br/> The BMS must store the same SoC value in three different EEPROM memory sections at 1Hz. <br/> - SoC must be bounded between 0% and 100%. <br/> - After charging is complete, the BMS must wait for the minimum cell voltage to settle. Then, the BMS shall determine the SoC with the minimum cell voltage using the OCV to cell capacity lookup table. The SoC stored on the EEPROM must be updated.
