...
DCM-19 | Torque request calculation | - The DCM may only request torque less than or equal to what the driver requested. <br/> - If regen is allowed and the regen request is above 0%, the DCM must map the regen request to a negative torque request (prioritize regen paddle over accelerator pedal). <br/> - Else, the DCM must map the mapped pedal percentage to a positive torque request. | EV.3.2.3
...
DIM-7 | Regen request reporting | The DIM must report the regen request percentage over CAN at 1kHz.
DIM-8 | Regen request calculation | - The DIM must linearly map the regen paddle position to regen request as a percentage <br/> - The mapping must have with upper and lower dead zones (paddle <=5% depressed maps to 0% regen request, paddle >=95% pressed maps to 100% regen request).
