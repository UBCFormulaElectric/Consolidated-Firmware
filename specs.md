...
DIM-8 | Regen request calculation | - The DIM must linearly map the regen paddle position to regen request as a percentage <br/> - The mapping must have upper and lower dead zones (paddle <=5% depressed maps to 0% regen request, paddle >=95% pressed maps to 100% regen request).
