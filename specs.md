...
DCM-13 | Torque request sending | The DCM must do the following in the drive state at 100Hz or faster: <br/> 1. Acquire each motor's RPM from the inverters over CAN. <br/> 2. Calculate a torque request as per DCM-19. <br/> 3. Decrease the torque request if necessary to meet both DCM-6 and the maximum power limits specified by the BMS. <br/> 4. Send the resultant torque request to both inverters over CAN.
...
PDM-6 | Voltage sense rationality checks | The PDM must run voltage rationality checks at 1kHz or faster on the following inputs, throwing a non-critical fault if a rationality check fails: <br/> - VBAT_SENSE: min =  6V, max = 8.5V. <br/> - 24V_AUX_SENSE: min = 22V, max = 26V. <br/> - 24V_ACC_SENSE: min = 22V, max = 26V.
...
BMS-4 | Cell voltages acquisition and logging | - The BMS must acquire all cell voltages over isoSPI at 100Hz or faster. <br/> - The BMS must log the highest cell, lowest cell, average cell, pack, and segment voltages at 100Hz or faster.
BMS-5 | Cell temperatures acquisition and logging | - The BMS must acquire all cell temperatures over isoSPI at 1Hz or faster. <br/> - The BMS must log the highest, lowest and average temperatures at 1Hz or faster.
...
BMS-9 | Charger detection and logging | - The BMS must check the charger connection status at 1Hz or faster by the state of the CHARGE_STATE_3V3 digital input. <br/> - The BMS must log the charger connection status over CAN at 1Hz or faster.
...
BMS-11 | Contactor weld/stuck open detection | The BMS must check that the contactors are in the desired open or closed state at 1kHz or faster, and if not the BMS must throw an AIR shutdown fault and enter the fault state.
...
ID | Title | Description | Associated Competition Rule(s)
--- | --- | --- | ---
BMS-34 | SoC calculation and storage | - The BMS must transmit SoC over CAN at 100Hz or faster. <br/> - The BMS must perform coulomb counting and calculate SoC at 100Hz or faster. <br/> The BMS must store the same SoC value in three different EEPROM memory sections at 1Hz or faster. <br/> - SoC must be bounded between 0% and 100%. <br/> - After charging is complete, the BMS must reset and store SoC as 100%.
